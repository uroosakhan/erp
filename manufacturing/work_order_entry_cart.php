...
include_once($path_to_root . "/manufacturing/includes/work_order_entry_ui_cart.inc");
...
if (isset($_GET['AddedID']))
{
 ...
 submenu_option(_("Enter New Work Order"),
            "/manufacturing/work_order_entry_cart.php");
...
function handle_new_order($WorkOrder)
{
	...
	if (isset($_SESSION['WorkOrder']))
	{
		$_SESSION['WorkOrder']->clear_items();
		unset($_SESSION['WorkOrder']);
	}

	$_SESSION['WorkOrder'] = new items_cart_wo(ST_WORKORDER, $WorkOrder);
  	$_SESSION['WorkOrder']->fixed_asset = isset($_GET['FixedAsset']);
	$_POST['AdjDate'] = new_doc_date();
	if (!is_date_in_fiscalyear($_POST['AdjDate']))
		$_POST['AdjDate'] = end_fiscalyear();
	$_SESSION['WorkOrder']->tran_date = $_POST['AdjDate'];
    copy_from_wo_cart();
}
...
function can_process()
{
    ...
    if (!($_POST['type'] == WO_ADVANCED)) {
        if ( count($_SESSION['WorkOrder']->line_item_wo) == 0) {
...
//                while ($bom_item = db_fetch($result))
                foreach ($_SESSION['WorkOrder']->line_item_wo as $line_no=>$stock_item) {
...
if (isset($_POST['Process']) && can_process())
{
    ...
    $AllowCart = 1;
    $id = add_work_order($_POST['wo_ref'], $_POST['StockLocation'],
        ...
        $_SESSION['WorkOrder'], $AllowCart);

    new_doc_date($_POST['date_']);

	$_SESSION['WorkOrder']->clear_items();
	unset($_SESSION['WorkOrder']);
    meta_forward($_SERVER['PHP_SELF'], "AddedID=$id&type=".$_POST['type_']."&date=".$_POST['date_']);
} /*end of process credit note */

//-----------------------------------------------------------------------------------------------
$Edit = find_submit('Edit');
$Delete = find_submit('Delete');
// display_error("asdsadsa".$_POST['stock_id_']);
...
if($ItemSearch == 1) {
    if(isset($_POST['FetchItems'])) {
        ...
        while ($myrow = db_fetch($result)) {
            add_to_order($_SESSION['WorkOrder'], $myrow['component'], input_num('quantity'),
...
{
	$id = $_POST['LineNo'];
   	$_SESSION['WorkOrder']->update_cart_item($id, input_num('quantity'),
...
{
	$_SESSION['WorkOrder']->remove_from_cart($id);
	line_start_focus();
}
//-----------------------------------------------------------------------------------------------
function handle_new_item()
{
    ...
    if($_POST['stock_id_'] != '') {
        ...
        //         $_POST['loc_code'], $_POST['stock_id_'])) {
                add_to_order($_SESSION['WorkOrder'], $_POST['stock_id'], input_num('quantity'),
...
//-----------------------------------------------------------------------------------------------
if (isset($_GET['NewWorkOrder']) || !isset($_SESSION['WorkOrder']))
{
...
elseif (isset($_GET['WorkOrder']) || !isset($_SESSION['WorkOrder']))
{
    if (isset($_GET['fixed_asset']))
        check_db_has_disposable_fixed_assets(_("There are no fixed assets defined in the system."));
    else
...
start_form();
global $Ajax;
display_order_header($_SESSION['WorkOrder']);
...
display_adjustment_items(_("BOM"), $_SESSION['WorkOrder']);
else
{
