...
text_cells(_("Other Customers (For Sale):"), 'other_cust_',null, 30);
end_row();
end_table();
start_table(TABLESTYLE_NOBORDER);
start_row();
...
function get_call_type($row)
{
	...
	$result = db_query($sql, "could not get call type");
...
function get_sql_for_task_inquiry1($from_date,$to_date,$debtor_no,$other_cust,$call_type,$contact_no,$remarks)
{
	...
	$end_date = date2sql($to_date);
		...
		$sql = " SELECT ".TB_PREF."task_history.`id`,
				...
				".TB_PREF."task_history.`call_type`,
				".TB_PREF."task_history.`contact_no`,
				".TB_PREF."task_history.`other_cust`,
	...
	if ($other_cust!= '')
	{
		...
		$sql .= " AND ".TB_PREF."task_history.other_cust LIKE ".db_escape($number_like);
	}
	if ($call_type != '')
	{
		$sql .= " AND ".TB_PREF."task_history.call_type = ".db_escape($call_type);
	}
	if ($contact_no!= '')
	{
		$number_like = "%".$contact_no."%";
		$sql .= " AND ".TB_PREF."task_history.contact_no LIKE ".db_escape($number_like);
	}
	if ($remarks!= '')
	{
		$number_like = "%".$remarks."%";
...
$sql = get_sql_for_task_inquiry1($_POST['from_date'], $_POST['to_date'],
