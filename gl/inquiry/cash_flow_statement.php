...
function get_account_types_cf_category($all=false, $class_id=false, $parent=false)
{
	...
	if ($class_id != false)
		$where[] = "c_type=".db_escape($class_id);
...
//		$where[] ="(parent = '' OR parent = '-1')";
	...
	$sql .= " ORDER BY c_type, id";

	return db_query($sql, "could not get account types");
}

function get_gl_accounts_cashflow_gl($from=null, $to=null, $type=null)
{
	$sql = " SELECT gl.*,cm.*,cat.name AS AccountTypeName
		FROM "
		.TB_PREF."cashflow_gl gl,"
		.TB_PREF."cashflow_categories cat,
		".TB_PREF."chart_master cm
		
		WHERE gl.cashflow_category_id=cat.id
		AND gl.gl_account=cm.account_code
		";

	if ($from != null)
		$sql .= " AND gl.id >= ".db_escape($from);
	if ($to != null)
		$sql .= " AND gl.id <= ".db_escape($to);
	if ($type != null)
		$sql .= " AND cashflow_category_id=".db_escape($type);

	$sql .= " ORDER BY id";

	return db_query($sql, "could not get gl accounts");
}
function get_account_cashflow_type($all=false, $balance=-1)
{
...
function display_trial_balance($type, $typename)
{
	...
	$accounts = get_gl_accounts_cashflow_gl(null, null, $type);
	...
	{
		$myrow=get_cashflow_category2($account['cashflow_category_id']);
		get_cash_flow_category($account['cashflow_category_id']);
		...
		{
			$open = get_balance($account["gl_account"], $_POST['Dimension'], $_POST['Dimension2'], $begin,  $begin, false, true);
			$offset = min($open['debit'], $open['credit']);
		} else
			$offset = 0;
		$prev = get_balance($account["gl_account"], $_POST['Dimension'], $_POST['Dimension2'], $begin, $_POST['TransFromDate'], false, false);
		$curr = get_balance($account["gl_account"], $_POST['Dimension'], $_POST['Dimension2'], $_POST['TransFromDate'], $_POST['TransToDate'], true, true);
		$tot = get_balance($account["gl_account"], $_POST['Dimension'], $_POST['Dimension2'], $begin, $_POST['TransToDate'], false, true);
...
//		}
		...
		$cbal += $curr['balance'];
		$tbal += $tot['balance'];
		$total_credit += $tot['credit']-$offset;
		$total_debit += $tot['debit']-$offset;
		end_row();
	}
...
//		amount_cell($ccre-$Accre );
	$myrow=get_cashflow_category2($type);
	if($myrow['flowtype'] == 1 )
...
$tableheader =  "<tr>
	...
	<td colspan=1 class='tableheader'>" . _("Amount") . "</td>
	</tr><tr>
...
$classresult = get_account_cashflow_type(false);
while ($class = db_fetch($classresult))
{
	start_row("class='inquirybg' style='font-weight:bold'");
	label_cell(_("Class")." - ".$class['id'] ." - ".$class['name'], "colspan=8");
	end_row();
	//Get Account groups/types under this group/type with no parents
	$typeresult = get_account_types_cf_category(false, $class['id'], -1);
