...
{  
	...
	$date = today();
	$new_date=date2sql($date);
	$mo = date('m',strtotime($new_date));
    $yr = date('Y',strtotime($new_date));
	...
	if (!isset($_POST['TransFromDate']))
		$_POST['TransFromDate'] = add_days(end_month($date), /*-user_transaction_days()*/ -27);
		}
		elseif($mon_days==30)
		{
		    if (!isset($_POST['TransFromDate']))
		$_POST['TransFromDate'] = add_days(end_month($date), /*-user_transaction_days()*/ -29);
		}
		else
		{
		    if (!isset($_POST['TransFromDate']))
		$_POST['TransFromDate'] = add_days(end_month($date), /*-user_transaction_days()*/ -30);
		}
