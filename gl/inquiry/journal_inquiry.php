...
if (get_company_pref('use_dimension') && isset($_POST['dimension']))
...
//bisma
function view_link_counterparty($row)
{
   if($row['trans_type']==ST_SALESINVOICE)
       return get_trans_view_str('CustomerInquiry', $row["trans_no"], $row["name"], false, '', '');
   elseif($row['trans_type']==ST_SUPPINVOICE)
       return get_trans_view_str('SupplierInquiry', $row["trans_no"], $row["name"], false, '', '');
}
...
function prt_link($row)
{
    ...
    if ($row['trans_type'] == ST_CUSTPAYMENT )
		return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print Receipt"), true, ST_CUSTPAYMENT, ICON_PRINT);
    else
    	return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print"), true, $row['trans_type'], ICON_PRINT);
...
//    if ($row['trans_type'] == ST_CUSTPAYMENT)
	...
	if($db_connections[$_SESSION["wa_current_user"]->company]["name"]=='CM')
	{
	   if ($row['trans_type'] == ST_BANKPAYMENT || $row['trans_type'] == ST_BANKDEPOSIT || $row['trans_type'] == ST_BANKTRANSFER
			|| $row['trans_type'] == ST_CPV || $row['trans_type'] == ST_CRV || $row['trans_type'] == ST_SUPPAYMENT
		)
			return print_document_link($row['trans_no'], _("Print"), true, ST_CUSTPAYMENT_A5_CM, ICON_PRINT, null, null, null, null, $row['trans_type']); 
			else
		return '';	
	}
	elseif($db_connections[$_SESSION["wa_current_user"]->company]["name"]=='NKR')
	{
		if ($row['trans_type'] == ST_BANKPAYMENT || $row['trans_type'] == ST_BANKDEPOSIT || $row['trans_type'] == ST_BANKTRANSFER
			|| $row['trans_type'] == ST_CPV || $row['trans_type'] == ST_CRV || $row['trans_type'] == ST_SUPPAYMENT
		)
			return print_document_link($row['trans_no'], _("Print"), true, ST_CUSTPAYMENT_A5_, ICON_PRINT, null, null, null, null, $row['trans_type']);
		else
			return '';
...
$sql = get_sql_for_journal_inquiry(get_post('filterType', -1), get_post('FromDate'),
	...
	null, null, get_post('status'), get_post('account'), get_post('trans_no'), get_post('cheque_no'),$_POST['searching'],$_POST['search_value']);
db_query("set @bal:=0");
...
if($_SESSION["wa_current_user"]->can_access('SA_VOUCHERAPPROVAL') && get_company_pref('gl_approval')) {
	$cols = array(
		...
		_("Counterparty") => array('fun'=>'view_link_counterparty','ord' => ''),
...
window.onscroll = function() {myFunction()};
    ...
    var header=document.getElementById("custom_row");

    var col=document.getElementsByClassName("header");
    console.log(col);
