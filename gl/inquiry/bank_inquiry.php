...
$result = get_bank_trans_for_bank_account($_POST['bank_account'], $_POST['TransAfterDate'], $_POST['TransToDate'], $_POST['cheque_no']);	
...
$th = array(_("Type"), _("#"), _("Reference"), _("Cheque No."), _("Date"),
	_("Debit"), _("Credit"), _("Balance"), _("Person/Item"), _("Memo"), _("Funds Transfer"), _("Bank Vouchers"), _("Cash Vouchers"), "", "", "");
table_header($th);
...
{
    if ($row["type"] == ST_BANKPAYMENT || $row["type"] == ST_BANKTRANSFER ||
        $row["type"] == ST_CUSTPAYMENT) {
        echo "<td>";
        echo pager_link(_("Funds Transfer"),
...
function dispatch_link2($row)
{
    ...
    {
        echo "<td>";
        echo   pager_link( _("Bank Voucher"),
            "/gl/gl_bank.php?NewDeposit=Yes&&"."trans_no1=".$row['trans_no']."&&trans_type=".$row['type'], ICON_DOC);
        echo "</td>";
    }
    elseif($row["type"] == ST_CUSTPAYMENT || $row["type"] == ST_BANKTRANSFER)
    {
        echo "<td>";
        echo  pager_link( _("Bank Voucher"),
...
function dispatch_link3($row)
{
    if ($row["type"] == ST_BANKPAYMENT)
    {
        echo "<td>";
        echo   pager_link( _("Cash Voucher"),
            "/gl/gl_bankCV.php?NewDeposit=Yes&&"."trans_no1=".$row['trans_no']."&&trans_type=".$row['type'], ICON_DOC);
        echo "</td>";
    }
    else
    {
...
label_cell(get_trans_view_str($myrow["type"],$myrow["trans_no"],$myrow['ref']));
    ...
    dispatch_link2($myrow);// for showing bank vouchers
    dispatch_link3($myrow);// for showing cash vouchers
