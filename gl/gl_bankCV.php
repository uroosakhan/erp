...
@($_SESSION['pay_items']->trans_type==ST_CPV)
    ? 'SA_PAYMENT' : 'SA_DEPOSIT';

include_once($path_to_root . "/includes/date_functions.inc");
include_once($path_to_root . "/includes/data_checks.inc");

include_once($path_to_root . "/gl/includes/ui/gl_bankCV_ui.inc");
...
if (isset($_GET['NewPayment'])) {
	$_SESSION['page_title'] = _($help_context = "Cash Payment Voucher Entry");
	create_cart(ST_CPV, 0);
} else if(isset($_GET['NewDeposit'])) {
	$_SESSION['page_title'] = _($help_context = "Cash Receipt Voucher Entry");
	create_cart(ST_CRV, 0,null,$_GET['trans_no1'], $_GET['trans_type']);
} else if(isset($_GET['ModifyPayment'])) {
	$_SESSION['page_title'] = _($help_context = "Modify Cash Payment Voucher Entry")." #".$_GET['trans_no'];
	create_cart(ST_CPV, $_GET['trans_no'], $_GET['approval']);
} else if(isset($_GET['ModifyDeposit'])) {
	$_SESSION['page_title'] = _($help_context = "Modify Cash Receipt Voucher Entry")." #".$_GET['trans_no'];
	create_cart(ST_CRV, $_GET['trans_no'], $_GET['approval']);
}
...
if (isset($_GET['AddedDep']))
{
	...
	global $path_to_root, $pdf_debug,$db_connections,$def_print_orientation;
...
echo'<a id="thickboxId" href="../reporting/prn_redirect.php?PARAM_0='.$trans_no.'&PARAM_1='.$doc_no.'&PARAM_2=&PARAM_3=0&PARAM_4='.$def_orientation.'&REP_ID=107774" 
target="_blank"></a>';


echo'<a id="thickboxform" href="../gl/gl_bankCV.php?NewDeposit=Yes" 
target="_blank"></a>';
...
if (isset($_POST['Process']) && !check_trans())
{
	...
	$trans = write_bank_transaction(
		...
		$_POST['ref'], $_POST['memo_'], true, input_num('settled_amount', null),0,0,
		$_POST['text_1'], $_POST['text_2'], $_POST['text_3'],0, $_SESSION['ParentID'], $_SESSION['ParentType']);
...
//			}
	...
	if ($new)
		meta_forward($_SERVER['PHP_SELF'], $trans_type==ST_CPV ?
...
{
	$amount = ($_SESSION['pay_items']->trans_type==ST_CPV ? 1:-1) * input_num('amount');
...
if (isset($_POST['go']))
{
	display_quick_entries($_SESSION['pay_items'], $_POST['person_id'], input_num('totamount'), 
		$_SESSION['pay_items']->trans_type==ST_CPV ? QE_PAYMENT : QE_DEPOSIT);
...
start_row();
echo "<td>";
display_gl_items($_SESSION['pay_items']->trans_type==ST_CPV ?
...
submit_center_last('Process', $_SESSION['pay_items']->trans_type==ST_CPV ?
