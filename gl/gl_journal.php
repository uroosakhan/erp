...
if (isset($_GET['AddedID'])) 
{
	...
	echo "</br>";
     submenu_print(_("&Print This Journal Entry"), $trans_type1, $trans_no, 'prtopt');

echo "</br>";
    submenu_print(_("&Print This Payment"), 0, $trans_no, 'prtopt');
...
elseif (isset($_GET['ModifyGL']))
{
	...
	create_cart($_GET['trans_type'], $_GET['trans_no'], $_GET['approval']);
}

function create_cart($type=0, $trans_no=0, $approval=0)
{
	global $Refs;
	...
	if ($trans_no) {
		...
		while ($detail = db_fetch($taxes))
		{
			...
			//$tax_info['tax_group'] = $detail['tax_group_id']; ansar 26-08-2017
...
if (isset($_POST['Process']))
{
	...
	if (check_value('taxable_trans'))
	{
		...
		//$cart->tax_info['tax_group'] = $_POST['tax_group']; ansar-08-17
		$taxes = get_all_tax_types();
		while ($tax = db_fetch($taxes))
		{
			$tax_id = $tax['id'];
