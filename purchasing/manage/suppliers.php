...
function supplier_settings(&$supplier_id)
{
 ...
 if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'TRISTAR') 
 {
    
	text_row(_("Supplier Name:"), 'supp_name', null, 42, 40);
	text_row(_("Phone Number:"), 'supp_ref', null, 30, 30);
	
		hidden('curr_code','PKR');
		hidden('tax_group_id', 1);

        payment_terms_list_row(_("Payment Terms:"), 'payment_terms', null);

		hidden('payable_account', '201001');
		hidden('purchase_account', '');
		hidden('payment_discount_account', '501006');
...
$supp_category = get_company_pref('supp_category');
	if($supp_category == 1 ) {/////only for supplier category
               supp_category_list_row(_("Supplier Category:"), 'combo1', null, false);

    }


	text_row(_("GSTNo:"), 'gst_no', null, 42, 40);
	text_row(_("NTN No:"), 'ntn_no', null, 42, 40);
	text_row(_("SST No:"), 'service_text', null, 42, 40);
    ...
    {
        combo1_po_list_cells(_("Term's Of Delivery :"), 'combo1', $_POST['combo1'], true, false, false);
//        text_row(_("Chasis No:"), 'h_text1', null, 16, 15);
//        text_row(_("Engin No:"), 'h_text2', null, 16, 15);
    }
	//
	...
	if ($input_error !=1 )
	{
		...
		if ($supplier_id) 
		{
			update_supplier($_POST['supplier_id'], $_POST['supp_name'], $_POST['supp_ref'], $_POST['address'],
				...
				$_POST['notes'], $_POST['tax_group_id'], get_post('tax_included', 0), $_POST['service_text']
                , $_POST['combo1']);
		...
		else 
		{
			add_supplier($_POST['supp_name'], $_POST['supp_ref'], $_POST['address'], $_POST['supp_address'],
				...
				$_POST['payment_discount_account'], $_POST['notes'], $_POST['tax_group_id'], check_value('tax_included'), $_POST['service_text']
                , $_POST['combo1']);
