...
//if($_GET['type'] == 10)
    $result = get_history_purch($_GET['stock_id'], $supplier_id);
//elseif($_GET['type'] == 30)
...
//    $GetSaleOrder = get_loc_code_from_sales_order($myrow['order_']);
    ...
    amount_cell(abs($myrow["quantity_ordered"]),null,$dec);
    label_cell($myrow["units_id"]);
    amount_cell($myrow["unit_price"],null,$dec);
    label_cell(get_location_name($myrow["into_stock_location"]));
