...
function alloc_link($row)
{
	...
	if ($row["type"] == ST_BANKPAYMENT || $row["type"] == ST_SUPPAYMENT ||
		(($row["type"] == ST_SUPPCREDIT || $row["type"] == ST_SUPPCREDIT_IMPORT || $row["type"] == ST_JOURNAL) && $row["TotalAmount"] < 0))
	...
	if ($row["type"] == ST_SUPPINVOICE || (($row["type"] == ST_SUPPCREDIT || $row["type"] == ST_SUPPCREDIT_IMPORT || $row["type"] == ST_JOURNAL) && $row["TotalAmount"] > 0))
		return floatcmp($row["TotalAmount"], $row["Allocated"]) ? $link : '';
