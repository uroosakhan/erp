...
while ($myrow = db_fetch($loc_details))
{
    ...
    $demand_qty1 = get_on_worder_qty($_POST['stock_id'], $myrow["loc_code"]);
    //  Urgent Jugaaaaaaaarh, maaf kerna haha:D
    $demand_qty1 -= get_on_issued_qty($_POST['stock_id'], $myrow["loc_code"]);

    //    $demand_qty_dummy = get_on_worder_qty($_POST['stock_id'], $myrow["loc_code"]);
//    $demand_qty = $demand_qty - $demand_qty_dummy;
...
//    $qoh += get_on_worder_qty($_POST['stock_id'], $myrow["loc_code"]);
...
//        $qoo += get_on_worder_qty($_POST['stock_id'], $myrow["loc_code"]);
        ...
        if($pref['batch']  == 1) {
            ...
            if($demand_qty1 < 0)
                $demand_qty1 = 0;
            qty_cell($demand_qty1, false, $dec);
            if ($pref['alt_uom'] == 1)
