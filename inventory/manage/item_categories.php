...
function get_assembly_costs_account($invent_acc)
{
    $sql = "SELECT (account_name)   FROM ".TB_PREF."chart_master WHERE account_code=".db_escape($invent_acc);
...
function get_inventory_accounts($invent_acc)
{
    $sql = "SELECT (account_name)   FROM ".TB_PREF."chart_master WHERE account_code=".db_escape($invent_acc);
...
function get_cogs_accounts($cogs_acc)
{
    $sql = "SELECT account_name FROM ".TB_PREF."chart_master WHERE account_code=".db_escape($cogs_acc);
...
function get_adjustment_accounts($adjust_acc)
{
    $sql = "SELECT account_name FROM ".TB_PREF."chart_master WHERE account_code=".db_escape($adjust_acc);
...
function get_sales_accounts($sales_acc)
{
    $sql = "SELECT account_name FROM ".TB_PREF."chart_master WHERE
	 account_code=".db_escape($sales_acc);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
    ...
    if (strlen($_POST['description']) == 0 || strlen($_POST['cat_code']) ==0
    )
    {
        $input_error = 1;
    ...
    $code = get_category_code($_POST['cat_code']);


    if($Mode!='UPDATE_ITEM')
        if ($_POST['cat_code'] == $code )
        {
            $input_error =0;
            display_error(_("The Item Category Code Already Exists."));
            set_focus('cat_code');
        }
    ...
    if ($input_error !=1)
    {
        if ($selected_id != -1)
        {
            update_item_category($selected_id,$_POST['cat_code'], $_POST['description'],
                ...
                check_value('no_sale'), check_value('no_purchase'),	$_POST['short_name']);
        ...
        else
        {
            add_item_category($_POST['description'],
                ...
                $_POST['cat_code'], $_POST['short_name']);
                
             if($_POST['mb_flag'] == 'F')
            display_notification(_('New Fixed Asset category has been added'));
            else
...
if ($fixed_asset) {
    $th = array(_("Cat Code"),_("Name"), _("Tax type"), _("Units"), _("Sales Act"),
        _("Asset Account"), _("Deprecation Cost Account"),
        _("Depreciation/Disposal Account"), "", "");
} else {
    $th = array(_("Stock ID"),_("Name"), _("Tax type"), _("Units"), _("Type"), _("Sales Act"),
...
while ($myrow = db_fetch($result))
{
    ...
    label_cell($myrow["cat_code"]);
    label_cell($myrow["description"]);
    label_cell($myrow["tax_name"]);
    label_cell($myrow["dflt_units"], "align=center");
    if (!$fixed_asset)
        label_cell($stock_types[$myrow["dflt_mb_flag"]]);
    label_cell($myrow["dflt_sales_act"]."- ".get_sales_accounts($myrow["dflt_sales_act"]), "align=center");

    label_cell(($myrow["dflt_inventory_act"])."- ".get_inventory_accounts($myrow["dflt_inventory_act"]), "align=center");

    label_cell($myrow["dflt_cogs_act"]."- ".get_cogs_accounts($myrow["dflt_cogs_act"]), "align=center");

    label_cell($myrow["dflt_adjustment_act"]."- ".get_adjustment_accounts($myrow["dflt_adjustment_act"]), "align=center");

    if (!$fixed_asset)
        label_cell($myrow["dflt_wip_act"]."-    ".get_assembly_costs_account($myrow["dflt_wip_act"]), "align=center");
...
if ($selected_id != -1)
{
    if ($Mode == 'Edit') {
        ...
        $_POST['short_name']  = $myrow["short_name"];

    }
    hidden('selected_id', $selected_id);
    hidden('category_id');
} else if ($Mode != 'CLONE' && $Mode != '') {
    ...
    if (get_post('wip_account') == "")
        ...
        $supp_item = get_company_pref('supp_item');
         $cust_item = get_company_pref('cust_item');

if ($selected_id != -1) {
    
     if($cust_item==1){
    echo '<a href="' . $path_to_root . '/inventory/customer_item.php?stock_id=' . $_POST['category_id'] . '" class="btn btn-info role="button" target="_blank"> Link category with Customers</a>';
     }
    
    if($supp_item==1){
         echo '&nbsp;&nbsp;&nbsp;&nbsp;<a href="' . $path_to_root . '/inventory/manage/suppliers_items.php?stock_id=' . $_POST['category_id'] . '" class="btn btn-info role="button" target="_blank">Link category with Suppliers </a>';
    }
...
text_row(_("Category Code:"), 'cat_code', null, 30, 30);
text_row(_("Category Name:"), 'description', null, 30, 50);
...
//for adding supplier id in database for ALI Electronics
// global $db_connections;
// if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'ALI')
// {
    supplier_list_row("Supplier", 'short_name', null);
// }
if($_POST['mb_flag']!='F')
