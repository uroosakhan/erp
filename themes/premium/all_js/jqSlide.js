...
var jqSlide = {
  ...
  **/
  constructSlider: function(element, headerElements, contentElements) {
    ...
    nodeString += '<div class="select-slider "><select class="form-control" id="select-slider" name="navigation" onchange="jqSlide.slideSelect(this);">';
    nodeString += '<option disabled="disabled" selected="selected" style="background-color:#fff;" >Menu</option>';
    $.each(headerElements, function(i, value) {
      nodeString += '<option style="background-color:#fff;color:#666;"  value="slider-' + (i + 1) + '">' + $(headerElements[i]).html() + '</option>';
    });
