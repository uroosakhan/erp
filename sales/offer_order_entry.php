...
include_once($path_to_root . "/sales/includes/offer_cart_class.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/sales/includes/sales_ui.inc");
include_once($path_to_root . "/sales/includes/ui/offer_order_ui.inc");
include_once($path_to_root . "/sales/includes/sales_db.inc");
include_once($path_to_root . "/sales/includes/db/offer_order_db.inc");
include_once($path_to_root . "/sales/includes/db/sales_types_db.inc");
include_once($path_to_root . "/reporting/includes/reporting.inc");

set_page_security( @$_SESSION['Offer']->trans_type,
...
if (isset($_GET['AddOffers'])) {
	...
	$_SESSION['page_title'] = _($help_context = "New Offer Entry");
...
if (isset($_GET['AddedID'])) {
	...
	check_edit_conflicts(get_post('cart_id'), 'Offer');
//-----------------------------------------------------------------------------
...
function copy_to_cart()
{
	...
	$cart->offer_title = $_POST['offer_title'];
	$cart->offer_note = $_POST['offer_note'];
	$cart->OfferDate = $_POST['OfferDate'];
	$cart->enable = $_POST['enable'];
...
function copy_from_cart()
{
	...
	$_POST['offer_title'] = $cart->offer_title;
	$_POST['offer_note'] = $cart->offer_note;
	$_POST['OfferDate'] = $cart->OfferDate;
	$_POST['enable'] = $cart->enable;
}
//--------------------------------------------------------------------------------
...
//		return false;
//	}
//	if ($_SESSION['Offer']->trans_type!=ST_SALESORDER && $_SESSION['Offer']->trans_type!=ST_SALESQUOTE && !is_date_in_fiscalyear($_POST['OrderDate'])) {
//		display_error(_("The entered date is out of fiscal year or is closed for further data entry."));
//		set_focus('OrderDate');
//		return false;
//	}
//	if (count($_SESSION['Offer']->line_items) == 0)	{
//		display_error(_("You must enter at least one non empty item line."));
//		set_focus('AddItem');
//		return false;
//	}
//	if (!$SysPrefs->allow_negative_stock() && ($low_stock = $_SESSION['Offer']->check_qoh()))
//	{
//		display_error(_("This document cannot be processed because there is insufficient quantity for items marked."));
//		return false;
//	}
//	if ($_SESSION['Offer']->payment_terms['cash_sale'] == 0) {
//		if (!$_SESSION['Offer']->is_started() && ($_SESSION['Offer']->payment_terms['days_before_due'] == -1) && ((input_num('prep_amount')<=0) ||
//			input_num('prep_amount')>$_SESSION['Offer']->get_trans_total())) {
...
//		}
//
//		if ($_SESSION['Offer']->trans_type != ST_SALESQUOTE && strlen($_POST['delivery_address']) <= 1) {
...
//			if ($_SESSION['Offer']->trans_type==ST_SALESQUOTE)
...
//		}
//	}
	if (!$Refs->is_valid($_POST['ref'], ST_OFFERCODE)) {
		display_error(_("You must enter a reference."));
		set_focus('ref');
		return false;
	}
//	if (!db_has_currency_rates($_SESSION['Offer']->customer_currency, $_POST['OrderDate']))
//		return false;
//
//   	if ($_SESSION['Offer']->get_items_total() < 0) {
...
if (isset($_POST['ProcessOrder']) && can_process()) {
	...
	$modified = ($_SESSION['Offer']->trans_no != 0);
	$so_type = $_SESSION['Offer']->so_type;
	$ret = $_SESSION['Offer']->write(1);
	if ($ret == -1)
	{
		display_error(_("The entered reference is already in use."));
		$ref = $Refs->get_next(ST_OFFERCODE, null, array('date' => Today()));
		if ($ref != $_SESSION['Offer']->reference)
		{
			display_error(_("The reference number field has been increased. Please save the document again."));
			$_POST['ref'] = $_SESSION['Offer']->reference = $ref;
			$Ajax->activate('ref');
		}	
		set_focus('ref');
	}
	else
	{
		...
		$trans_no = key($_SESSION['Offer']->trans_no);
		$trans_type = $_SESSION['Offer']->trans_type;
		new_doc_date($_SESSION['Offer']->document_date);
		processing_end();
		if ($modified) {
...
//		} elseif ($trans_type == ST_SALESINVOICE) {
			meta_forward($path_to_root.'/sales/inquiry/offer_inquiry.php', "Updated=1");
		} else {
            meta_forward($path_to_root.'/sales/inquiry/offer_inquiry.php', "AddedID=1");
...
function check_item_data()
{
	...
	elseif (!check_num('qty', 0) || !check_num('Disc', 0, 100)) {
		...
		set_focus('price');
		return false;
	} elseif (isset($_POST['LineNo']) && isset($_SESSION['Offer']->line_items[$_POST['LineNo']])
	    && !check_num('qty', $_SESSION['Offer']->line_items[$_POST['LineNo']]->qty_done)) {
	...
	$cost = $cost_home / get_exchange_rate_from_home_currency($_SESSION['Offer']->customer_currency, $_SESSION['Offer']->document_date);
	if (input_num('price') < $cost)
	{
		$dec = user_price_dec();
		$curr = $_SESSION['Offer']->customer_currency;
...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Offer']->update_cart_item($_POST['LineNo'],
...
{
    if ($_SESSION['Offer']->some_already_delivered($line_no) == 0) {
	    $_SESSION['Offer']->remove_from_cart($line_no);
    } else {
...
//    while($fetch = db_fetch($query))
    {
        add_to_order($_SESSION['Offer'], get_post('stock_id'), get_post('item_brand'),
            get_post('in'), input_num('values'), $_POST['date_from'], $_POST['date_to'],
            get_post('offer_status'), get_post('status'), get_post('offer_on'), get_post('offer_calc_level'),
            $_POST['ref'], $_POST['offer_title'], $_POST['offer_note'], $_POST['OfferDate'], $_POST['enable']);
//        add_to_order($_SESSION['Offer'], get_post('stock_id'), get_post('item_brand'),
//            get_post('in'), input_num('values'), $_POST['date_from'], $_POST['date_to'],
//            get_post('offer_status'), get_post('status'), get_post('offer_on'), get_post('offer_calc_level'),
//            $_POST['ref'], $_POST['offer_title'], $_POST['offer_note'], $_POST['OfferDate'], $_POST['enable']);
...
function  handle_cancel_order()
{
	...
	if ($_SESSION['Offer']->trans_type == ST_CUSTDELIVERY) {
	...
	} elseif ($_SESSION['Offer']->trans_type == ST_SALESINVOICE) {
	...
	} elseif ($_SESSION['Offer']->trans_type == ST_SALESQUOTE)
	{
		if ($_SESSION['Offer']->trans_no != 0) 
			delete_sales_order(key($_SESSION['Offer']->trans_no), $_SESSION['Offer']->trans_type);
		display_notification(_("This sales quotation has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Quotation"), "/sales/sales_order_entry.php?NewQuotation=Yes");
	} else { // sales order
		if ($_SESSION['Offer']->trans_no != 0) {
			$order_no = key($_SESSION['Offer']->trans_no);
...
{ 
	...
	if (isset($_GET['NewQuoteToSalesOrder']))
	{
		...
		$doc = new offer_Cart(ST_SALESQUOTE, $trans_no, true);
		$doc->Comments = _("Sales Quotation") . " # " . $trans_no;
		$_SESSION['Offer'] = $doc;
	}	
	elseif($type != ST_SALESORDER && $type != ST_SALESQUOTE && $trans_no != 0) { // this is template

		$doc = new offer_Cart(ST_SALESORDER, array($trans_no));
		$doc->trans_type = $type;
		$doc->trans_no = 0;
		...
		$_SESSION['Offer'] = new offer_Cart($type, array($trans_no));
	copy_from_cart();
}
...
hidden('cart_id');
$customer_error = display_order_header($_SESSION['Offer'], !$_SESSION['Offer']->is_started(), $idate);

if ($customer_error == "") {
//	start_table(TABLESTYLE, "width='80%'", 10);
	echo "<tr><td>";
	display_order_summary($orderitems, $_SESSION['Offer'], true);
	echo "</td></tr>";
//	echo "<tr><td>";
//	display_delivery_details($_SESSION['Offer']);
//	echo "</td></tr>";
	end_table(1);
	...
	if ($_SESSION['Offer']->trans_no == 0) {
		...
		submit_center_last('CancelOrder', $cancelorder,
	   		_('Cancels document entry or removes sales order when editing an old document'));
		if ($_SESSION['Offer']->trans_type==ST_SALESORDER)
