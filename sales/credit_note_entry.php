...
function check_item_data()
{
    ...
    $cost_home = get_unit_cost_credit(get_post('stock_id'));
    if($cost_home['mb_flag'] != 'D')
    {
       if (!$cost_home['material_cost']) {
        display_warning("Standard cost is 0 for ".$cost_home['stock_id']." - ".$cost_home['description']);
    } 
    }
...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != "" && check_item_data()) {
		$_SESSION['Items']->update_cart_item($_POST['line_no'], input_num('qty'),
			input_num('price'), input_num('Disc') / 100, get_post('stock_id_text'),$_POST['units_id'],0,
			...
			$_POST['combo1'], $_POST['combo2'], $_POST['combo3'],$_POST['combo4'], $_POST['combo5'], $_POST['combo6'], $_POST['batch']);
	}
    line_start_focus();
}
