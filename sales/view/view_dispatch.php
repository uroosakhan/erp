...
//Text Boxes Headings
    ...
    if ($pref['item_location'] == 1) {
		if ($myrow233['sale_enable'] == 1) {
			...
			if ($pref['batch'] == 1) {
				if ($pref['alt_uom'] == 1) {
					{
                        array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"),
							$myrow_factor, _("Price"), _("Discount %"), _("Total")));
					}
				} else {
                    array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"),
				...
				if ($pref['alt_uom'] == 1) {
					{
                        array_append($th, array(_("Quantity"), _("Units"), _("Location"), $myrow_factor, _("Price"), _("Discount %"), _("Total")));
					}
				} else {
                    array_append($th, array(_("Quantity"), _("Units"), _("Location"), _("Price"), _("Discount %"), _("Total")));
				}
			}
		} else {
			if ($pref['batch'] == 1) {
				if ($pref['alt_uom'] == 1) {
					{
                        array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"), _("Price"), _("Discount %"), _("Total")));
					}
				} else {
                    array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"), _("Price"), _("Discount %"), _("Total")));
				}
	...
	if ($myrow233['sale_enable'] == 1) {
		...
		if ($pref['batch'] == 1) {
			if ($pref['alt_uom'] == 1) {
				{
					array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"),
						$myrow_factor, _("Price"), _("Discount %"), _("Total")));
				}
			} else {
				array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"),
			...
			if ($pref['alt_uom'] == 1) {
				{
					array_append($th, array(_("Quantity"), _("Units"), $myrow_factor, _("Price"), _("Discount %"), _("Total")));
				}
			} else {
				array_append($th, array(_("Quantity"), _("Units"), _("Price"), _("Discount %"), _("Total")));
			}
		}
		} else {
		if ($pref['batch'] == 1) {
			if ($pref['alt_uom'] == 1) {
				{
					array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"), _("Price"), _("Discount %"), _("Total")));
				}
			} else {
				array_append($th, array(_("Quantity"), _("Batch"), _("Exp.Date"), _("Units"), _("Price"), _("Discount %"), _("Total")));
			}
    ...
    if ($pref['item_location'] == 1) {
        if ($myrow233['sale_enable'] == 1) {
            ...
            if ($pref['batch'] == 1) {
                if ($pref['alt_uom'] == 1) {
                    {
                        array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"),
                            $myrow_factor, _("Price"), _("Discount %"), _("Total")));
                    }
                } else {
                    array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"),
                ...
                if ($pref['alt_uom'] == 1) {
                    {
                        array_append($th, array(_("Quantity"),_("Bonus"), _("Units"), _("Location"), $myrow_factor, _("Price"), _("Discount %"), _("Total")));
                    }
                } else {
                    array_append($th, array(_("Quantity"),_("Bonus"), _("Units"), _("Location"), _("Price"), _("Discount %"), _("Total")));
                }
            }
        } else {
            if ($pref['batch'] == 1) {
                if ($pref['alt_uom'] == 1) {
                    {
                        array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"), _("Price"), _("Discount %"), _("Total")));
                    }
                } else {
                    array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"), _("Location"), _("Price"), _("Discount %"), _("Total")));
                }
        ...
        if ($myrow233['sale_enable'] == 1) {
            ...
            if ($pref['batch'] == 1) {
                if ($pref['alt_uom'] == 1) {
                    {
                        array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"),
                            $myrow_factor, _("Price"), _("Discount %"), _("Total")));
                    }
                } else {
                    array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"),
                ...
                if ($pref['alt_uom'] == 1) {
                    {
                        array_append($th, array(_("Quantity"),_("Bonus"), _("Units"), $myrow_factor, _("Price"), _("Discount %"), _("Total")));
                    }
                } else {
                    array_append($th, array(_("Quantity"),_("Bonus"), _("Units"), _("Price"), _("Discount %"), _("Total")));
                }
            }
        } else {
            if ($pref['batch'] == 1) {
                if ($pref['alt_uom'] == 1) {
                    {
                        array_append($th, array(_("Quantity"),_("Bonus"), _("Batch"), _("Exp.Date"), _("Units"), _("Price"), _("Discount %"), _("Total")));
                    }
                } else {
                    array_append($th, array(_("Quantity"), _("Bonus"),_("Batch"), _("Exp.Date"), _("Units"), _("Price"), _("Discount %"), _("Total")));
                }
	...
	while ($myrow2 = db_fetch($result))
	{
		...
		if($pref['batch'] == 1) {
			...
			label_cell($myrow2["batch"], "align=right");
	...
	label_row(_("Discount1 "), price_format($myrow["discount1"]), "align=right colspan=20",
		"nowrap align=right", 1);
	label_row(_("Discount2 "), price_format($myrow["discount2"]), "align=right colspan=20",
		"nowrap align=right", 1);

	label_row(_("Sub-total"), $display_sub_tot, "colspan=20 align=right",
...
display_customer_trans_tax_details($tax_items, 620);

$display_total = price_format($myrow["ov_freight"]+$myrow["ov_amount"]+$myrow["ov_freight_tax"]+$myrow["ov_gst"]-$myrow["discount1"]-$myrow["discount2"]);

label_row(_("TOTAL VALUE"), $display_total, "colspan=20 align=right",
	"nowrap align=right");
}
end_table(1);
