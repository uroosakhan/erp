...
function get_dim1($row)
{
    $sql = "SELECT reference FROM ".TB_PREF."dimensions 
    WHERE id=".db_escape($row['dimension_id']);
...
function get_dim2($row)
{
    $sql = "SELECT reference FROM ".TB_PREF."dimensions 
    WHERE id=".db_escape($row['dimension2_id']);
    $result = db_query($sql, "could not get task type");
...
function get_cust_ref($row)
{
	$sql = "SELECT customer_ref
	    FROM ".TB_PREF."sales_orders so
		LEFT JOIN ".TB_PREF."debtor_trans dt ON dt.order_ = so.order_no
		WHERE dt.order_ = ".db_escape($row['order_']);
...
if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'BNW') {
	$cols = array(
		...
		_("Reference")=> array('fun' => 'get_cust_ref'),
