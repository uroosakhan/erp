...
if ($trans_type == ST_SALESORDER)
{
	if (isset($_GET['OutstandingOnly']) && ($_GET['OutstandingOnly'] == true))
	{
		...
		$_SESSION['page_title'] = _($help_context = "Search Outstanding Sales Orders - itemised");
	}
...
{
	global $Ajax;
	display_error("Asdasdasdasdasd");
	$Ajax->activate('orders_tbls');
}
...
//check_cells(_("show Quantity item"), 'Quantity_items',null,true);
check_cells( _("Show closed:"), 'Quantity_items', null,TRUE);
//yesno_list_cells("Show Closed", 'hello', null, "Yes", "No", true);
if ($trans_type == ST_SALESQUOTE)
...
//
hyperlink_params("$path_to_root/sales/inquiry/sales_orders_view.php?OutstandingOnly=1", _("Sales Orders Inquiry"));

$sql = get_sql_for_sales_orders_view_itemised($trans_type, get_post('OrderNumber'), get_post('order_view_mode'),
...
//display_error($_POST['Quantity_items']);
if ($trans_type == ST_SALESORDER  || $_POST['Quantity_items'] == 1) {
	$cols = array(
...
if (get_post('hello'))
{
	$cols[_("Quantity")] = 'skip';
	$cols[_("Name")] = 'skip';
}
$table =& new_db_pager('orders_tbls', $sql, $cols);
