...
include_once($path_to_root . "/sales/includes/pos_cart_class.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/sales/includes/sales_ui.inc");
include_once($path_to_root . "/sales/includes/ui/pos_order_ui.inc");
...
//            "/sales/customer_delivery.php?OrderNumber=$order_no");
	...
	submenu_option(_("Enter a &New Order"),	"/sales/pos_order_entry.php?NewOrder=0");
...
echo'<a id="thickboxform" href="../sales/pos_order_entry.php?NewInvoice=0" 
target="_self">f</a>';
...
// 	}
	...
	if ((isset($_GET['Type']) && $_GET['Type'] == 1))
		...
		submenu_option(_("Enter a &New Direct Invoice"),
			"/sales/pos_order_entry.php?NewInvoice=0");
...
function check_item_data()
{
	...
	$is_inventory_item = is_inventory_item(get_post('barcode'));
	if(!get_post('barcode_text', true)) {
		display_error( _("Item description cannot be empty."));
		set_focus('barcode_edit');
		return false;
	}
	...
	$cost_home = get_unit_cost(get_post('barcode')); // Added 2011-03-27 Joe Hunt
...
function handle_update_item()
{
	 ...
	 if($formula != '')
   {
      ...
      $b= ($_POST[$amount3]."".$value[1]."".$_POST[$amount2]);
		...
		$_POST['combo1'], $_POST['combo2'], $_POST['combo3'],$_POST['combo4'], $_POST['combo5'], $_POST['combo6'], $_POST['batch'], $_POST['item_location']);
	}
	page_modified();
  line_start_focus();
}
...
function handle_new_item()
{
   ...
   $item_info = get_item_edit_info($_POST['barcode']);
...
//       $_POST['qty'] = input_num('amount2') - input_num('amount3');
     ...
     if(get_post('barcode_pos') != "" && get_post('barcode') == "") {
         $_POST['barcode'] = get_post('barcode_pos');
     }
     elseif(get_post('barcode_pos') != "" && get_post('barcode') != "") {
         $_POST['barcode'] = get_post('barcode');
     }
     else {
         $_POST['barcode'] = get_post('barcode');
     }
		  ...
		  add_to_order($_SESSION['Items'], $_POST['barcode'], input_num('qty'),
			 input_num('price'), input_num('Disc') / 100, get_post('barcode_text'),
			 ...
			 $_POST['combo5'], $_POST['combo6'], $_POST['batch'],$_POST['item_location']);

   unset($_POST['_barcode_edit'], $_POST['barcode']);
   page_modified();
   line_start_focus();
...
{ 
	...
	if (isset($_GET['NewQuoteToSalesOrder']))
	{
		...
		$doc = new cart_pos(ST_SALESQUOTE, $trans_no, true);
	...
	elseif($type != ST_SALESORDER && $type != ST_SALESQUOTE && $trans_no != 0) { // this is template
		...
		$doc = new cart_pos(ST_SALESORDER, array($trans_no));
		$doc->trans_type = $type;
		$doc->trans_no = 0;
		...
		$_SESSION['Items'] = new cart_pos($type, array($trans_no));
	copy_from_cart();
}
...
if ($customer_error == "") {
    ...
    echo '<div style="float: left; margin-left: 0px; width: 900px;">';
	echo "<tr><td>";
