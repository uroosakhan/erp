...
page(_($help_context = "footer Combo 3"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			update_f_combo3($selected_id, $_POST['description']);
			$note = _('Selected combo has been updated');
    	} 
    	else 
    	{
			add_f_combo3($_POST['description']);
...
$result = get_f_combo3(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_f_combo_code3($selected_id);
