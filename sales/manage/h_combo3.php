...
page(_($help_context = "header Combo 3"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			update_h_combo3($selected_id, $_POST['description']);
			$note = _('Selected combo has been updated');
    	} 
    	else 
    	{
			add_h_combo3($_POST['description']);
...
$result = get_h_combo3(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_comboh_code3($selected_id);
