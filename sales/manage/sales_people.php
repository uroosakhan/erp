	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			...
			update_salesman($selected_id, $_POST['salesman_name'], $_POST['salesman_phone'], $_POST['salesman_fax'],
				$_POST['salesman_email'], $_POST['gl_account'], $_POST['user_id'], input_num('provision'), input_num('break_pt'), input_num('provision2'));
    	}
    	else
    	{
    		/*Selected group is null cos no item selected on first time round so must be adding a record must be submitting new entries in the new Sales-person form */
			add_salesman($_POST['salesman_name'], $_POST['salesman_phone'], $_POST['salesman_fax'], $_POST['salesman_email'],
			    $_POST['gl_account'], $_POST['user_id'], input_num('provision'), input_num('break_pt'), input_num('provision2'));
    	}
...
function get_gl_account_salesman($account_code)
{
...
function get_user_salesman($id)
{
    $sql = "SELECT user_id FROM ".TB_PREF."users WHERE id=".db_escape($id);
...
if($db_connections[$_SESSION["wa_current_user"]->company]["name"] != 'ITMEDVISION' )
{
$th = array(_("Name"), _("Phone"), _("Fax"), _("Email"), _("GL Account"), _("User"), _("Provision"), _("Break Pt."), _("Provision")." 2", "", "");
}
else
$th = array(_("Name"), _("Phone"), _("Fax"), _("Email"), _("User"), _("Provision"), _("Break Pt."), _("Provision")." 2", "", "");
...
if($db_connections[$_SESSION["wa_current_user"]->company]["name"] != 'ITMEDVISION' )
{
	label_cell(get_gl_account_salesman($myrow["gl_account"]));
}
	label_cell(get_user_salesman($myrow["user_id"]));
