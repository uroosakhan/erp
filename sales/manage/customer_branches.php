...
function branch_settings($selected_id, $num_branches) {
    ...
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DSSLS') {
        sales_groups_list_row(_("Gender:"), 'group_no', null, true);
    }
    else
    {
	...
	gl_all_accounts_list_row(_("Sales Account:"), 'sales_account', null, false, false, "Use Item Sales Account");
...
echo "</center><br>";
 echo '<center>';
 $debtor_=$_GET['debtor_no'];
 //display_error($debtor_);
 //if ($debtor_)  {
	//	start_row();
		echo '<td class="label">'._('').'</td>';
	  	hyperlink_params_td($path_to_root . "/sales/manage/add_customer_branches.php",
			'<b>'. ($page_nested ?  _("Select or &Add") : _("&ADD NEW BRANCH ")).'</b>', 
			"debtor_no=".$debtor_.($page_nested ? '&popup=1':''));
	//	end_row();
//	}
...
$get_debtors_with_no_branches = get_debtors_with_no_branches();

//dz 7/12/17
if(db_num_rows($get_debtors_with_no_branches) != 0)
{
echo "LIST OF CUSTOMERS WITH OUT BRANCHES:";    
echo "</br>";

while($myrow = db_fetch($get_debtors_with_no_branches))
{
echo $myrow['name'];
echo "</br>";
}
...
$get_branches_with_no_customers = get_branches_with_no_customers();
if(db_num_rows($get_branches_with_no_customers) != 0)
{
echo "LIST OF BRANCHES WITH OUT CUSTOMERS DATA:";    
echo "</br>";

while($myrow = db_fetch($get_branches_with_no_customers))
{
echo $myrow['br_name'];
echo "</br>";
}
