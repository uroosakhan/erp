...
page(_($help_context = "Sales Combo 4"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			update_combo_so_4($selected_id, $_POST['description']);
			$note = _('Selected combo has been updated');
    	} 
    	else 
    	{
			add_combo_so_4($_POST['description']);
...
if ($Mode == 'Delete')
{
	...
	if (key_in_foreign_table($selected_id, 'stock_master', 'combo4'))
	{
		$cancel_delete = 1;
...
$result = get_combo_so_4(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_combo_code_so_4($selected_id);
