...
function can_process()
{
    ...
    if (!check_num('credit_allowed', 0))
    {
        display_error(_("The credit Allowed must be numeric and not less than zero."));
        set_focus('credit_allowed');
        return false;
    }
    if (!check_num('credit_allowed', 0))
    {
...
function handle_submit(&$selected_id)
{
    ...
    if ($selected_id)
    {
        update_customer($_POST['customer_id'], $_POST['CustName'], $_POST['cust_ref'], $_POST['address'],
            $_POST['tax_id'],$_POST['ntn_no'],$_POST['ftn_no'],$_POST['service_text'], $_POST['curr_code'], $_POST['dimension_id'], $_POST['dimension2_id'],
            $_POST['credit_status'], $_POST['payment_terms'], input_num('discount') / 100, input_num('pymt_discount') / 100,
            input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],input_num('credit_allowed'),$_POST['cnic_no'],$_POST['cust_type']);
    ...
    { 	//it is a new customer
        ...
        add_customer($_POST['CustName'], $_POST['cust_ref'], $_POST['address'],
            ...
            $_POST['credit_status'], $_POST['payment_terms'], input_num('discount') / 100, input_num('pymt_discount') / 100,
            input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],$_POST['ntn_no'],$_POST['ftn_no'],$_POST['service_text'],input_num('credit_allowed'),$_POST['cnic_no'], $_POST['cust_type']);
        ...
        if (isset($SysPrefs->auto_create_branch) && $SysPrefs->auto_create_branch == 1)
        {
            add_branch($selected_id, $_POST['CustName'], $_POST['cust_ref'],
                ...
                get_company_pref('default_sales_discount_act'),/* get_company_pref('debtors_act')*/$_POST['receivables_account'], get_company_pref('default_prompt_payment_act'),
...
function customer_settings($selected_id)
{
    ...
    if (!$selected_id)
    {
        if (list_updated('customer_id') || !isset($_POST['CustName'])) {
            $_POST['CustName'] = $_POST['cust_ref'] = $_POST['address'] = $_POST['tax_id']  = $_POST['ntn_no']= $_POST['ftn_no'] = $_POST['cnic_no'] = $_POST['service_text'] = '';
            //$_POST['ntn_no'] = '';
    ...
    else
    {
        ...
        $_SESSION["AttachmentDebtorNo"] = $myrow["debtor_no"];
        $_POST['CustName'] = $myrow["name"];
        $_POST['cust_ref'] = $myrow["debtor_ref"];
        $_POST['address']  = $myrow["address"];
        $_POST['tax_id']  = $myrow["tax_id"];
        $_POST['ntn_no']  = $myrow["ntn_no"];
        $_POST['ftn_no']  = $myrow["ftn_no"];
        $_POST['cnic_no']  = $myrow["cnic_no"];
        $_POST['service_text']  = $myrow["service_text"];
        ...
        $_POST['credit_allowed']	= price_format($myrow["credit_allowed"]);
        $_POST['notes']  = $myrow["notes"];
        $_POST['inactive'] = $myrow["inactive"];
        $_POST['cust_type'] = $myrow["cust_type"];
    }
    ...
    text_row(_("Customer Short Name:"), 'cust_ref', null, 40, 40);

    global  $db_connections;
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"]=='SNI')
    {
        yesno_list_row_cust_type(_("Customer Type"), 'cust_type', null, _("AI"), _("AA"), _("AB"), _("AE"), _("AU"), _("AM"), false);
    }
    elseif ($db_connections[$_SESSION["wa_current_user"]->company]["name"]=='HSFHD')
    {
        yesno_list_row_cust_type2(_("Customer Type"), 'cust_type', null, _("Individual"), _("Leasing/Individual"), _("Corporate"), _("Bank Employees"),false);
    }

    textarea_row(_("Address:"), 'address', $_POST['address'], 35, 5);

    text_row(_("STRN:"), 'tax_id', null, 40, 40);
    text_row(_("NTN No:"), 'ntn_no', $_POST['ntn_no'], 40, 40);
    text_row(_("FTN No:"), 'ftn_no', $_POST['ftn_no'], 40, 40);
    text_row(_("CNIC No:"), 'cnic_no', $_POST['cnic_no'], 40, 40);
    text_row(_("SST-PRA:"), 'service_text', null, 40, 40);
    ...
    amount_row(_("Credit Allowed:"), 'credit_allowed', $_POST['credit_allowed']);
    ...
    if ($selected_id)  {
        ...
        end_row();
        start_row();
        echo '<td class="label">'._('Attachment').':</td>';
        hyperlink_params_td($path_to_root . "/admin/attachments.php",
            '<b>'. ($page_nested ?  _("Upload Attachment") : _("&Upload Attachment")).'</b>',
            "debtor_no=".$selected_id.($page_nested ? '&popup=1':''));
        end_row();
    }
