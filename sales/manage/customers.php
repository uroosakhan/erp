...
function can_process()
{
    ...
    if ($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'BNW' || 
        $db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DSSLS') {
        $string = str_replace(' ', '-', $_POST['cust_ref']); // Replaces all spaces with hyphens.
        $clean = preg_replace("/[^0-9]/", "", $string); // Removes special chars.
        if (strlen($clean) > 12 || strlen($clean) < 12) {
            display_error(_("Please enter in proper format 92XXXXXXXXXX"));
            set_focus('cust_ref');
            return false;
        }
    }
...
function handle_submit(&$selected_id)
{
	...
	if ($selected_id) 
	{
		update_customer($_POST['customer_id'], $_POST['CustName'], $_POST['cust_ref'], $_POST['address'],
			...
			$_POST['credit_status'], $_POST['DOB'], $_POST['payment_terms'], input_num('discount') / 100, input_num('pymt_discount') / 100,
			input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],input_num('credit_allowed'));
	...
	{ 	//it is a new customer
		...
		add_customer($_POST['CustName'], $_POST['cust_ref'], $_POST['address'],
			...
			$_POST['credit_status'], $_POST['DOB'], $_POST['payment_terms'], input_num('discount') / 100, input_num('pymt_discount') / 100,
			input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],$_POST['ntn_no'],$_POST['service_text'],input_num('credit_allowed'));
		...
		if (isset($SysPrefs->auto_create_branch) && $SysPrefs->auto_create_branch == 1)
		{
        	add_branch($selected_id, $_POST['CustName'], $_POST['cust_ref'],
                ...
                $_POST['location'], $_POST['address'], $_POST['group_no'], $_POST['ship_via'], $_POST['notes'], $_POST['bank_account']);
...
function customer_settings($selected_id) 
{
    ...
    if ($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'BNW' || 
        $db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DSSLS')
    {
        text_row(_("Customer Contact No:"), 'cust_ref', null, 30, 12);
    }
    else
        {
    ...
    if ($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DSSLS') {
        text_row(_("Age"), 'tax_id', null, 40, 40);
        text_row(_("File No"), 'ntn_no', null, 40, 40);
    }
    else
        {
        text_row(_("STRN:"), 'tax_id', null, 40, 40);
        text_row(_("NTN No:"), 'ntn_no', null, 40, 40);
    }
    
    global $leftmenu_save, $db_connections;
    if ($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'HOC2') {
text_row(_("CNIC No:"), 'service_text', null, 40, 40);
}
else
{
 ...
 global $leftmenu_save, $db_connections;
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DSSLS') {
        textarea_row(_("Reg Date:"), 'notes', null, 35, 5);
    }
    else
        {
