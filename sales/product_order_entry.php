...
include_once($path_to_root . "/sales/includes/product_cart_class.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/sales/includes/sales_ui.inc");
include_once($path_to_root . "/sales/includes/ui/product_order_ui.inc");
include_once($path_to_root . "/sales/includes/sales_db.inc");
include_once($path_to_root . "/sales/includes/db/product_order_db.inc");
include_once($path_to_root . "/sales/includes/db/sales_types_db.inc");
include_once($path_to_root . "/reporting/includes/reporting.inc");

set_page_security( @$_SESSION['Product']->trans_type,
...
if (isset($_GET['NewProductReg'])) {

	$_SESSION['page_title'] = _($help_context = "New Product Entry");
	create_cart(ST_SALESORDER, 0, $_GET['OfferNo']);
...
if (isset($_GET['AddedID'])) {
	...
	check_edit_conflicts(get_post('cart_id'), 'Product');
//-----------------------------------------------------------------------------
...
//		return false;
//	}
//	if ($_SESSION['Product']->trans_type!=ST_SALESORDER && $_SESSION['Product']->trans_type!=ST_SALESQUOTE && !is_date_in_fiscalyear($_POST['OrderDate'])) {
//		display_error(_("The entered date is out of fiscal year or is closed for further data entry."));
//		set_focus('OrderDate');
//		return false;
//	}
//	if (count($_SESSION['Product']->line_items) == 0)	{
//		display_error(_("You must enter at least one non empty item line."));
//		set_focus('AddItem');
//		return false;
//	}
//	if (!$SysPrefs->allow_negative_stock() && ($low_stock = $_SESSION['Product']->check_qoh()))
//	{
//		display_error(_("This document cannot be processed because there is insufficient quantity for items marked."));
//		return false;
//	}
//	if ($_SESSION['Product']->payment_terms['cash_sale'] == 0) {
//		if (!$_SESSION['Product']->is_started() && ($_SESSION['Product']->payment_terms['days_before_due'] == -1) && ((input_num('prep_amount')<=0) ||
//			input_num('prep_amount')>$_SESSION['Product']->get_trans_total())) {
...
//		}
//
//		if ($_SESSION['Product']->trans_type != ST_SALESQUOTE && strlen($_POST['delivery_address']) <= 1) {
...
//			if ($_SESSION['Product']->trans_type==ST_SALESQUOTE)
...
//		}
//	}
//	if (!$Refs->is_valid($_POST['ref'], $_SESSION['Product']->trans_type)) {
//		display_error(_("You must enter a reference."));
//		set_focus('ref');
//		return false;
//	}
//	if (!db_has_currency_rates($_SESSION['Product']->customer_currency, $_POST['OrderDate']))
//		return false;
//
//   	if ($_SESSION['Product']->get_items_total() < 0) {
...
if (isset($_POST['ProcessOrder']) && can_process()) {
    ...
    foreach ($_SESSION['Product']->line_items as $line=>$itm)
    {
        $_SESSION['Product']->line_items[$line]->on_value1 = check_value('on_value1'.$line);
    }
	$modified = ($_SESSION['Product']->trans_no != 0);
	$so_type = $_SESSION['Product']->so_type;
	$ret = $_SESSION['Product']->write(1);
//	if ($ret == -1)
//	{
//		display_error(_("The entered reference is already in use."));
//		$ref = $Refs->get_next($_SESSION['Product']->trans_type, null, array('date' => Today()));
//		if ($ref != $_SESSION['Product']->reference)
//		{
//			display_error(_("The reference number field has been increased. Please save the document again."));
//			$_POST['ref'] = $_SESSION['Product']->reference = $ref;
//			$Ajax->activate('ref');
//		}
//		set_focus('ref');
//	}
...
//			display_footer_exit();
//		}
		$trans_no = key($_SESSION['Product']->trans_no);
		$trans_type = $_SESSION['Product']->trans_type;
		new_doc_date($_SESSION['Product']->document_date);
		processing_end();
		/*if ($modified) {
		...
		{
			meta_forward($_SERVER['PHP_SELF'], "NewProductReg=Yes&OfferNo=".$_SESSION['Product']->offer_id);
		}
	}	
}
...
function check_item_data()
{
	...
	elseif (!check_num('qty', 0) || !check_num('Disc', 0, 100)) {
		...
		set_focus('price');
		return false;
	} elseif (isset($_POST['LineNo']) && isset($_SESSION['Product']->line_items[$_POST['LineNo']])
	    && !check_num('qty', $_SESSION['Product']->line_items[$_POST['LineNo']]->qty_done)) {
	...
	$cost = $cost_home / get_exchange_rate_from_home_currency($_SESSION['Product']->customer_currency, $_SESSION['Product']->document_date);
	if (input_num('price') < $cost)
	{
		$dec = user_price_dec();
		$curr = $_SESSION['Product']->customer_currency;
...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Product']->update_cart_item($_POST['LineNo'],
...
{
    if ($_SESSION['Product']->some_already_delivered($line_no) == 0) {
	    $_SESSION['Product']->remove_from_cart($line_no);
    } else {
...
//			return;
//	}
	add_to_order($_SESSION['Product'], get_post('stock_id'), $_POST['from_date'],
    $_POST['to_date'], input_num('dozen'), $_POST['packing1'], $_POST['packing2'],
    $_POST['packing3'], $_POST['scheme'], $_POST['scheme_on_ctn'], $_POST['scheme_from_date'], $_POST['scheme_to_date']);
...
function  handle_cancel_order()
{
	...
	if ($_SESSION['Product']->trans_type == ST_CUSTDELIVERY) {
	...
	} elseif ($_SESSION['Product']->trans_type == ST_SALESINVOICE) {
	...
	} elseif ($_SESSION['Product']->trans_type == ST_SALESQUOTE)
	{
		if ($_SESSION['Product']->trans_no != 0) 
			delete_sales_order(key($_SESSION['Product']->trans_no), $_SESSION['Product']->trans_type);
		display_notification(_("This sales quotation has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Quotation"), "/sales/sales_order_entry.php?NewQuotation=Yes");
	} else { // sales order
		if ($_SESSION['Product']->trans_no != 0) {
			$order_no = key($_SESSION['Product']->trans_no);
...
function create_cart($type, $trans_no, $offer_id)
{ 
	global $Refs, $SysPrefs;
	...
	if (isset($_GET['NewQuoteToSalesOrder']))
	{
		...
		$doc = new product_cart(ST_SALESQUOTE, $trans_no, true);
		$doc->Comments = _("Sales Quotation") . " # " . $trans_no;
		$_SESSION['Product'] = $doc;
	}	
	elseif($type != ST_SALESORDER && $type != ST_SALESQUOTE && $trans_no != 0) { // this is template

		$doc = new product_cart(ST_SALESORDER, array($trans_no));
		$doc->trans_type = $type;
		$doc->trans_no = 0;
		...
		$_SESSION['Product'] = new product_cart($type, array($trans_no), false, $offer_id);
	copy_from_cart();
}
...
//--------------------------------------------------------------------------------
if ($_SESSION['Product']->fixed_asset)
...
hidden('cart_id');
$customer_error = display_order_header($_SESSION['Product'], !$_SESSION['Product']->is_started(), $idate);

if ($customer_error == "") {
	start_table(TABLESTYLE, "width='80%'", 10);
	echo "<tr><td>";
	display_order_summary($orderitems, $_SESSION['Product'], true);
	echo "</td></tr>";
//	echo "<tr><td>";
//	display_delivery_details($_SESSION['Product']);
//	echo "</td></tr>";
	end_table(1);
	...
	if ($_SESSION['Product']->trans_no == 0) {
		...
		submit_center_last('CancelOrder', $cancelorder,
	   		_('Cancels document entry or removes sales order when editing an old document'));
		if ($_SESSION['Product']->trans_type==ST_SALESORDER)
