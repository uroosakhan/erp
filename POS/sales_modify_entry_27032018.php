...
?>
    ...
    <?php
    echo '<a id="thickboxId" href="../reporting/prn_redirect.php?PARAM_0='.$doc_no.'&PARAM_1='.$doc_no.'&PARAM_2=&PARAM_3=0&PARAM_4=&PARAM_5=&PARAM_6='.$def_orientation.'&REP_ID='.$rep.'" target="_self"></a>';
    echo '<a id="thickboxform" href="../sales/pos_order_entry.php?NewInvoice=0" target="_blank"></a>';
...
//        }
    ...
    if (!db_has_currency_rates($_SESSION['Items']->customer_currency, $_POST['OrderDate'])){
        display_error("Invoice total amount cannot be less than zero1232133213123.");
        return false;
    }
...
//--
  ...
  <div class="col-md-5.5 " >';
    start_table_pos_running_orders(TABLESTYLE, "width=0%");
        ...
        $th = array( _('Tables'), _('Time'), _('Total'),  _('KOT'),_('P.P'),_('Cash Received'), _('Cash Return'),
         _('Settle '),'', 'Done', '');
...
//            }
            ...
            if($getorder_no != 0)
            {
                label_cell_pos(get_cust_branches($myrow["branch_code"]));
                label_cell_pos($myrow["order_time"]);
                label_cell_pos(number_format($myrow["TotalAmount"], $dec));
                $max_order=get_customer_order_maxss($myrow['debtor_no']);
                ...
                label_cell_pos("<input type='text' style='color:black; width:50px;'  onfocusout='myFunction($num)' id='cash_recieved$num' name='txt_cash_recieved'");
                label_cell_pos("<input type='text' style='color:black;width:50px;'   style='margin-top:5px;' id='cash$num' name='cash$num' readonly='true'");
...
//                }
                ...
                delete_button_cell("Delete" . $myrow['order_no'], _("Delete"));
            }
            end_row();
            $num++;
        }
