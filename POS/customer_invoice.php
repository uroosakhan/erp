...
if (isset($_POST['_InvoiceDate_changed'])) {
	$_POST['due_date'] = get_invoice_duedate_pos($_SESSION['Items']->payment, $_POST['InvoiceDate']);
...
function set_delivery_shipping_sum($delivery_notes) 
{
    ...
    {
        $myrow = get_customer_trans_pos($delivery_num, 13);
        //$branch = get_branch_pos($myrow["branch_code"]);
        //$sales_order = get_sales_order_header_pos($myrow["order_"]);
...
label_cells(_("Branch"), get_branch_name_pos($_SESSION['Items']->Branch), "class='tableheader2'");
...
$_POST['InvoiceDate']= get_current_company_date(3);
}
else{
   $_POST['InvoiceDate']; 
...
label_row(_("Total Discount"), $_SESSION['Items']->total_discount.' %', "colspan=$colspan align=right","align=right", $is_batch_invoice ? 2 : 0);

$taxes = $_SESSION['Items']->get_taxes(input_num('ChargeFreightCost'));
$tax_total = display_edit_tax_items($taxes, $colspan, $_SESSION['Items']->tax_included, $is_batch_invoice ? 2:0);

$total_disc = ($inv_items_total + input_num('ChargeFreightCost') + $tax_total) / 100 * $_SESSION['Items']->total_discount ;
$display_total = price_format(($inv_items_total + input_num('ChargeFreightCost') + $tax_total - $total_disc ));
