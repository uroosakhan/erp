...
{
	$br = get_branch_pos($_GET['SelectedBranch']);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1)
		{
			update_branch_pos($_POST['customer_id'], $_POST['branch_code'], $_POST['br_name'], $_POST['br_ref'],
...
//				'cust_branch', 'branch_code');
		...
		{
			add_branch_pos($_POST['customer_id'], $_POST['br_name'], $_POST['br_ref'],
				$_POST['br_address'], $_POST['salesman'], $_POST['area'], $_POST['tax_group_id'], $_POST['sales_account'],
				$_POST['sales_discount_account'], $_POST['receivables_account'], $_POST['payment_discount_account'],
...
elseif ($Mode == 'Delete')
{
	...
	if (branch_in_foreign_table_pos($_POST['customer_id'], $_POST['branch_code'], 'debtor_trans'))
	{
	...
	{
		if (branch_in_foreign_table_pos($_POST['customer_id'], $_POST['branch_code'], 'sales_orders'))
		{
			display_error(_("Cannot delete this branch because sales orders exist for it. Purge old sales orders first."));
		}
		else
		{
			delete_branch_pos($_POST['customer_id'], $_POST['branch_code']);
...
function branch_settings($selected_id) {
	...
	if ($selected_id != -1)
	{
	 	if ($Mode == 'Edit' || !isset($_POST['br_name'])) {
			...
			$myrow = get_cust_branch_pos($_POST['customer_id'], $_POST['branch_code']);
			set_focus('br_name');
...
//		    $_POST['email'] =$myrow["email"];
	...
	{ //end of if $SelectedBranch only do the else when a new record is being entered
		$myrow = get_default_info_for_branch_pos($_POST['customer_id']);
//		$_POST['rep_lang'] = $myrow['rep_lang'];
		if(!$num_branches) {
	...
	gl_all_accounts_list_row(_("WHT:"), 'payment_discount_account');
...
$sql = get_sql_for_customer_branches_pos();
