...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_sales_area_pos($selected_id, $_POST['description']);
			$note = _('Selected sales area has been updated');
    	} 
    	else 
    	{
    		add_sales_area_pos($_POST['description']);
...
if ($Mode == 'Delete')
{
	...
	if ($cancel_delete == 0) 
	{
		delete_sales_area_pos($selected_id);
...
$result = get_sales_areas_pos(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_sales_area_pos($selected_id);
