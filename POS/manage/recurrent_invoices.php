...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_recurrent_invoice_pos($selected_id, $_POST['description'], $_POST['order_no'], input_num('debtor_no'), 
    			input_num('group_no'), input_num('days', 0), input_num('monthly', 0), $_POST['begin'], $_POST['end']);
    		if (isset($_POST['last_sent']))	
				update_last_sent_recurrent_invoice_pos($selected_id, $_POST['last_sent']);
			$note = _('Selected recurrent invoice has been updated');
    	} 
    	else 
    	{
    		add_recurrent_invoice_pos($_POST['description'], $_POST['order_no'], input_num('debtor_no'), input_num('group_no'),
...
if ($Mode == 'Delete')
{
	...
	if ($cancel_delete == 0) 
	{
		delete_recurrent_invoice_pos($selected_id);
...
$result = get_recurrent_invoices_pos();
...
while ($myrow = db_fetch($result)) 
{
	...
	{
		label_cell("");
		label_cell(get_sales_group_name_pos($myrow["group_no"]));
	}	
	else
	{
		label_cell(get_customer_name_pos($myrow["debtor_no"]));
		label_cell(get_branch_name_pos($myrow['group_no']));
	}	
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_recurrent_invoice_pos($selected_id);
