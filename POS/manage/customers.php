...
function list_table_type($name, $selected_id=null, $name_yes="", $name_no="", $submit_on_change=false)
{
    $items = array();
    $items['0'] = strlen($name_no) ? $name_no   : _("DINE IN");
    $items['1'] = strlen($name_yes) ? $name_yes : _("TAKE AWAY");
    $items['2'] = strlen($name_yes) ? $name_yes : _("DELIVERY");
...
function list_cells_table_type($label, $name, $selected_id=null, $name_yes="", $name_no="", $submit_on_change=false)
{
    if ($label != null)
        echo "<td>$label</td>\n";
    echo "<td>";
    echo list_table_type($name, $selected_id, $name_yes, $name_no, $submit_on_change);
    echo "</td>\n";
}

function table_type($label, $name, $selected_id=null, $name_yes="", $name_no="", $submit_on_change)
{
    echo "<tr><td class=''>$label</td>";
    list_cells_table_type(null, $name, $selected_id, $name_yes, $name_no, $submit_on_change);
    echo "</tr>\n";
}
...
function handle_submit(&$selected_id)
{
	...
	if ($selected_id)
	{
		update_customer_pos($_POST['customer_id'], $_POST['CustName'], $_POST['cust_ref'], $_POST['address'], $_POST['ntn_id'],
			...
			input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],$_POST['select_type']);
	...
	{ 	//it is a new customer
		...
		begin_transaction();
		add_customer_pos($_POST['CustName'], $_POST['cust_ref'], $_POST['address'],$_POST['ntn_id'],
			$_POST['tax_id'], $_POST['curr_code'], $_POST['dimension_id'], $_POST['dimension2_id'],
			$_POST['credit_status'], $_POST['payment_terms'], input_num('discount') / 100, input_num('pymt_discount') / 100,
			input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],$_POST['select_type']);
		...
		{
			add_branch_pos($selected_id, $_POST['CustName'], $_POST['cust_ref'],
...
if (isset($_POST['delete']))
{
	...
	{ 	//ie not cancelled the delete as a result of above tests
		...
		delete_customer_pos($selected_id);
...
function customer_settings($selected_id)
{
	...
	if (!$selected_id)
	{
		if (list_updated('customer_id') || !isset($_POST['CustName'])) {
			$_POST['CustName'] = $_POST['cust_ref'] = $_POST['address'] = $_POST['ntn_id']  = $_POST['tax_id'] = '';
	...
	else
	{
		...
		$_POST['select_type']  = $myrow["table_type"];
	...
	text_row(_("NTN No:"), 'ntn_id', null, 40, 40);

	text_row(_("GST No:"), 'tax_id', null, 40, 40);

	table_type(_("Select Type:"), 'select_type', null,	'', '',false);
	...
	if ($dim >= 1)
		dimensions_list_row(_("Cost Centre")." 1:", 'dimension_id', $_POST['dimension_id'], true, " ", false, 1);
	if ($dim > 1)
		dimensions_list_row(_("Cost Centre")." 2:", 'dimension2_id', $_POST['dimension2_id'], true, " ", false, 2);
	if ($dim < 1)
