...
$myrow = get_customer_trans_invoice($trans_id, ST_CUSTDELIVERY);

$branch = get_branch_pos($myrow["branch_code"]);

$sales_order = get_sales_order_header_pos($myrow["order_"], ST_SALESORDER);
...
$result = get_customer_trans_details_invoice(ST_CUSTDELIVERY, $trans_id);
...
label_row(_("Total Discount"), $myrow["total_discount"].' %', "colspan=6 align=right", "nowrap align=right");

$tax_items = get_trans_tax_details(ST_CUSTDELIVERY, $trans_id);
//display_customer_trans_tax_details($tax_items, 6);

$total_disc = ($myrow["ov_freight"]+$myrow["ov_amount"]+$myrow["ov_freight_tax"]+$myrow["ov_gst"]) / 100 * $myrow["total_discount"] ;
$display_total = price_format($myrow["ov_freight"]+$myrow["ov_amount"]+$myrow["ov_freight_tax"]+$myrow["ov_gst"]- $total_disc);
