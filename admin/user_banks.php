...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	$num = check_user_banks_duplication(get_post('user_id'), get_post('loc_code'));
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
            update_users_banks($selected_id, $_POST['user_id'], $_POST['loc_code']);
    		display_notification_centered(_("The selected user's location has been updated."));
    	} 
    	else 
    	{
            add_user_banks( $_POST['user_id'], $_POST['loc_code']);
			$id = db_insert_id();
...
$result = get_users_banks(check_value('show_inactive'), $user_id);
start_form();
 start_table(TABLESTYLE2, "width=40%");

$th = array(  _("User"), _("Bank Name"),  "");
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell(get_bank_description($myrow["loc_code"]));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_user_bank($selected_id);
...
bank_accounts_list_for_bank_restriction_setup_cells(_("Bank:") , 'loc_code');
