...
{
	add_shipper($_POST['shipper_name'], $_POST['contact'], $_POST['phone'], $_POST['phone2'], $_POST['address'], $_POST['shipping_account']);
...
{
	update_shipper($selected_id, $_POST['shipper_name'], $_POST['contact'], $_POST['phone'], $_POST['phone2'], $_POST['address'], $_POST['shipping_account']);
...
$th = array(_("Name"), _("Contact Person"), _("Phone Number"), _("Secondary Phone"), _("Address"), _("Shipping Account"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell(get_gl_account_name_for_shippment($myrow["shipping_account"]));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['shipping_account'] = $myrow["shipping_account"];
	}
...
gl_all_accounts_list_row(_("Shipping Charged Account:"), 'shipping_account', get_company_pref('freight_act'));
end_table(1);
