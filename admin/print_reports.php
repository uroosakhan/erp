...
if ( get_post('submit'))
{
//	$sql = "UPDATE `0_print_reports` SET
//                                report_name  =".db_escape(get_post('Prn'))."
//                                WHERE  report_no=
//                                101";
...
//	}
	...
	if (!$error)
	{
		$prof = array('' => get_post('Prn'),0 => get_post('check'),
			1 => get_post('prn_name'),2 => get_post('remarks'));
		

		// store default value/profile name
		foreach (get_reports() as $rep => $descr) {
			$val = get_post('Prn'.$rep);
			$val1 = get_post('check'.$rep);
			$val2 = get_post('prn_name'.$rep);
			$val3 = get_post('remarks'.$rep);
			$prof[$rep] = $val.",".$val1.",".$val2.",".$val3;
//			$prof1[$rep] = $val1;
//			$prof2[$rep] = $val2;
			

//			display_error($val4."/".$val1."/".$val2);
//			die;
		...
		//	if ($_POST['profile_id']=='')
		$com_code = $db_connections[$_SESSION["wa_current_user"]->company]["name"];
		
		
		update_printer_report_profile($com_code, $prof);
		if ($selected_id == '') {
...
$com_code = $db_connections[$_SESSION["wa_current_user"]->company]["name"];
$result = get_report_profile($com_code);
$prints = array();
while ($myrow = db_fetch($result)) {
	$prints[$myrow['report_no']] = $myrow['inactive'];
	$prints1[$myrow['report_no']] = $myrow['remarks'];

	
}

start_table(TABLESTYLE);
$th = array(_("Report Id"), _("Description"), _(""), _("Reports Name"),_("Remarks"), _("Security Check"));
table_header($th);
...
foreach(get_reports() as $rep => $descr)
{
	...
	$_POST['check'.$rep] = isset($prints[$rep]) ? $prints[$rep] : '';
	$_POST['remarks'.$rep] = isset($prints1[$rep]) ? $prints1[$rep] : '';
	echo '<td>';
	hidden('report_no'.$rep,$rep);
	text_cells(_(""), 'prn_name'.$rep, $descr);
	text_cells(_(""), 'remarks'.$rep);
	check_cells(_(""), 'check'.$rep, null);
	hidden('report_no',$rep);
	echo '</td>';
	if ($descr == '') $unkn = 1;
	end_row();
}
