	...
	if (can_process($Mode == 'ADD_ITEM'))
	{
    	if ($selected_id != -1) 
    	{
    		update_user_prefs($selected_id,
    			get_post(array('user_id', 'real_name', 'phone', 'email', 'role_id', 'language',
					'print_profile', 'rep_popup' => 0, 'pos'/*, 'BankAccount'*/)));
    	...
    	else 
    	{
    		add_user($_POST['user_id'], $_POST['real_name'], md5($_POST['password']),
				...
				$_POST['print_profile'], check_value('rep_popup'), $_POST['pos']/*, $_POST['BankAccount']*/);
			$id = db_insert_id();
			add_user_locations($id,'DEF');
...
$th = array(_("User login"), _("Full Name"), _("Phone"),
	_("E-mail"), _("Last Visit"), _("Access Level")/*, _("Bank Account")*/, "", "");
...
// 	label_cell(get_bank_account_name($myrow["BankAccount"]));
...
// bank_accounts_list_all_row_pos(_("Bank Account:"), 'BankAccount', null, false);
