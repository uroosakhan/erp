...
function exist_transaction($type, $type_no)
{
    ...
    $data = get_voidng_entry_data($type, $type_no);
    $date = Today();
    $qoh = get_qoh_on_date($data['stock_id'],$data['loc_code'],$date,$data['batch']);
    $after_voiding_batch_qty = $qoh - $data['qty'];
    ...
    switch ($type)
    {
        ...
        case ST_INVADJUST   :
        case ST_WORKORDER   :
            if ($after_voiding_batch_qty < 0 && $pref['batch'] == 1) {
                display_error("The entered transaction can not be voided as it would result in negative inventory .");
                return false;
            }
            break;
        ...
        case ST_CPV:  // it's a payment
        case ST_CRV : // it's a payment
        ...
        case ST_SUPPCREDIT_IMPORT : // it's a supplier credit note
...
function get_amount_type_wise($row)
{
     $trans_no=$row['trans_no'];
    $type=$row['type'];
    if($type!='')
   {
	$sql="SELECT SUM(`amount`) FROM `0_gl_trans` WHERE `type_no`=$trans_no AND `type`=$type AND amount>0 ";
...
if (isset($_POST['ProcessVoiding']))
{
    ...
    if (!$del_count) {
        ...
        //var_dump($selected);
        //meta_forward($path_to_root . '/sales/sales_tax_invoice.php','BatchInvoice=Yes');
    }
}
function voiding_controls()
{
    ...
    $not_implemented =  array(ST_PURCHORDER, ST_SALESORDER, ST_SALESQUOTE);
    ...
    date_cells(_("From Date:"), 'from_date', '', null, -user_transaction_days(),0,0, $year['begin'],true);
    date_cells(_("To Date:"), 'to_date', '', null, 0, 0,0,$year['end'],true);
    submit_cells('ProcessSearch', _("Search"), '', '', 'default');

    //Uroosa
    echo '<a href="/gl/inquiry/void_inquiry.php">Voided Inquiry</a>';
    ///
    ...
    $sql = get_sql_for_view_transactions(get_post('filterType'), get_post('FromTransNo'),
        get_post('ToTransNo'), $trans_ref,get_post('from_date'),get_post('to_date'), get_post('ref_no'));
    if ($sql == "")
        return;
    ...
    $cols = array(
        ...
        _("Amount") => array('fun'=>'get_amount_type_wise', 'type'=>'amount', 'ord'=>''),
    ...
    {
        $a=1;
        foreach ($_SESSION['trans_no'] as $value)
        {
            $_SESSION["id"]= $value;

            if (!exist_transaction($_POST['filterType'],$value))
            {
...
function check_valid_entries()
{
    foreach ($_SESSION['trans_no'] as $value) {
        ...
        if (is_closed_trans($_POST['filterType'],$value))
        {
...
function handle_void_transaction()
{
    if (check_valid_entries()==true)
    {
        ...
        foreach ($_SESSION['trans_no'] as $value) {
            ...
            $msg = void_transaction($_POST['filterType'], $value,
