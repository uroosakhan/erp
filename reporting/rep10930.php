...
$cols = array(4, 45,140,200, 248, 250, 310, 355, 405,445, 515);

	// $headers in doctext.inc
	//$aligns = array('left',	'left',	'right', 'left', 'right', 'right', 'right');
	$aligns = array('left',	'left','center','left','left', 'left', 'left', 'left', 'right');
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		//	$rep->TextCol(0, 1,	$myrow2['stk_code'], -2);
			...
			$expired_date = get_purchasing_date($myrow2['stk_code'],$myrow2['batch']);
			$newrow = $rep->row;
			$rep->row = $oldrow;
...
$amount = $myrow2["quantity"] * $myrow2["unit_price"];
				...
				$rep->TextCol(3, 4,$myrow2['batch'].	"   ", -2);
				//$rep->TextCol(4, 5,	" ".$expired_date, -2);
				$rep->TextCol(5, 6,$myrow2['bonus'], -2);
				$rep->TextCol(6, 7,	$DisplayPrice, -2);
				$rep->TextCol(7, 8,	$DisplayAmount, -2);
		...
		$rep->TextCol(6, 8, _("Sub-total"), -20);
		$rep->TextCol(8, 9,	"     ".$DisplaySubTot, -2);
		$rep->NewLine();
		$rep->TextCol(6, 8, _("Shipping"), -20);
		...
		$DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal, $dec);
		$rep->Font('bold');
		$rep->TextCol(6, 8, _("TOTAL ORDER GST INCL."), - 2);
