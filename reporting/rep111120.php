...
function getTransactions($category, $location, $fromcust, $from, $to, $item_location,$salesmen)
{
	$from = date2sql($from);
	$to = date2sql($to);
	...
	$sql = "SELECT
			...
			AND
				...
				".TB_PREF."cust_branch.debtor_no = ".TB_PREF."debtors_master.debtor_no
                        ...
                        AND
				".TB_PREF."cust_branch.salesman= ".TB_PREF."salesman.salesman_code
			...
			AND
				".TB_PREF."sales_orders.delivery_date = '$from'
...
//	".TB_PREF."item_units.abbr = '$unit'
			...
			if ($location != '')
				$sql .= " AND ".TB_PREF."sales_orders.from_stk_loc= ".db_escape($location);
                        ...
                        if ( $salesmen!= -1)
				$sql .= " AND ".TB_PREF."cust_branch.salesman= ".db_escape($salesmen);
...
//	$to = $_POST['PARAM_1'];
	...
	$headers = array(_('Serial No.'), _('Stock Id'), _('Description'), _(''), _('Urdu Name'), _('Units'), _('Quantity'));
	...
	$sql = "SELECT * FROM ".TB_PREF."item_location WHERE inactive = 0";
	if ($item_location != -1)
		$sql .= " AND ".TB_PREF."item_location.id = ".db_escape($item_location);
	$sql .= " ORDER BY order_no";
	$fetch = db_query($sql, "Error");
	$S_no = 0;

	while($get_item_location = db_fetch($fetch))
	{
		$rep->fontSize += 1;
		$rep->Font('bold');
		$rep->TextCol(0, 1, $get_item_location['description']);
		$rep->Font('');
		$rep->NewLine();
		$rep->fontSize -= 1;

		$res = getTransactions($category, $location, $fromcust, $from, $to, $get_item_location['id'],$salesmen);
