...
function get_item_purchase_price( $stock_id)
{
...
function getTransactions($category, $location, $date)
{
    ...
    $sql = "SELECT item.category_id,
			...
			move.loc_code,
			move.units_id,
			SUM(move.qty * item.con_factor) AS QtyOnHand,  
			(item.material_cost  / item.con_factor)  AS UnitCost,
			SUM(move.qty * move.con_factor) * item.material_cost AS ItemTotal 
			FROM "
...
function print_inventory_valuation_report()
{
    ...
    $rep = new FrontReport(_('Inventory Valuation Report (Sec.UOM)'), "InventoryValReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
    ...
    while ($trans=db_fetch($res))
    {
      ...
      /*  if (isset($SysPrefs->use_costed_values) && $SysPrefs->use_costed_values==1)
        ...
        if ($detail)
        {
            ...
            $rep->TextCol(2, 3, $item['alt_units']);
          ...
          //  $rep->AmountCol(4, 5, $UnitCost, $dec2);
            $purch = get_item_purchase_price($trans['stock_id']);
            $rep->AmountCol(4, 5, $UnitCost, $dec);//Ayesha 10-11-2017
