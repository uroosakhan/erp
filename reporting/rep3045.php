...
function getTransactions($category, $location, $fromcust, $from, $to,$dist_cust,$trans_no)
{
	...
	$sql = "SELECT ".TB_PREF."stock_master.category_id,
			...
			".TB_PREF."stock_master.description as desasdasc, 
...
function get_distinct_customer($category, $location, $fromcust, $from, $to)
{
	...
	$sql = "SELECT * FROM ".TB_PREF."debtor_trans 
	    	...
	    	AND (".TB_PREF."debtor_trans.type=".ST_CUSTDELIVERY." OR ".TB_PREF."debtor_trans.type=".ST_CUSTCREDIT.")";
	    	
	$sql .= " GROUP BY ".TB_PREF."debtor_trans.trans_no  ORDER BY ".TB_PREF."debtor_trans.trans_no";
// 	$sql = " SELECT
   ...
   // $dec = user_price_dec();
	...
	while($dist_cust = db_fetch($distinct_cust))
    {
		...
		while ($trans = db_fetch($res))
        {
			...
			$rep->TextCol(2, 3, $trans['stock_id'] . " - " . $trans['desasdasc'] ."-". $trans['reference'], -1);

			$rep->AmountCol(3, 4, /*$trans['price']*/$trans['amt'], $dec);
			$rep->AmountCol(4, 5, $trans['qty'], get_qty_dec($trans['stock_id']));
			$rep->AmountCol(5, 6, $discount_amount);
			...
			$net_amount = abs($trans['amt'] );
			$net_amount_neg = abs($trans['amt'] - $discount_amount);

			$rep->AmountCol(8, 9, $net_amount_neg, $dec);
