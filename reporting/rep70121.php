...
include_once($path_to_root . "/includes/dathe_functions.inc");
...
function get_gl_trans11($type, $trans_id)
{
    $sql = "SELECT gl.*, cm.account_name, IFNULL(refs.reference, '') AS reference, user.real_name, 
			...
			IF(ISNULL(st.supp_reference), '', st.supp_reference) AS supp_reference, gl.cheque as chq
...
function print_Chart_of_Accounts()
{
    ...
    $cols = array(2, 50, 199, 200,450);

    $headers1 = array(_('    Code'), _('                   '), _('                           '), _('      Rs'), _('        Rs'));
...
//    $rep->getFontSize(12);
    ...
    $rep->MultiCell(160, 20, "Number :  ". $from_trans["ref"] ,0, 'L', 0, 2, 50,90, true);

    //  $from_trans['person_id']$from_trans['account_code']." - ". $from_trans['bank_account_name']
$res1=get_gl_trans11($from_trans['type'],$from_trans['trans_no']);
$myrow21 = db_fetch($res1);
    ...
    else
    {
        ...
        if($myrow21['cheque_no'] != 0)
         $rep->MultiCell(160, 20, "Cheque Ref:  ".$myrow21['cheque_no'] ,0, 'L', 0, 2, 435,135, true);
        else
        $rep->MultiCell(160, 20, "Cheque Ref:  ".$myrow21['chq'] ,0, 'L', 0, 2, 435,135, true);
...
//    $rep->MultiCell(170, 30, $trans_no."++++".$type ,0, 'L', 0, 2, 90,199, true);
 ...
 $res=get_gl_trans11($from_trans['type'],$from_trans['trans_no']);   
    ...
    // display_error($myrow1["person_type_id"]);
        ...
        if($myrow1['amount'] >0)
        {
            $rep->AmountCol(3, 4,	($myrow1['amount']), 2);
            $dr += ($myrow1['amount']);
        }
        else
        {
            ...
            $rep->AmountCol(4, 5,	($myrow1['amount']*-1), 2);
        }
...
{
     $rep->TextWrapLines(240, 180, get_comments_string($from_trans['type'], $from_trans['trans_no']));
 }
else
{
         $rep->TextWrapLines(240, 180, $myrow1['memo_']);
...
//    $rep->MultiCell(525, 15,"",1, 'L', 0, 2, 40,516, true);
    ...
    $rep->MultiCell(150, 15,"",1, 'L', 0, 2, 88,516, true);
    $rep->MultiCell(187, 15,"Total   ",1, 'R', 0, 2, 238,516, true);
//    $rep->MultiCell(62, 15,"",1, 'L', 0, 2, 418,516, true);
    $rep->MultiCell(70, 15,"".number_format2($dr,2),1, 'R', 0, 2, 425,516, true);
    $rep->MultiCell(70, 15," ".number_format2($dr,2),1, 'R', 0, 2, 495,516, true);
 global  $db_connections;
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL' || $db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL2'){
        ...
        $rep->MultiCell(120, 20, number_format2(abs($dr),2),1, 'R', 0, 2, 410,560, true);
    }
    ...
    $rep->MultiCell(100, 25, "Checked By" ,0, 'C', 0, 2, 170,770, true);
    $rep->MultiCell(100, 25, "Approved By" ,0, 'C', 0, 2, 300,770, true);
