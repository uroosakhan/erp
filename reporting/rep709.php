...
function getTaxTransactions($from, $to, $type)
{
	$fromdate = date2sql($from);
	$todate = date2sql($to);

	$sql = "SELECT tt.name as taxname, taxrec.*, taxrec.amount*ex_rate AS amount,
	            taxrec.net_amount*ex_rate AS net_amount,debt.ntn_no,debt.address,
        ...
        if ($type != -1)
        $sql .= " AND taxrec.trans_type= ".db_escape($type);
...
//function get_party_data($month,$dept,$employee)
//{
//    $sql = "SELECT present FROM ".TB_PREF."presence
//    WHERE employee_id=".db_escape($employee)
//	;
//
...
function print_tax_report()
{
	...
	if($orientation == 'P') {
        ...
        $cols = array(0, 80, 130, 180, 270, 350, 400, 430, 480, 485, 520, 570);

        $headers = array(_('Trans Type'), _('Ref'), _('Date'), _('Name'), _('Address'),
            _('Net'), _('Rate'), _('Tax'), '', _('Name'), _('NTN'));
        $aligns = array('left', 'left', 'left', 'left', 'left', 'right', 'right', 'right', 'right',
            'left','left');
    }
    if ($orientation == 'L')
	...
	$totalnet = 0.0;
	$totaltax = 0.0;
	$transactions = getTaxTransactions($from, $to, $type);
	...
	while ($trans=db_fetch($transactions))
	{
		...
		if (!$summaryOnly)
		{
            ...
            if($orientation == 'P')
            {
                ...
                $rep->TextCol(4, 5, $trans['address']);
                $rep->TextCol(10, 11, $trans['ntn_no']);
            }
