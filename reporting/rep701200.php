...
{
    $sql = "SELECT bt.*, act.*, supplier.* , dt.branch_code, cb.branch_ref, cb.br_name, supplier.supp_ref,debtor.name,
(bt.amount - (+ st.supply_disc + st.service_disc + st.fbr_disc +  st.srb_disc)) as amount,
        ...
        LEFT JOIN ".TB_PREF."supp_trans st ON st.type=bt.type AND st.trans_no=bt.trans_no
         LEFT JOIN ".TB_PREF."cust_branch cb ON cb.debtor_no=debtor.debtor_no
...
function get_cust_name_od($debtor_no)
{
...
function print_Chart_of_Accounts()
{
    ...
    $cols = array(2, 50, 199, 370, 440, 480);
...
//    $headers = array(_(' Account'), _(' Account'), _(''),_('Centre'), _('Rs'), _('Rs'));
    ...
    $rep = new FrontReport(_(''), "ChartOfAccounts", user_pagesize(), 8, $orientation);
    if ($orientation == 'L')
        recalculate_cols($cols);

    $rep->Font();
    $rep->SetHeaderType('Header701200');
...
//    $rep->getFontSize(12);
    ...
    $rep->MultiCell(160, 20, "Number :  ". $from_trans["trans_no"] ,0, 'L', 0, 2, 50,89, true);

    //  $from_trans['person_id']$from_trans['account_code']." - ". $from_trans['bank_account_name']
 
    if( $from_trans['type'] == 1)
    {
...
//        $rep->MultiCell(170, 30, "Bank Title:  ".htmlspecialchars_decode($from_trans['bank_account_name']) ,0, 'L', 0, 2, 50,155, true);
        $rep->MultiCell(200, 20, "Pay to: (Customer / Supplier)",0, 'L', 0, 2, 50,140, true);
        $rep->MultiCell(200, 20, "".$from_trans['name'] .$from_trans['supp_name'] ,0, 'L', 0, 2, 50,150, true);
        $rep->MultiCell(200, 20, "Branch Name: ".get_branch_name_od($from_trans['branch_code']),0, 'L', 0, 2, 50,160, true);
        $rep->MultiCell(160, 20, "Cheque Ref:  ".$from_trans['cheque'] ,0, 'R', 0, 2, 370,140, true);
        $rep->MultiCell(160, 20, "Cheque Date:".sql2date($from_trans['cheque_date']) ,0, 'R', 0, 2, 370,150, true);
    }
    $rep->MultiCell(160, 20, "Journal Ref:   ". $from_trans["ref"] ,0, 'L', 0, 2, 50,110, true);
    $rep->MultiCell(200, 20, get_account_chart_master_reciept($from_trans),0, 'C', 0, 2, 180,630, true);
    $rep->MultiCell(200, 20, "Pay to:  ".$from_trans['name'] .$from_trans['supp_name'] ,0, 'L', 0, 2, 50,140, true);
    // $rep->MultiCell(300, 20, "".$from_trans['branch_ref'] .$from_trans['supp_name'] ,0, 'L', 0, 2, 50,150, true);
    $rep->MultiCell(300, 20, "Branch Name: ".get_branch_name_od($from_trans['branch_code']),0, 'L', 0, 2, 50,160, true);
...
//    $rep->MultiCell(170, 30, $trans_no."++++".$type ,0, 'L', 0, 2, 90,199, true);
    ...
    // display_error($myrow1["person_type_id"]);
        ...
        $rep->TextCollines(1, 2,	get_account_chart_master_($myrow1['account'])." : ".$counterpartyname, -2);
                        $rep->NewLine(-1);
...
//        alt_table_row_color($k);
   ...
   /* if($from_trans['type']==22){
    ...
    $rep->MultiCell(525, 15,"",1, 'L', 0, 2, 40,566, true);
    $rep->MultiCell(48, 15,"",1, 'L', 0, 2, 40,566, true);
    $rep->MultiCell(132, 15,"",1, 'L', 0, 2, 88,566, true);
    $rep->MultiCell(198, 15,"TOTAL   ",1, 'R', 0, 2, 220,566, true);
    $rep->MultiCell(62, 15,"",1, 'L', 0, 2, 418,566, true);
    $rep->MultiCell(40, 15,"".number_format2($dr),1, 'R', 0, 2, 479.5,566, true);
    $rep->MultiCell(45, 15," ".number_format2($dr),1, 'R', 0, 2, 520,566, true);
 global  $db_connections;
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL' || $db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL2'){
        $rep->MultiCell(350, 20,"Amount in Words:" ."  ".convert_number_to_words(-$bank_amount) ." "."Only",1, 'L', 0, 2, 60,590, true);
        $rep->MultiCell(120, 20, number_format2(abs(-$bank_amount)),1, 'C', 0, 2, 420,590, true);
    } else {
        $rep->MultiCell(370, 20,"Amount in Words:" ."  ".convert_number_to_words($dr) ." "."Only",1, 'L', 0, 2, 40,590, true);
        $rep->MultiCell(120, 20, number_format2(abs($dr)),1, 'R', 0, 2, 410,590, true);
    }

    $rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 40,755, true);
    $rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 240,755, true);
//    $rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 300,755, true);
    $rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 430,755, true);
    $rep->MultiCell(100, 25, "Prepared By" ,0, 'C', 0, 2, 40,770, true);
    $rep->MultiCell(100, 25, "Approved  By" ,0, 'C', 0, 2, 240,770, true);
//    $rep->MultiCell(100, 25, "Checked  By" ,0, 'C', 0, 2, 300,770, true);
    $rep->MultiCell(100, 25, "Received  By" ,0, 'C', 0, 2, 430,770, true);
    $rep->MultiCell(100, 25, "Rs." ,0, 'C', 0, 2, 370,590, true);
   ...
   //  $rep->MultiCell(420, 20, $from_trans['type']."", 0, 'L', 0, 2, 50, 40, true);
      
    $rep->setfontsize(+19);
                if($type==1 ) {
//$rep->setfontsize(+5);
$rep->SetFont('Arial','',12);
        $rep->MultiCell(420, 20, "BANK PAYMENT VOUCHER", 0, 'L', 0, 2, 50, 40, true);
         $rep->SetFont('','','');
         $rep->setfontsize(+8);
        $rep->MultiCell(100, 5, "Doc #: ST-FR-50" , 0, 'L', 0, 2, 50,78, true);
        $rep->setfontsize(-8);
       

    }
    else{
         $rep->MultiCell(420, 20, "CASH PAYMENT VOUCHER", 0, 'L', 0, 2, 50, 40, true);
        $rep->setfontsize(10);
        $rep->setfontsize(+8);
                $rep->MultiCell(100, 5, "Doc #: ST-FR-24" , 0, 'L', 0, 2, 50,78, true);
 $rep->setfontsize(-8);
