...
function print_deliveries()
{
	...
	$cols = array(2, 20, 135, 170, 205, 235, 270, 305, 350, 402, 452, 505);
	$cols2 = array(2, 20, 135, 170, 205, 235, 270, 305, 350, 402, 452, 505);
	// $headers in doctext.inc
	$aligns = array('center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'right');
	$aligns2 = array('center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'right');
	...
	for ($i = $from; $i <= $to; $i++)
	{
			...
			while ($myrow2=db_fetch($result))
			{
	    		...
	    		$AddTax = number_format2($myrow2["unit_tax"] ,$dec);
				$IncludTxprice = number_format2(($myrow2["unit_price"] + $AddTax) ,$dec);
				$Total = number_format2($myrow2["quantity"] * ($myrow2["unit_price"] + $AddTax) ,$dec);
				$TotalQty += ($myrow2["quantity"]); 
				$NewSubTotal += ($myrow2["quantity"] * ($myrow2["unit_price"] + $AddTax));
...
//		  			$DisplayDiscount = number_format2($myrow2["discount_percent"]*100,user_percent_dec()) . "%";
				...
				$rep->TextCol(1, 2, get_category_name($myrow2['category_id']), -2);
				$rep->TextCol(2, 3, $myrow2['stock_id'], -2);
				$rep->TextCol(3, 4, $myrow2["Thickness"]	, -2);
				$rep->TextCol(4, 5, $myrow2["Width"]	, -2);
				$rep->TextCol(5, 6, $myrow2["Length"]	, -2);
				$rep->TextCol(6, 7, $DisplayQty	, -2);
				$rep->TextCol(7, 8, sql2date($myrow["tran_date"]));
				$rep->TextCol(8, 9, $DisplayPrice , -2);
				$rep->TextCol(9, 10, $AddTax , -2);
				$rep->TextCol(10, 11, $IncludTxprice , -2);
				$rep->TextCol(11, 12, $Total , -2);
				$rep->NewLine();
...
//				$rep->row = $newrow;
		   ...
		   $rep->Font('bold');
           $rep->NewLine(+13.5);
           $rep->TextCol(1, 1, "Total", -2);
           $rep->TextCol(6, 7, $TotalQty, -2);
           $rep->TextCol(11, 12, $NewSubTotal, -2);
           $rep->Font('');
           $rep->NewLine(-2);
