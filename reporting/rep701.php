...
function display_type ($type, $typename, &$dec, &$rep, $showbalance, $level,$inactive)
{
	$printtitle = 0; //Flag for printing type name	

	//Get Accounts directly under this group/type
	$result = get_gl_accounts(null, null, $type, $inactive);	
	...
	while ($accounttype=db_fetch($result))
	{
		...
		$nextlevel = $level + 1;
		display_type($accounttype["id"], $accounttype["name"].' ('.$typename.')', $dec, $rep, $showbalance, $nextlevel, $inactive);
	}
}
...
function print_Chart_of_Accounts()
{
	...
	while ($class = db_fetch($classresult))
	{
		...
		{
			display_type($accounttype["id"], $accounttype["name"], $dec, $rep, $showbalance, 0, $inactive);
		}
		$rep->NewLine();
	}
	$rep->Line($rep->row + 10);
