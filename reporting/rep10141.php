...
//    	$dec = user_price_dec();
	...
	$cols = array(0,25, 220, 260, 330,	400, 470, 540, 610, 680, 750);
	$headers = array(_('S.'), _('Customer Name '), _('Opening '), _('Sales'), _('Sales'),
		_('Customer'),_('Customer'),_('Credit'),_('Journal'),_('Balance'));
	$headers2 = array(_('No'), _(''), _('Balance'), _('Invoices'), _('Return'), _('Receipts'),_('Refunds'),_('Note'),_('Entries'),
		_(''));
...
global  $db_connections;
  ...
  if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'VETZ' ) {
        ...
        if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'VETZ' ) {
            $init[1] = round2(($credit_note), $dec);
        }
        else{
            $init[1] = round2(($bal2['credit_note_tax_incl'] + $bal2['credit_note']), $dec);
		}
...
// 			if ($trans['type'] == ST_SALESINVOICE )
		...
		if($debtor_no != $myrow['debtor_no'])
		{
			...
			if (!$destination)
			{
			$str = $myrow['name'] ." - ". $myrow['debtor_ref'];
			if (strlen($str) > 48)
            $str = substr($str, 0, 45).'...';
		//	$rep->TextCol(1, 2, $str);
			    }
...
// 			$rep->Newline();
	...
	$rep->TextCol(1, 2, $str);
		...
		$rep->AmountCol(4, 5, $init[2], $dec); //sales return
		$rep->AmountCol(5, 6, $init[4], $dec); //customer receipts
		$rep->AmountCol(6, 7, $init[3], $dec); //customer refunds
		$rep->AmountCol(7, 8, $init[1], $dec); //credit notes
		$rep->AmountCol(8, 9, $init[5], $dec); //journal
...
$net_sales = $gt_sales - $gt_cr_notes;
		...
		$rep->AmountCol(4, 5, $gt_cr_notes, $dec);
		$rep->AmountCol(5, 6, $gt_receipts, $dec);		
		$rep->AmountCol(6, 7, $gt_payments, $dec);
		$rep->AmountCol(7, 8, $gt_sales_return, $dec);
		
		$rep->AmountCol(8, 9, $gt_journal, $dec);
		
		$rep->AmountCol(9, 10, ($total_cust_balance), $dec);


	$rep->NewLine(1.3);
		$rep->TextCol(3, 4, _("Net Sales"), $dec);    	
		$rep->AmountCol(4, 5, $net_sales, $dec);    	
	if($gt_gst>0)	
	{
	$rep->NewLine(1);
		$rep->TextCol(3, 4, _("Sales Tax"), $dec);    	
		$rep->AmountCol(4, 5, $gt_gst , $dec); 		
	$rep->NewLine(1);
		$rep->TextCol(3, 4, _("Sales Excl. Tax"), $dec);    	
		$rep->AmountCol(4, 5, $net_sales-$gt_gst , $dec); 		
	}
