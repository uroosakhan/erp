...
//----------------------------------------------------------------------------------------------------
function get_imei_no($order_no,$stock_id)
{
	$sql = "SELECT * FROM ".TB_PREF."sales_order_details WHERE 
...
function get_customer_detailSSs($customer_id, $to=null, $all=true)
{
...
function print_invoices()
{
	...
	$cols = array(8, 30, 95, 120, 160, 200,220);
...
//		$rep->fontSize += 2;
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$imei=get_imei_no($myrow2['debtor_trans_no'],$myrow2['stock_id']);
			...
			$rep->NewLine(0.20);
			$rep->TextCol(0, 3, "IMEI: ".$myrow2['text1'], -2);
			$rep->fontSize += 1;
			$newrow = $rep->row;
			$rep->row = $oldrow;
...
//AND type IN (11,12,2)
		...
		$rep->Font('b');
		//$rep->fontSize += 1;
	 $customer_record = get_customer_detailsSS($myrow[debtor_no], $to=null, $all=true);
         $total_balance=$customer_record["Balance"];
		$rep->TextCol(0, 3, _("Closing Balance:"));
			$rep->TextCol(4, 5, number_format2($customer_record["Balance"]));
		//$rep->fontSize -= 1;
		$rep->Font();
		$rep->Font('b');
		$rep->NewLine(4);
		$rep->TextCol(0, 4, _(" Signature ....................................."), -2);
