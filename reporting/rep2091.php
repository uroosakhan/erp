...
{
    $sql = "SELECT ".TB_PREF."purch_order_details.*, units, ".TB_PREF."stock_master.description as desc_
...
//                $rep->TextColLines(1, 2,	$myrow2['description'], -2);
         ...
         $TOTAL_RATE +=$rate;
            ...
            $rep->TextCollines(1, 2, $myrow2['desc_'], -2);
//            $rep->NewLine(1);
//            $rep->NewLine(+3);
...
//                $rep->TextCol(6, 7,	$DisplayTax, -2);
        ...
        $rep->MultiCell(100, 25, "".get_user_name_701231($user) ,0, 'C', 0, 2, 40,758, true);
    ...
    $rep->multicell(80, 5, " TOTAL AMOUNT", 0, 'L', 0, 0, 100, 690);
    $rep->multicell(85, 5, price_format($TOTAL_QTY), 0, 'R', 0, 0, 270, 690);

    if ($myrow['tax_group_id'] == 1)
    {
        // $rep->multicell(90, 5, price_format($TOTAL_RATE), 0, 'R', 0, 0, 390, 690);
        $rep->multicell(65, 5, price_format($grosstot), 0, 'R', 0, 0, 500, 690);
        $amount_in_words = _number_to_words($grosstot);
        $rep->multicell(400, 15, " " . $amount_in_words, 0, 'L', 0, 0, 125, 718);
    }
    else
    {
     $rep->multicell(65, 5, price_format($grosstot), 0, 'R', 0, 0, 495, 690);
     $amount_in_words = _number_to_words($grosstot);
     $rep->multicell(400, 15, " " . $amount_in_words, 0, 'L', 0, 0, 125, 718);
    }
        $rep->Font('');
