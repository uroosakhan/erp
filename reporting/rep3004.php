...
function getTransactions($from, $to,$category=null, $location=null,$items)
{
    $from = date2sql($from);
    $to = date2sql($to);
    ...
    $sql .=" GROUP BY d.`debtor_trans_no`";
...
//        4 => array('text' => _('Customer'), 'from' => $fromc, 'to' => ''),
    ...
    $rep = new FrontReport(_('Inventory Sales And Return Report Sceheme Wise'), "InventorySalesReport", user_pagesize(), 8.5, $orientation);
    if ($orientation == 'L')
    ...
    $sql ="SELECT DISTINCT(d.stock_id) FROM 0_debtor_trans_details d , 0_debtor_trans dt Where d.debtor_trans_type=11
            AND dt.trans_no= d.debtor_trans_no AND dt.ov_amount >0 
            AND dt.tran_date>=".db_escape(date2sql($from))." 
            AND dt.tran_date<=".db_escape(date2sql($to))." ";

    if($items!=null)
    $sql.=" AND stock_id=".$items;

    $result = db_query($sql,'No Item Found');
    ...
    while($item=db_fetch($result))
    {
        ...
        $res = getTransactions($from, $to,$category, $location,$item['stock_id']);
         if(db_num_rows($res)== 0) continue;

        $name = get_item_name($item['stock_id']);
        $rep->TextCol(0, 1,$name);
        $rep->newline();
        while ($trans=db_fetch($res))
        {
//            $checker = check_if_cust_delivery($item['stock_id'],$trans['debtor_trans_no']);
//            display_error(db_num_rows($checker));
...
//            $rep->NewLine();
            ...
            $rep->NewLine();
            if(db_num_rows($cre_note_id)== 0) continue;
            $rep->NewLine(-1);
...
//    $rep->Line($rep->row - 16);
    ...
    $rep->AmountCol(5,6, "0".$total_sceheme_value2);
    //Grand total item_detail
    ...
    $rep->NewLine(2);
    $rep->TextCol(11,12,'Total Trade Value : ');
    $rep->AmountCol(12,14,$total_trade_value2);
    $rep->NewLine();
    $rep->TextCol(11,12,'Total Return Value : ');
    $rep->AmountCol(12,14,$grand_total_returned_values);
