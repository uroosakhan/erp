...
$pref = get_company_prefs();
	...
	$cols = array(4, 50, 250, 220, 260, 320, 370, 420,440, 515);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'right', 'left','left', 'left', 'right', 'right', 'right', 'right');
     }
     else{
         	$cols = array(4, 50, 250, 300, 340, 380, 420, 450, 515);
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$SubTotal += $Net;
			$SubTotal2 += (1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $qty;
...
//                         $rep->Font('bold');
  ...
  $rep->MultiCell(410, 30, "Discount" ,0, 'L', 0, 2, 260,660, true);
		}
		
		if($myrow['discount2'] != 0) {

 $discount_value =$myrow["disc2"]/100;
                    $rep->MultiCell(65, 10, "".price_format($myrow["discount2"]) ,0, 'R', 0, 2, 495,658, true);
                    $rep->MultiCell(410, 30, "Discount" ,0, 'L', 0, 2, 340,658, true);
		 ...
		 $rep->Font('');
            ...
            $DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal - $myrow["discount1"] - $myrow["discount2"] + $DisplayTax , $dec);
            $rep->Font('bold');
            ...
            $DisplayTotal2 = number_format2($myrow["freight_cost"] + $SubTotal2 - $myrow["discount1"] - $myrow["discount2"] + $tax , $dec);
            $rep->Font('bold');
            if ($pref['alt_uom'] == 1 && $myrow3['sale_enable'] == 1) {
                ...
                $rep->TextCol(3, 7, _("TOTAL ORDER GST INCL.") . ' ' . $rep->formData['curr_code'], -2);
                $rep->TextCol(7, 8, $DisplayTotal2   , -2);
            }
        }
