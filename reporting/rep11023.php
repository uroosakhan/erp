...
function get_delivery($id)
{
    ...
    WHERE type = 13 AND trans_no = ".db_escape($id);
    $query = db_query($sql, "Cannot get dimensions");
    return db_fetch($query);
...
function get_delivery_details($id)
{
    ...
    WHERE debtor_trans_type = 13 AND debtor_trans_no = ".db_escape($id)."
    ";
    $query= db_query($sql, "Cannot get dimensions");
    return $query;
...
function print_po()
{
    ...
    $cols = array(4, 30, 80, 320, 430,500);
	...
	for ($i = $from; $i <= $to; $i++)
	{
        ...
        $myrow = get_delivery($i);
        $salesorder = get_sales_order_header($myrow['order_'], ST_SALESORDER);
		...
		$rep->currency = $cur;
		$rep->Font();
        $rep->Info($params, $cols, null, $aligns, null);

		$contacts = get_customer_contacts($myrow['debtor_no'], 'order');
		$rep->SetCommonData($myrow, null, $salesorder, $baccount, ST_PURCHORDER, $contacts);
		$rep->NewPage();
...
//                $purpose = get_purpose($myrow2['po_detail_item']);
              ...
              $rep->TextCol(1, 2, $myrow2['stock_id'], -2);
                ...
                // $rep->NewLine(-1);
                 $rep->TextCol(3, 5,	 $myrow2['text3'], -2);
                   $rep->TextCol(4, 5,	 $myrow2['text2'], -2);
                // $rep->NewLine(-1);
                //  $rep->TextCollines(5, 6,	 $batch['name'], -2);
                // $rep->NewLine(-1);
                //  $rep->TextCollines(6, 7,	 $batch['exp_date'], -2);
...
//                }
                ...
                $rep->TextColLines(2, 3,	$myrow2['description']."/".$item['long_description'], -2);
            }
...
//                $SubTotal += $tax_item['Value'];
        ...
        //  $rep->MultiCell(70, 15,"Total", 0, 'L', 0, 2, 120,617, true);
      //  $rep->AmountCol(4, 5, $TotalGrossAmount, $dec);
       ...
       // $rep->AmountCol(7, 8,$total_sales_tax_other, $dec);
        ...
        //  $rep->MultiCell(70, 15,$DisplayTotal, 0, 'R', 0, 2, 470,617, true);
        $rep->NewLine(2);
...
//     $rep->MultiCell(250, 10, "".$myrow['DebtorName'], 0, 'L', 0, 2, 55,170, true);
// $rep->MultiCell(200, 10, "".$myrow['address'], 0, 'L', 0, 2, 55,200, true);


//   $rep->MultiCell(105, 30, "".$tax_type_name." ".$DisplayTax, 1, 'L', 0, 2, 457,640, true);
//     $rep->MultiCell(105, 30, "".$DisplayTotal.$myrow['curr_code'] , 1, 'L', 0, 2, 457,673, true);

    $rep->Font('');
    // $reference = get_dn_no_11025($myrow['trans_no']);
     $rep->MultiCell(225, 60, "", 0, 'L', 0, 2, 40,685, true);
    $rep->MultiCell(515, 30, "Molecular Biology Product is not responsible for any damge or theft of products after delivery to recepient. Complaints, if any, should be notified to us within 7 business days"  , 1, 'L', 0, 2, 43,710, true);
