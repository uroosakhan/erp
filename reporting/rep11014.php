...
function print_deliveries()
{
	...
	$cols = array(4, 35, 90, 150, 260, 310,360);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'left', 'left', 'left');
	$headers = array(_("S.NO"),_("PO Position"), _("Material No"), _("Material Description"),
		_("UOM"), _("PO Qty"), _("Delivery Qty"));
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$DisplayQty_done = number_format2($myrow2["quantity"],get_qty_dec($myrow2['stock_id']));
...
//                $item=get_item($myrow2['stk_code']);
			...
			//	display_error($myrow["order_"]."---".$myrow2["stock_id"]);
				$rep->TextCol(5, 6,get_quantity_report($myrow2["stock_id"],$myrow["order_"])	, -2);
				$rep->TextCol(6, 7,$DisplayQty_done	, -2);
...
//							$rep->TextCol(3, 7, _("Included") . " " . $tax_type_name . _("Amount") . ": " . $DisplayTax, -2);
		...
		$rep->MultiCell(770, 30,"Contact Person:", 0, 'L', 0,1,50,135, true);
		$rep->MultiCell(770, 30,"". $rep->formData['customer_ref'], 0, 'L', 0,1,120,135, true);

		$rep->MultiCell(770, 30,"Phone:", 0, 'L', 0,1,50,150, true);
		$rep->MultiCell(150, 10,"". $sales_order['contact_phone'], 0, 'L', 0,1,120,150, true);


		$rep->MultiCell(770, 30,"Delievery Terms:", 0, 'L', 0,1,50,165, true);
		$rep->MultiCell(770, 30,"".$sales_order['f_text3'], 0, 'L', 0,1,120,165, true);

		$rep->MultiCell(770, 30,"Phone:", 0, 'L', 0,1,50,120, true);
		$rep->MultiCell(150, 10,"". $customer_imformation['phone'], 0, 'L', 0,1,120,120, true);

		$rep->MultiCell(770, 30, "Customer PO No.", 0, 'L', 0,1,359,110, true);
		$rep->MultiCell(770, 30, "Customer PO Date.", 0, 'L', 0,1,359,120, true);
		
// 		$rep->MultiCell(770, 30, $rep->formData['reference'], 0, 'L', 0,1,439,110, true);
// 		$rep->MultiCell(770, 30, sql2date($rep->formData['ord_date']), 0, 'L', 0,1,439,120, true);
...
//		$rep->MultiCell(770, 30, get_location_reports($rep->formData['from_stk_loc']), 0, 'L', 0,1,406,122, true);
		$rep->MultiCell(770, 30, "Sales Order No.", 0, 'L', 0,1,359,134, true);
		$rep->MultiCell(770, 30, "Sales Order Date.", 0, 'L', 0,1,359,144, true);
		$rep->MultiCell(770, 30, $sales_order['reference'], 0, 'L', 0,1,439,134, true);
		$rep->MultiCell(770, 30, sql2date($sales_order['ord_date']), 0, 'L', 0,1,439,144, true);
		//$rep->MultiCell(770, 30, "__________", 0, 'L', 0,1,400,110, true);
		
		$rep->MultiCell(770, 30, $rep->formData['f_text2'], 0, 'L', 0,1,439,110, true);
		$rep->MultiCell(770, 30, sql2date($rep->formData['po_date']), 0, 'L', 0,1,439,120, true);
		
// 		$rep->MultiCell(770, 30,$rep->formData['customer_ref'], 0, 'L', 0,1,439,110, true);
// 		$rep->MultiCell(770, 30, sql2date($rep->formData['tran_date']), 0, 'L', 0,1,439,120, true);

		$rep->MultiCell(500, 30, "Goods received as per above description and quantity in acceptable condition", 0, 'L', 0,1,40,700, true);
		$rep->MultiCell(500, 30, "We are pleased to deliver following items as per your purchase order. Invoice will be submitted to you separately", 0, 'L', 0,1,40,220, true);

		$rep->MultiCell(770, 30, "Prepared By", 0, 'L', 0,1,40,790, true);
...
// 		$rep->MultiCell(770, 30, "________________________", 0, 'L', 0,1,410,750, true);
		$rep->MultiCell(770, 30, "Approved By", 0, 'L', 0,1,357,790, true);
		$rep->MultiCell(770, 30, "Project Manager", 0, 'L', 0,1,200,790, true);

		$rep->MultiCell(69, 30, "Recived By:  (with date, time and company stamp)", 0, 'L', 0,1,480,790, true);
