...
function print_grn_valuation()
{
	...
	$cols = array(0, 75, 190, 225, 260, 295, 330, 370, 410, 455, 515);
	$headers = array(_('Stock ID'), _('Description'), _('PO No'), _('GRN')."#", _('Inv')."#", _('Qty'), _('Inv Price'), _('PO Price'), _('Std. Cost'), _('Total'));
		...
		if ($trans['quantity_inv'])
		{
			...
			{	
                ...
                if(!user_check_access('SA_ITEMSPRICES')) {
                    ...
                    $rep->AmountCol(8, 9, $trans['std_cost_unit'], $dec);

                    $amt = round2($inv['inv_qty'] * $inv['inv_price'], $dec);
                    $rep->AmountCol(9, 10, $amt, $dec);
                }
				$rep->NewLine();
				$total += $amt;
				$qtotal += $inv['inv_qty'];
	...
	if ($stock_id != '')
	{
        ...
        if(!user_check_access('SA_ITEMSPRICES')) {
            $rep->AmountCol(9, 10, $grandtotal, $dec);
        }
	}
