...
if ($divison == ALL_TEXT)
  ...
  if ($month == ALL_TEXT)
	...
	$cols = array(10, 100, 300, 430, 520);

	$headers = array(_('Name'), _('Project'), _('Designation'), _('Project Salary'), _('Total Salary'));
...
//			);
$orientation = 'L';
    $rep = new FrontReport(_('                                                 Payroll Cheque List For The Month Of June 2017'), "ManMonthReport", user_pagesize(), 10, $orientation);
    if ($orientation == 'L')
    	recalculate_cols($cols);

			$rep->SetHeaderType('Header17');
...
//		$NetSalary = $myrow['basic_salary'] - $myrow['advance_deduction'] - $myrow['late_deduction'] - $myrow['tax'] + $myrow['Overtime'] ;
        ...
        {
        $rep->TextCol(0, 1, $empp['emp_name']);
        $rep->TextCol (1, 2, $emppp['name']);
        $rep->TextCol (2, 3, get_designation_names($empp['emp_desig']));
        $rep->TextCol(3, 4,$myrow2['project_wise_salary'] );
