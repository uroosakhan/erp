...
function print_invoices()
{
    ...
    $to = max($fno[0], $tno[0]);
     ...
     $cols = array(8, 30, 100, 120, 160, 160, 230);
   // $cols = array(8, 30, 65, 90, 120, 145, 400);
    // $headers in doctext.inc
    $headers = array( _("Item"), _(""), _("Qty"), _("Price"), _(""), _("Total"."           "));
...
$DisplaySubTot=0;
        while ($myrow2=db_fetch($result))
        {
            ...
            // $Net = number_format2($sign * ((1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $myrow2["quantity"]),
            //     user_price_dec());
            $item = get_item($myrow2['stock_id']);
            
            if($item['units'] != $myrow2['units_id'] ){
                if($item['con_factor']!=0){
...
else
{
  ...
  $items = get_item($myrow2['stock_id']);
if($items['units'] != $myrow2['units_id'] ){
    
    
	    		$DisplayQty = number_format2($sign*$myrow2["quantity"] *  $item['con_factor'],get_qty_dec($myrow2['stock_id']));
	    		  }
	    		  else{
...
//                $rep->TextCol(4, 5,	$DisplayDiscount, -2);
                $rep->TextCol(5, 6,	number_format2($DisplayNet)."           ", -2);
...
//        $rep->NewLine();
        ...
        if($sales_order["total_discount_pos"]) {
            $rep->TextCol(2, 3, "- ".$total_disc_name);
        ...
        if($bank_data['account_type'] == 3)
        {
            ...
            $rep->TextCol(5, 6, $sales_order['h_text1'] . "         ", -2);
            $rep->fontSize -= 1;
            $rep->NewLine(1.5);
            $rep->Font('bold');
            $rep->fontSize += 1;
            $rep->TextCol(0, 4, _("Cash Returned"), -2);
            $rep->TextCol(5, 6, $sales_order['h_text2'] . "         ", -2);
            $rep->fontSize -= 1;
        }
        ...
        if($message != "")
        {
            $rep->NewLine(2.5);
            $rep->Font('bold');
            $rep->TextColLines(0, 5, $message, - 2);
            $rep->Font('');
            $rep->NewLine(-2);
        }
        ...
        $rep->NewLine(3);
        $rep->Font('bold');
        $rep->TextCol(0, 6, "Wifi:                         ".$rep->company['domicile']);
        $rep->NewLine();
        $rep->TextCol(0, 6, "Phone#:                                   ".$rep->company['phone']);
        $rep->NewLine();
        $rep->TextCol(0, 6,"Website:                ".$rep->company['website']);
        $rep->NewLine();
        $rep->TextCol(0, 6, "Email:                  ".$rep->company['email']);
        $rep->NewLine(2);
