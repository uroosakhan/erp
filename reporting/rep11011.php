...
function get_itemcode($stock_id)
{
...
function print_deliveries()
{
	...
	$cols = array(4, 50, 105, 235,275,310,380,470);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'left', 'left', 'left', 'left');
	$headers = array(_("Spar Art#"),_("Bar Code #"), _("Article Description"), _("Count"),
		_("UOM"),_("Order Quantity"), _("Delivered Quantity"), _("Remarks"));
	...
	for ($i = $from; $i <= $to; $i++)
	{
			...
			while ($myrow2=db_fetch($result))
			{
				...
				$rep->TextCol(1, 2,	get_itemcode($myrow2['stock_id']), -2);
				$oldrow = $rep->row;
...
//                $item=get_item($myrow2['stk_code']);
					...
					$rep->TextCol(6, 7, $myrow2['qty_done'], -2);
...
//		$rep->MultiCell(770, 30,"". $customer_imformation['phone'], 0, 'L', 0,1,98,150, true);
		...
		$rep->MultiCell(770, 30, "ORDER NO :", 0, 'L', 0,1,359,147, true);
		$rep->MultiCell(770, 30, "ORDER DATE :", 0, 'L', 0,1,359,160, true);
		$rep->MultiCell(770, 30, "".$sales_order['ord_date'], 0, 'L', 0,1,434,160, true);
			$rep->MultiCell(770, 30, "Supplier Name".$sales_order['h_text6'], 0, 'L', 0,1,359,134, true);
...
//		$rep->MultiCell(770, 30, "dsfsdf".($rep->formData['customer_ref']), 0, 'L', 0,1,406,134, true);
		//$rep->MultiCell(770, 30, "__________", 0, 'L', 0,1,400,110, true);
//		$rep->MultiCell(770, 30, $sales_order['reference'], 0, 'L', 0,1,413,134, true);
       
//       $rep->MultiCell(500, 30, "", 0, 'L', 0,1,40,200, true);
       
		$rep->MultiCell(770, 30, "Prepared By:", 0, 'L', 0,1,40,400, true);
		$rep->MultiCell(770, 30, "SPAR Seal:", 0, 'L', 0,1,380,400, true);
		$rep->MultiCell(770, 30, "Supplier Seal:", 0, 'L', 0,1,230,400, true);

		$rep->MultiCell(770, 30, "________________________", 0, 'L', 0,1,100,400, true);
		$rep->MultiCell(770, 30, "________________________", 0, 'L', 0,1,100,450, true);
		$rep->MultiCell(770, 30, "Recieved By:", 0, 'L', 0,1,40,450, true);
		$rep->MultiCell(770, 30, "GDN #:", 0, 'L', 0,1,380,450, true);
//		$rep->MultiCell(770, 30, "", 0, 'L', 0,0,380,450, true);
//		$rep->MultiCell(50, 514, "", 1, 'L', 0,1,40,178, true);
//		$rep->MultiCell(120, 514, "", 1, 'L', 0,1,330,178, true);
		
		$rep->MultiCell(545, 500, "", 1, 'L', 0,1,35,30, true);
