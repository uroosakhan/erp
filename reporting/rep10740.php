...
function print_invoices()
{
	...
	$cols = array(4, 30,80, 200, 225,260, 285, 317, 375, 408, 473);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'left', 'left', 'left', 'center', 'right','right', 'right');
...
//                $item=get_item($myrow2['stk_code']);
					...
					$rep->TextCol(9, 10,	$disc_amount."       " , -2);
					$net_amount = $trade_value - $myrow2["discount_percent"];
					$rep->TextCol(10, 11,$net_amount	  , -2);
                ...
                $rep->MultiCell(410, 30, $sales_order['h_text2'] ,0, 'L', 0, 2, 423,161, true);
	$rep->MultiCell(410, 30, $sales_order['h_text4'] ,0, 'L', 0, 2, 423,148, true);
	$rep->MultiCell(410, 30, $sales_order['h_text3'] ,0, 'L', 0, 2, 423,175, true);
		$rep->MultiCell(410, 30, sql2date($sales_order['po_date']) ,0, 'L', 0, 2, 423,135, true);
			$rep->MultiCell(410, 30, $sales_order['h_text5'] ,0, 'L', 0, 2, 423,120, true);
				$rep->row = $newrow;
//				$rep->NewLine(1);
...
//        $rep->multicell(50,35,"Closing Bal:",0,'l',0,0,415,727);
         $rep->font('b');
         				     $rep->MultiCell(410, 30, "Contact #" ,0, 'L', 0, 2, 344,161, true);
 $rep->MultiCell(410, 30, "Sales Officer #" ,0, 'L', 0, 2, 344,148, true);
 $rep->MultiCell(410, 30, "Days Limit" ,0, 'L', 0, 2, 344,175, true);
        // $rep->multicell(200,10,"Opening Balance:",0,'L',0,0,344,115);
        $rep->multicell(200,10,"P.O No:",0,'L',0,0,344,120);
        $rep->multicell(200,10,"P.O Date:",0,'L',0,0,344,135);
        $rep->font('');
		...
		$rep->MultiCell(300,19, $sales_order['contact_phone'],0,'L',0,0,230,175);
...
//        $rep->multicell(170,20," ".get_user_name_1($prepared_by),1,'L',0,0,115,650);
		...
		$rep->NewLine(-15);
//		$rep->TextCol(7, 9, $rep->formData['prepaid'] ? _("Transportation Exp 2.0%.") : _("Net Bill Amount")/*. ' ' . $rep->formData['curr_code']*/, - 2);
		 $rep->MultiCell(410, 30, "Transportation Exp 2.0%" ,0, 'L', 0, 2, 349,417, true);
 $rep->MultiCell(410, 30, "Grand Total:" ,0, 'L', 0, 2, 130,380, true);
 	 $rep->MultiCell(410, 30, "Damage & Expiry Exp 0.5%" ,0, 'L', 0, 2, 349,434, true);
 	 $rep->MultiCell(410, 30, "Net Bill Amount" ,0, 'L', 0, 2, 349,453, true);
//		$rep->TextCol(9, 11, $net_amount1, -2);
        $net_amount_2 = $net_amount1 * 0.02 ;
        $net_amount_3 = $net_amount1 * 0.05 ;
$net_bill = $net_amount_3 - $net_amount_2;
        $rep->MultiCell(410, 30, "".$net_amount_3 ,0, 'L', 0, 2, 550,434, true);
        $rep->MultiCell(410, 30, "".$net_amount_2 ,0, 'L', 0, 2, 550,417, true);
        $rep->MultiCell(410, 30, "".$net_bill ,0, 'L', 0, 2, 550,452, true);

//		$amount_in_words = convert_number_to_words($DisplayTotal);
		$rep->MultiCell(400,15,"Rupees ".convert_number_to_words($net_amount1)." Only",0,'L',0,0,50,400);
			if ($rep->formData['prepaid'])
			{
				$rep->NewLine();
				$rep->Font('bold');
...
//				$rep->TextCol(1, 7, $myrow['curr_code'] . ": " . $words, - 2);
			...
			if ($email == 1)
			{
				...
				//	$net_amount1  += $net_amount;
				$rep->NewLine(-6.3);
					$rep->TextCol(7,8, $tottrade, -2);
						$rep->TextCol(5, 6, $myrow2['units'], -2);
						$rep->TextCol(10,11, $net_amount1, -2);
//    			$rep->TextCol(3, 4, $totqty."   ", -2);
        $rep->MultiCell(410, 30, $totqty ,0, 'L', 0, 2, 260,380, true);



	}
	$rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 50,560, true);
	   $rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 250,560, true);
    $rep->MultiCell(150, 25, "___________________________" ,0, 'L', 0, 2, 430,560, true);
    $rep->MultiCell(100, 25, "Prepared By" ,0, 'C', 0, 2, 40,570, true);
    $rep->MultiCell(100, 25, "Verified  By" ,0, 'C', 0, 2, 250,570, true);
    $rep->MultiCell(200, 25, "Receivers Signature & Stamp" ,0, 'C', 0, 2, 390,570, true);
    $rep->MultiCell(200, 25, "Software Develop By : Hisaab.pk" ,0, 'C', 0, 2, 390,600, true);
    $rep->MultiCell(200, 25, "+92 300 8296954 , Info@dys-solution.com" ,0, 'C', 0, 2, 390,610, true);
	if ($email == 0)
		$rep->End();
}
