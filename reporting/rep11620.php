...
//    $cols = array(5, 45, 190, 250, 300, 350, 400, 450, 470, 550);
    $cols = array(5, 250, 300, 360,400, 450, 520);
...
//        $rep->Font();
        ...
        $rep->Font('bold');
        $rep->MultiCell(75 ,20 ,"Signature:",0,'L', 0, 2,50,740,true);
        $rep->MultiCell(80 ,20 ,"..........................",0,'L', 0, 2,120,740,true);

        $rep->MultiCell(75 ,20 ,"Desigination:",0,'L', 0, 2,50,760,true);
        $rep->MultiCell(75 ,20 ,"Cheif Executive",0,'L', 0, 2,120,760,true);


        $rep->MultiCell(75 ,20 ,"Place:",0,'L', 0, 2,50,780,true);
        $rep->MultiCell(75 ,20 ,"Karachi",0,'L', 0, 2,120,780,true);
        $rep->Font();

        $rep->MultiCell(250,10,"Note:",0,'L', 0, 2,45,815,true);
        $rep->MultiCell(300,40," ----".$order_no['term_cond'],0,'L', 0, 2,70,817,true);
        $total_price =0;
...
//            $rep->TextCol(0, 1,	$s_no, -2);
            ...
            $rep->TextCol(1,2, $myrow2['batch'], -2);


            $rep->TextCol(2,3, $rep->formData['h_text3'], -2);


            $rep->TextColLines(0, 1, $myrow2['StockDescription'], -2);
            $newrow = $rep->row;
            $rep->row = $oldrow;
...
//                $rep->TextCol(5, 6,	get_tax_rate($i, $myrow2['stock_id'])."%", -2);
...
//                $rep->AmountCol(6,7,	$amount_including_tax, $dec);
                ...
                $total_price += $DisplayPq;

//                display_error($total_price);
                $discount=$total_price * $myrow['ov_discount']/100;
                $grand_total=abs($total_price - $discount);



//                $rep->MultiCell(75 ,28 ,"".$discount,1,'L', 0, 2,489,617,true);
        ...
        $rep->Font('bold');
        $rep->MultiCell(75 ,43 ,''.$total_price,1,'L', 0, 2,489,574,true);
        $rep->MultiCell(75 ,20 ," ".$discount,0,'L', 0, 2,489,600,true);
        $rep->Font();
...
//        $rep->NewLine(+25);
//        $rep->Font('bold');
//        $rep->TextCol(4,6, "Grand Total"."  ".$grand_total, - 2);
//        $rep->Font('');
//        $rep->NewLine(-25);
        $rep->fontSize += 10;

        $rep->Font('italic');
        $rep->MultiCell(75 ,30 ,"".$myrow['ov_discount']."%",0,'L', 0, 2,390,594,true);
        $rep->setfontsize(+10);
        $rep->MultiCell(100 ,28 ,"Grand Total",0,'C', 0, 2,350,617,true);
        $rep->MultiCell(75 ,28 ," ".$grand_total,1,'L', 0, 2,489,617,true);
        $rep->setfontsize(+8);
        $rep->Font('');
        $rep->fontSize -= 10;
...
//        $rep->TextCol(1,3, "             Grand Total", -2);
        //$rep->TextCol(2, 3,pr ice_format($total_price), -2);
...
//        $rep->MultiCell(200,10, "Customer Reference",0,'L', 0, 2,350,180,true);
        ...
        $rep->Font('bold');
        $rep->MultiCell(800,15,"Rupees:   ".convert_number_to_words($grand_total)." Only",0,'L',0,0,45,620);
