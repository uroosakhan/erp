...
if($pref['batch'] == 1){
	...
	$cols = array(4, 60, 170, 230 , 260, 315, 355, 395, 450, 475);
	...
	$aligns = array('left',	'left',	'left',	'left',	'right', 'center', 'right', 'right', 'right');
      ...
      //-------------code-Descr-Qty--uom--tax--prc--Disc-Tot--//
	$cols = array(4, 60, 245, 280, 345, 365, 415,470, 515);
...
//                $item=get_item($myrow2['stk_code']);
				 ...
				 else
				    {
				          ...
				          $rep->TextCol(5 , 6 ,	$myrow2['bonus'], -2);
				          $total_b += $myrow2['bonus'];
...
//                $item=get_item($myrow2['stk_code']);
			...
			if (isset($prepayments))
			{
			 ...
			 //display_error($total_b);
			 //display_error($total_qty);
		...
		if($myrow['discount1']) {
			...
			$rep->Amountcol(7, 8, $myrow['discount1'], $dec);
			$rep->NewLine();
		}
		if($myrow['discount1']) {
			$rep->TextCol(3, 6, _("Discount 2"), -2);
			$rep->Amountcol(7, 8, $myrow['discount2'], $dec);
			$rep->NewLine();
		}
    ...
    // 		$rep->NewLine();
    		$total_bonus = $total_b + $total_qty;
	$rep->TextCol(3, 6, _("Total Quantity"), -2);
			$rep->TextCol(7, 8,	$total_bonus, -2);
				$rep->NewLine();
 $rep->TextCol(3, 6,  _("TOTAL INVOICE"). ' (' . $rep->formData['curr_code'].')', -2);
	...
	$pref=get_company_prefs();
			...
			$rep->MultiCell(400, 25, "Signature:__________________",0, 'L', 0, 2, 35,770, true);
			$rep->MultiCell(400, 25, "Customer Signature:__________________",0, 'L', 0, 2, 350,770, true);
