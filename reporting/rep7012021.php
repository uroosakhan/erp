...
//----------------------------------------------------------------------------------------------------
function get_user_id2($trans_no,$type)
{
...
function get_user_name_222($user_id)
{
...
function print_Chart_of_Accounts()
{
    ...
    $cols = array(2, 75, 90, 420, 420,400);
    $headers1 = array(_('Account Code'), _(''), _('Account Title '),
        _('Debit'), _('Credit'));
...
//        recalculate_cols($cols);
    ...
    $rep->Font();
    $rep->SetHeaderType('Header7012021');
...
//    if($pagesize=='A5') {
    $rep->MultiCell(160, 20, "Date:  " . sql2date($from_trans['trans_date']), 0, 'L', 0, 2, 488, 70, true);

    $rep->MultiCell(524, 55, "", 1, 'L', 0, 2, 40, 70, true);
//    }
//    elseif($pagesize=='A4'){
...
//        $as_dimen = get_dimension_new($myrow1['dimension2_id']);
        ...
        if($myrow1['amount'] > 0)
        {
            $rep->TextCol(3, 4,	number_format2($myrow1['amount']), -2);
            $dr += ($myrow1['amount']);
        }
        else
        {
            ...
            $rep->TextCol(4, 5,	number_format2($myrow1['amount']*-1), -2);
        }

        $rep->TextCollines(2, 3,	get_account_chart_master_($myrow1['account']), -2);
        // $rep->NewLine(-1);
        if($myrow1['amount'] > 0)
        {
            ...
            {
//$rep->NewLine(-1);
                $rep->MultiCell(800, 316, "".$myrow1['memo_'],0, 'L', 0, 2, 97,82, true);
            }
// }else{
...
//        $rep->NewLine(1);
//            $rep->font('b');
            $rep->MultiCell(60, 185, "",1, 'C', 0, 2, 40,125, true);
            // $rep->MultiCell(386, 170, "",1, 'C', 0, 2, 178,125, true);
            $rep->MultiCell(60, 185, "",1, 'C', 0, 2, 440,125, true);
            $rep->MultiCell(64, 185, "",1, 'C', 0, 2, 500,125, true);
              $rep->MultiCell(124, 15, "",1, 'C', 0, 2, 440,295, true);
...
//            $rep->font('b');
        $rep->setfontsize(10);
        $rep->MultiCell(220, 20, "".$trans_name ." "."Voucher" ,0, 'C', 0, 2, 170,50, true);
        $rep->setfontsize(8.5);
//            $rep->font('');
...
//        $rep->MultiCell(160, 20, "Cheque Date:" . sql2date($from_trans['cheque_date']), 0, 'L', 0, 2, 435, 120, true);
    ...
    $rep->MultiCell(160, 20, "Voucher No:   ". $from_trans["ref"] ,0, 'L', 0, 2, 41,71, true);
    $rep->MultiCell(160, 20, "Narration:   " ,0, 'L', 0, 2, 41,82, true);
//    }
//    elseif($pagesize=='A5') {
//    $rep->MultiCell(525, 15, "", 1, 'L', 0, 2, 40, 295, true);
//    $rep->MultiCell(48, 15, "", 1, 'L', 0, 2, 40, 295, true);
//    $rep->MultiCell(132, 15, "", 1, 'L', 0, 2, 88, 295, true);
    $rep->MultiCell(400, 15, "Total   ", 1, 'C', 0, 2, 40, 295, true);
    $rep->MultiCell(42, 15, " " . number_format2($dr), 0, 'R', 0, 2, 432, 295, true);
    $rep->MultiCell(45, 15, " " . number_format2($dr), 0, 'R', 0, 2, 500, 295, true);
    ...
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL' || $db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL2') {
        ...
        $rep->MultiCell(120, 20, "", 1, 'C', 0, 2, 420, 324, true);
    }
    ...
    $rep->MultiCell(100, 25, "Posted  By", 0, 'C', 0, 2, 170, 405, true);
    $rep->MultiCell(100, 25, "Sr. Manager Finance", 0, 'C', 0, 2, 300, 405, true);
    $rep->MultiCell(100, 25, "Director", 0, 'C', 0, 2, 430, 405, true);
//    }
	$user =get_user_id2($trans_no,$type);
			
			$a =get_user_name_701234($user);
        $rep->MultiCell(200, 25, $a['real_name'] ,0, 'L', 0, 2, 50,390, true);
    $rep->End();
    $rep->NewLine(2);
    ...
    if ($memo != "")
    {
        $rep->MultiCell(500, 316, "".$memo,0, 'L', 0, 2, 97,82, true);
