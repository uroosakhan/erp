...
//    $headers = array(_(' Account'), _(' Account'), _(''),_('Centre'), _('Rs'), _('Rs'));
    ...
    $rep = new FrontReport(_(''), "ChartOfAccounts", 'A5_', 9, $orientation);

//    if ($orientation == 'L')
//        recalculate_cols($cols);

    $rep->Font();
    $rep->SetHeaderType('Header701241');
...
//    if($pagesize=='A5') {
    ...
    $rep->MultiCell(160, 20, "Number :  " . $from_trans["ref"], 0, 'L', 0, 2, 40, 70, true);
//    }
//    elseif($pagesize=='A4'){
...
//        $as_dimen = get_dimension_new($myrow1['dimension2_id']);
        ...
        if($myrow1['amount'] < 0)
        {
            ...
            $rep->TextWrapLines(222, 203, get_comments_string($from_trans['type'], $from_trans['trans_no']));
        }
        else
        {
            $rep->TextWrapLines(222, 203,$myrow1['memo_']);
...
//    elseif($pagesize=='A5') {
    ...
    $rep->MultiCell(205, 15, "Total   ", 1, 'R', 0, 2, 220, 295, true);
    $rep->MultiCell(70, 15, " " . number_format2($dr,2), 1, 'R', 0, 2, 425, 295, true);
    $rep->MultiCell(70, 15, " " . number_format2($dr,2), 1, 'R', 0, 2, 495, 295, true);
    ...
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL' || $db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'RPL2') {
        ...
        $rep->MultiCell(120, 20, number_format2(abs($dr),2), 1, 'C', 0, 2, 420, 324, true);
    }
