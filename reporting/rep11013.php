...
function get_combo1_name($stock_id,$name)
{
...
function customer_phone_no($debtor_no)
{
...
function print_deliveries()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
			...
			while ($myrow2=db_fetch($result))
			{
                ...
                $rep->TextColLines(2, 3,get_combo1_name($myrow2['combo1']), -2);
				$newrow = $rep->row;
				$rep->row = $oldrow;
...
//				$rep->Font();
        ...
        $phone=customer_phone_no($rep->formData['debtor_no']);
		$rep->font('b');
		$rep->MultiCell(510, 35,$rep->company['coy_name'] , 0, 'L', 0, 2, 40,670, true);
		$rep->font('');
		$rep->MultiCell(510, 35,$rep->company['postal_address'] , 0, 'L', 0, 2, 40,685, true);
		$rep->MultiCell(510, 35,"Tel:".$rep->company['phone']."," ."Fax:". $rep->company['fax'].","."Email:".$rep->company['email'] , 0, 'L', 0, 2, 40,710, true);
        $rep->MultiCell(110, 48 ,'PO Box' , 0, 'L', 0, 2, 40,179, true);
        $rep->MultiCell(113, 24 ,"T#".$phone['phone'] , 0, 'L', 0, 2, 150,179, true);
        $rep->MultiCell(113, 24 ,"F#".$phone['fax'] , 0, 'L', 0, 2, 150,204, true);
        $rep->MultiCell(110, 23 ,'Attn' , 0, 'L', 0, 2, 40,227, true);
        $rep->MultiCell(113, 23 ,"M#".$phone['email'], 0, 'L', 0, 2, 150,227, true);

        $rep->MultiCell(99, 24 , 'DC No' ,1,'L', 0, 2, 263,154, true);
		$rep->MultiCell(202, 24 , $myrow['reference'] ,1,'L', 0, 2, 363,154, true);
		$rep->MultiCell(99, 24 ,'Date' , 1, 'L', 0, 2, 263,179, true);
		$rep->MultiCell(202, 24 ,sql2date($rep->formData['document_date']) , 1, 'L', 0, 2, 363,179, true);
		$rep->MultiCell(99, 24 ,'Your Ref (PO)No' , 1, 'L', 0, 2, 263,204, true);
		$rep->MultiCell(202, 24 , '' , 1, 'L', 0, 2, 363,204, true);
		$rep->MultiCell(99, 23 ,'Your Contact' , 1, 'L', 0, 2, 263,227, true);
		$rep->MultiCell(202, 23 , $rep->company['phone'], 1, 'L', 0, 2, 363,227, true);
