...
function print_invoices()
{
	...
	if ($email == 0)
	{
		$rep = new FrontReport(_('ESTIMATE'), "InvoiceBulk", 'POS4', '8');
		$rep->SetHeaderType('Header1184');
		$rep->currency = $cur;
		$rep->Font();
...
//		$rep->fontSize += 2;
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		//	$rep->NewLine();
			...
			$rep->TextCol(3, 5, $DisplayTotal, -2);
			$rep->fontSize -= 4;
...
$TotalBalance2 = number_format2($TotalDebit-$TotalCredit); 
		...
		$rep->NewLine(3);
		$rep->fontSize += 2;
		$rep->TextCol(0, 6, _("No Exchange Once Open, No Refund Policy"), -2);
		$rep->fontSize -= 2;
