...
function print_Chart_of_Accounts()
{
	...
	$cols = array(2, 50, 180, 380, 435, 430);

	$headers1 = array(_('ActCode'), _('A/c .Title'), _('Description'),_('Cost Center'), _('Amount'), _(''));
//    $headers = array(_(' Account'), _(' Account'), _(''),_('Centre'), _('Rs'), _('Rs'));
	...
	$rep->Font();
    $rep->SetHeaderType('Header701202');
    ...
    $rep->font('b');
    $rep->setfontsize(15);
    $rep->MultiCell(525, 20, "".$trans_name ,1, 'C', 0, 2, 40,120, true);
    $rep->setfontsize(8);
    $rep->font('');
...
//    $rep->MultiCell(200, 60, "".htmlspecialchars_decode(get_branch_name_od($from_trans['person_id'])) ,0, 'L', 0, 2, 110,155, true);
    ...
    $rep->MultiCell(60, 20, "Rupees :",0, 'C', 0, 2, 40,340, true);
    $rep->MultiCell(350, 20, convert_number_to_words($from_trans['amount']*-1),0, 'C', 0, 2, 30,340, true);
...
//
   $rep->MultiCell(95, 20, number_format2($from_trans['amount']*-1),1, 'C', 0, 2, 470,332, true);
...
//    $rep->MultiCell(250, 20, "ENTRY DATE:  ".$date ,0, 'L', 0, 2, 450,50, true);
//    $rep->MultiCell(250, 20, "TIME :  ".$time ,0, 'L', 0, 2, 450,60, true);

    $rep->MultiCell(160, 20, "Dated                :  ".sql2date($from_trans['trans_date']) ,0, 'L', 0, 2, 50,165, true);

//    $rep->MultiCell(160, 20, "Voucher No       :". $from_trans["trans_no"] ,0, 'L', 0, 2, 50,150, true);

    //  $from_trans['person_id']$from_trans['account_code']." - ". $from_trans['bank_account_name']
//    display_error($from_trans['type']);
    if($from_trans['type']==41 || $from_trans['type']==42)
    {
        $rep->MultiCell(160, 20, "Main A/c            :".$from_trans['account_code'] ,0, 'L', 0, 2, 50,178, true);
        $rep->MultiCell(170, 30, "".htmlspecialchars_decode($from_trans['bank_account_name']) ,0, 'L', 0, 2, 180,178, true);
        $rep->MultiCell(160, 20, "Voucher No       :". $from_trans["ref"] ,0, 'L', 0, 2, 50,150, true);
//        $rep->MultiCell(160, 20, "Voucher No       :"."BPV   ". $from_trans["trans_no"] ,0, 'L', 0, 2, 50,150, true);
    ...
    else
    {
        $rep->MultiCell(160, 20, "Main A/c            :".$from_trans['account_code'] ,0, 'L', 0, 2, 50,178, true);
        $rep->MultiCell(170, 30, "".htmlspecialchars_decode($from_trans['bank_account_name']) ,0, 'L', 0, 2, 180,178, true);
        $rep->MultiCell(160, 20, "Cheque Ref:  ".$from_trans['cheque'] ,0, 'L', 0, 2, 52,360, true);
    $rep->MultiCell(160, 20, "Cheque Date:  ".sql2date($from_trans['cheque_date']) ,0, 'L', 0, 2,146,360, true);
        $rep->MultiCell(160, 20, "Voucher No       :". $from_trans["ref"] ,0, 'L', 0, 2, 50,150, true);
...
//        alt_table_row_color($k);
    ...
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 50,400, true);
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 180,400, true);
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 300,400, true);
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 400,400, true);
    $rep->MultiCell(100, 25, "Prepered By" ,0, 'C', 0, 2, 40,420, true);
    $rep->MultiCell(100, 25, "Approved  By" ,0, 'C', 0, 2, 170,420, true);
    $rep->MultiCell(100, 25, "Checked  By" ,0, 'C', 0, 2, 290,420, true);
    $rep->MultiCell(100, 25, "Recieved  By" ,0, 'C', 0, 2, 390,420, true);
...
//    $rep->MultiCell(43, 15, "",1, 'C', 0, 2, 471,181, true);
