...
function print_credits()
{
	...
	$cols = array(4, 70, 130, 180, 260, 360, 430);
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$rep->TextCol(0, 1,	$myrow2['text1'], -2);
			$oldrow = $rep->row;
			$rep->TextColLines(1, 2, sql2date($myrow2['date1']), -2);
			$newrow = $rep->row;
			$rep->row = $oldrow;
			$rep->TextCol(2, 3,	sql2date($myrow2['date2']), -2);
...
// 		$rep->TextCol(6, 7,	$DisplaySubTot, -2);
		...
		$rep->Font('bold');
		$rep->TextCol(4, 5, _("TOTAL Amount"). ' ' . "USD $", - 2);
		$rep->TextCol(6, 7, price_format($DisplayTotal), -2);
		//	$rep->NewLine();
				$rep->TextCol(6, 7, "_____________________________________", - 2);

		$words = price_in_words($myrow['Total'], ST_CUSTCREDIT);
		if ($words != "")
		{
			$rep->NewLine(1);
			$rep->TextCol(1, 7, "USD $: " . $words, - 2);
		}	
