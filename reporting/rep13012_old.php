...
elseif ($p_q==0)
{
    ...
    $Net_percent = $Net_profit / $qty_cost * 100;
              ...
              elseif($week==5)
            {
                $prices_list = get_price_for_item($myrow['stock_id']);
                $rep->TextCol(0, 1, $myrow['description']."-".$prices_list);
                $rep->TextCol(6, 7, round($Total_w1));
    			$rep->TextCol(13, 14, round($Total_w2));
    			$rep->TextCol(20, 21, round($Total_w3));
    			$rep->TextCol(27, 28, round($Total_w4));
                $rep->Font('bold');
                if ($p_q == 1) {
                    $total_qty += $qty;
                    $rep->AmountCol(36, 37, $qty);
...
//--------------month wise end------------//
        ...
        {
            $rep->TextCol(0, 1, _('Grand Total'));
            $rep->TextCol(6, 7, "(".$grand_total_w1.")");
            $rep->TextCol(13, 14, "(".$grand_total_w2.")");
            $rep->TextCol(20, 21, "(".$grand_total_w3.")");
            $rep->TextCol(27, 28, "(".$grand_total_w4.")");
        ...
        $invoices_total_w4 = $invoices['per22'] + $invoices['per23'] + $invoices['per24'] +$invoices['per25'] + $invoices['per26'] + $invoices['per27'] + $invoices['per28'] + $invoices['per29'] + $invoices['per30'];
            ...
            if($week == 5)
            {
                ...
                $rep->TextCol(6, 7, "(".$invoices_total_w1.")");
                $rep->TextCol(13, 14, "(".$invoices_total_w2.")");
                $rep->TextCol(20, 21, "(".$invoices_total_w3.")");
                $rep->TextCol(27, 28, "(".$invoices_total_w4.")");
...
if($week == 5)
{
    ...
    $rep->TextCol(6, 7, "(".round($overall_discount_w1).")");
    $rep->TextCol(13, 14, "(".round($overall_discount_w2).")");
    $rep->TextCol(20, 21, "(".round($overall_discount_w3).")");
    $rep->TextCol(27, 28, "(".round($overall_discount_w4).")");
...
//-------------------for week end----------------//
        ...
        $grand_total_w4 = $tot_per22 + $tot_per23 + $tot_per24 + $tot_per25 + $tot_per26 +
                ...
                {
                    $rep->TextCol(0, 1, _('Grand Total'));
                    $rep->TextCol(6, 7, "(".round($grand_total_w1).")");
                    $rep->TextCol(13, 14, "(".round($grand_total_w2).")");
                    $rep->TextCol(20, 21, "(".round($grand_total_w3).")");
                    $rep->TextCol(27, 28, "(".round($grand_total_w4).")");
...
//            $rep->AmountCol(36, 37, $grand_total_w1, 0);
...
if($week == 5)
{
    ...
    $rep->TextCol(6, 7, "(".(round($grand_total_w1 - $overall_discount_w1)).")", 0);
    $rep->TextCol(13, 14, "(".(round($grand_total_w2 - $overall_discount_w2)).")", 0);
    $rep->TextCol(20, 21, "(".(round($grand_total_w3 - $overall_discount_w3)).")", 0);
    $rep->TextCol(27, 28, "(".(round($grand_total_w4 - $overall_discount_w4)).")", 0);
