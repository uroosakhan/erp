...
{
	$sql = "SELECT poline.*, units,poline.con_factor
...
function print_po()
{
	...
	$cols = array(1, 200, 270, 330, 370, 400, 450);
	...
	for ($i = $from; $i <= $to; $i++)
	{
	 ...
	 if($pref['alt_uom'] == 1 && $item['units'] != $myrow2['units_id'])
			$Net = round2(($myrow2["unit_price"] * ($myrow2["quantity_ordered"] * $myrow2["con_factor"])), user_price_dec());
			else
...
//				$rep->TextCol(0, 1,	$myrow2['item_code'], -2);
				$rep->TextCol(0, 1,	$myrow2['description'], -2);
			} else
				$rep->TextCol(0, 1,	$myrow2['description'], -2);
			$rep->TextCol(1, 2,	sql2date($myrow2['delivery_date']), -2);
			
			 $pref = get_company_prefs();
              $item=get_item($myrow2['item_code']);
                if($pref['alt_uom'] == 1 && $item['units'] != $myrow2['units_id'])
			$rep->Amountcol(3, 4,	$myrow2["quantity_ordered"] * $myrow2['con_factor'], $dec);
			else
			$rep->Amountcol(3, 4,	$myrow2["quantity_ordered"], $dec);
			
			//$rep->TextCol(4, 5,	$myrow2['units'], -2);
			$rep->TextCol(2, 3,	$myrow2['amount3']."  ".$item['carton'], -2);
			$myrow3 = get_company_item_pref('con_factor');
			display_error(get_company_item_pref('con_factor'));
			if($pref['alt_uom'] == 1 ){
			   if($myrow3['purchase_enable'] == 1){
         //   $rep->TextCol(5, 6,	$myrow2['con_factor'], -2);
					$rep->TextCol(5, 6,	$DisplayPrice."/".$myrow2['units_id'], -2);
