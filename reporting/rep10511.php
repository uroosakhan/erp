...
function getTransactions($category, $location, $fromcust, $from, $to, $item_location)
{
	$from = date2sql($from);
	$to = date2sql($to);
...
//	".TB_PREF."item_units.abbr = '$unit'
			...
			if ($item_location != -1)
				$sql .= " AND ".TB_PREF."stock_master.combo1 = ".db_escape($item_location);
			if ($fromcust != '')
...
//	$to = $_POST['PARAM_1'];
	...
	$sql = "SELECT * FROM ".TB_PREF."combo1 WHERE inactive = 0";
	if ($item_location != -1)
		$sql .= " AND ".TB_PREF."combo1.combo_code = ".db_escape($item_location);
//	$sql .= " ORDER BY combo_code";
	...
	while($get_item_location = db_fetch($fetch))
	{
		...
		$res = getTransactions($category, $location, $fromcust, $from, $to, $get_item_location['combo_code']);
