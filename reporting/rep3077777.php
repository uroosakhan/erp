...
function get_stock_movements_for_reports($stock_id, $StockLocation='',$BeforeDate, $AfterDate, $category, $sys_type, $sorter)
{
    ...
    $after_date = date2sql($AfterDate);
    $sql = "SELECT ".TB_PREF."stock_moves.type,".TB_PREF."stock_moves.con_factor, ".TB_PREF."stock_moves.trans_no, ".TB_PREF."stock_moves.tran_date, ".TB_PREF."stock_moves.person_id, 
	".TB_PREF."stock_moves.qty, ".TB_PREF."stock_moves.reference,".TB_PREF."stock_moves.stock_id,".TB_PREF."stock_moves.standard_cost,
...
//		$location = '';
    ...
    $before_qty = get_pry_qoh_on_date($item,$location,add_days($from_date, -1),$batch);
...
//	$after_qty = $before_qty;
    ...
    while ($myrow=db_fetch($result))
    {
        ...
        if ($myrow["qty"]  * $myrow["con_factor"] > 0)
        {
            $quantity_formatted = number_format2($myrow["qty"] * $myrow["con_factor"], $dec);
            $total_in += $myrow["qty"];
        }
        else
        {
            $quantity_formatted = number_format2(-$myrow["qty"] * $myrow["con_factor"], $dec);
            $total_out += -$myrow["qty"] * $myrow["con_factor"];
        }
        $after_qty += $myrow["qty"] * $myrow["con_factor"];
...
//		$rep->NewLine();
        ...
        $rep->TextCol(6, 7, (($myrow["qty"] * $myrow["con_factor"] >= 0) ? $quantity_formatted : ""));
        $rep->TextCol(7, 8, (($myrow["qty"] * $myrow["con_factor"] < 0) ? $quantity_formatted : ""));
        ...
        if($myrow['type'] == 25 || $myrow['type'] == 17 || $myrow['type'] == 13) {
            ...
            $rep->AmountCol(11, 12, $myrow['price'] * $myrow["qty"] * $myrow["con_factor"], $dec);
        }
        if ($location == "") {
            if ($destination == 0) {
//	$rep->NewLine(+1);
