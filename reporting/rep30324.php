...
$rep->font('B');
            ...
            elseif($templates == 1){
                $rep->SetFontSize(8);
                $rep->MultiCell(58, 35,  $myrow['stock_id'], 0, 'L', 0, 2,6, 4, true);
               $name =  preg_replace('/[^A-Za-z0-9\-]/', '', $rep->company['coy_name']);
                  $rep->MultiCell(108, 35,"".$comments ."" , 0, 'L', 0, 2,50, 4, true);
                

                $rep->MultiCell(58, 35,  $myrow['stock_id'], 0, 'L', 0, 2, 145, 4, true);
                 $rep->MultiCell(108, 35, "". $comments."", 0, 'C', 0, 2, 175, 4, true);

                $myrow7 = get_company_item_pref('text1');
                if($code_length < 8  ) {
                    
                    $rep->MultiCell(108, 35, get_category_name($item['category_id']), 0, 'L', 0, 2, 6, 15, true); // for price
                    $rep->MultiCell(108, 35, get_category_name($item['category_id']), 0, 'L', 0, 2, 150, 17, true); // for price
                }
                else{
                ...
                $rep->SetFontSize(8.2);
                $rep->MultiCell(87, 28,  $item['description'] , 0, 'L', 0, 2, 6, 25, true); // for price
                $rep->MultiCell(200, 30, $item['description'] , 0, 'L', 0, 2, 150, 35, true); // for price

$rep->SetFontSize(8.5);
$rep->font('B');
                $rep->MultiCell(200, 30, "Rs.". $retailprice."/".$item['units'] , 0, 'L', 0, 2, 6, 55, true); // for price
                $rep->MultiCell(200, 33, " Rs.". $retailprice."/".$item['units'] , 0, 'L', 0, 2, 150, 45, true); // for price
