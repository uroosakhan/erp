...
function get_combo1_name1112($stock_id,$name)
{
...
function get_salesman_name11112($id)
{
...
//     if($pref['alt_uom'] == 1  && $myrow233['sale_enable'] == 1){
	$cols = array(4, 30, 90, 140, 340, 370, 440);
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		$rep->SetCommonData($myrow, $branch, $sales_order, $baccount, 10, $contacts);
		$rep->SetHeaderType('Header10728');
		$rep->NewPage();
...
//				else
//				$rep->Amountcol(3, 4,	$DisplayQty, $dec);
...
//                    $rep->TextCol(6, 7,	$DisplayNet, -2);
                ...
                $total_amt=number_format($myrow2['quantity']*$myrow2['unit_price'],get_qty_dec($myrow2['item_code']));
                $rep->TextCol(1,2,($myrow2['text1']),-2);
                $rep->TextCol(2,3,get_combo1_name1112($myrow2['combo1']),-2);
                $rep->TextCol(4,5, number_format($myrow2['quantity'],get_qty_dec($myrow2['item_code'])), $dec);
                $rep->TextCol(5,6, number_format($myrow2['unit_price'],get_qty_dec($myrow2['item_code'])), $dec);
                $rep->TextCol(6,7,  $total_amt);
        ...
        $rep->font('');
        $rep->MultiCell(50, 10,number_format($qty_total,get_qty_dec($myrow2['item_code'])) ,0, 'R', 0, 2, 350,500, true);
        $rep->MultiCell(50, 10,number_format($total_amt_,get_qty_dec($myrow2['item_code'])) ,0, 'R', 0, 2, 510,500, true);
		if ($myrow['comments'] != "")
		{
			$rep->NewLine();
...
//		$rep->MultiCell(525, 30, "".$myrow['discount1'] ,0, 'L', 0, 2, 540,645, true);//S.no
        $amount_in_words = _number_to_words($total_amt_);
	$rep->MultiCell(525, 30, 'Dear Sir, we thank you for your purchase order and we are pleased to submit our PI as follows.' ,0, 'L', 0, 2, 40,220, true);
		$rep->Font('bold');
	$rep->MultiCell(525, 30, 'Amount in words:'.$amount_in_words ,0, 'L', 0, 2, 40,530, true);
		$rep->Font();
	$rep->MultiCell(525, 30, 'Please let us know if any further clarification required in this regard.' ,0, 'L', 0, 2, 40,700, true);
	$rep->MultiCell(525, 30, 'Thanks & kind regards, ' ,0, 'L', 0, 2, 40,730, true);
		$rep->Font('bold');
		$rep->MultiCell(525, 30, get_salesman_name11112($myrow['Salesman_']) ,0, 'L', 0, 2, 40,750, true);
		$rep->Font();
		$rep->MultiCell(525, 30, 'Sr. Sales And Application Engr' ,0, 'L', 0, 2, 40,760, true);
		...
		$rep->font('b');
		$rep->MultiCell(525, 30,$rep->company['coy_name'] , 0, 'L', 0, 2, 40,780, true);
		$rep->font();
		$rep->MultiCell(525, 30,$rep->company['postal_address'] , 0, 'L', 0, 2, 40,800, true);
		$rep->MultiCell(525, 30,"Tel:".$rep->company['phone']."," ."Fax:". $rep->company['fax'].","."Email:".$rep->company['email'], 0, 'L', 0, 2, 40,820, true);

		$rep->setfont('','','16');
		$rep->font('b');
		$rep->MultiCell(525, 30, 'PERFORMA INVOICE' , 0, 'L', 0, 2, 240,80, true);
		$rep->font();
		$rep->setfont('','','');
	}
	if ($email == 0)
