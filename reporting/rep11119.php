...
//----------------------------------------------------------------------------------------------------
function get_phone_number($debtor_no)
{
...
function print_sales_quotations()
{
	...
	$cols = array(4, 29, 90, 200, 250, 320, 366, 420 ,470);
	...
	for ($i = $from; $i <= $to; $i++)
	{
        ...
        $DisplayQuan = number_format2($quantity,$dec);
        $DisplaySubTot = number_format2($SubTotal,$dec);
        $DisplayFreight = number_format2($myrow["freight_cost"],$dec);
        $DisplayPacking = number_format2($packing,$dec);
        $rep->NewLine(1);

        $rep->NewLine(-1);
        $rep->Font('bold');
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);
        $rep->NewLine(+1);

        $rep->TextCol(2, 3, _("Sub-total"), -2);

        $rep->TextCol(7, 8,	$DisplaySubTot, -2);
        $rep->TextCol(3, 4,	$DisplayQuan, -2);
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);
        $rep->Font('');
        if ($myrow['comments'] != "")
		{
		...
		//////////////////////
       $rep->MultiCell(522, 80, "" ,1, 'L', 0, 2, 45,620, true);
        $rep->Font('bold');
        $rep->TextCol(1, 3, _("Price Title:"), -2);
        $rep->Font('');
//        $rep->TextCol(3, 4,, -2);
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);
        ...
        $rep->NewLine();
        $rep->Font('bold');
        $rep->TextCol(1, 3, _("Currency:"), -2);
        $rep->Font('');
        $rep->TextCol(3, 4,$myrow['curr_code'], -2);
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);

        $rep->NewLine();
        $rep->Font('bold');
        $rep->TextCol(1, 3, _("Sales Respresentative:"), -2);
        $rep->Font('');
        $rep->TextCol(3, 5, get_salesman_name($myrow['salesman']), -2);
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);

        $rep->NewLine();
        $rep->Font('bold');
        $rep->TextCol(1, 3, _("Estimated Delivery Time:"), -2);
        $rep->Font('');
        $rep->TextCol(3, 4,$myrow['h_text3'], -2);
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);

        $rep->NewLine();
        $rep->Font('bold');
        $rep->TextCol(1, 3, _("Payment Terms:"), -2);
        $rep->Font('');
        $rep->TextCol(3, 4,get_payment_term_name($myrow['payment_terms']), -2);
        $rep->TextCol(0, 11, "_______________________________________________________________________________________________________________", -2);
...
//			$rep->TextCol(7, 8,	$DisplayTotal, -2);
		...
		///Net Total
        $Total=$SubTotal+ $myrow["freight_cost"] + $tax_item['Value'];
		/////
        $Totals= number_format2($Total,$dec);
        $rep->Font('bold');
        $rep->TextCol(5, 7, _("Net Total:"), -2);
        $rep->Font('');
        $rep->TextCol(7, 8,$Totals, -2);
        $rep->NewLine();
		$rep->NewLine();

		$DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal, $dec);
		$rep->Font('bold');
//		$rep->TextCol(5, 7, _("TOTAL ORDER GST INCL."), - 2);
//		$rep->TextCol(7, 8,	$DisplayTotal, -2);
