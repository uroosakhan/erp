...
function print_sales_orders()
{
    ...
    if($pref['batch'] == 1)
    {
        $cols = array(4, 60, 200, 258, 303, 350, 370, 420 ,480,460, 485);
        $aligns = array('left',	'left',	'left', 'left', 'left', 'right', 'right', 'right', 'right');
    }
    else
    {
        $cols = array(4, 60, 280,  330, 370, 410, 450);
...
//                $item=get_item($myrow2['stk_code']);
        ...
        if($pref['batch'] == 0)
        {
            ...
            if ($myrow['tax_included'] == 0) {
                $rep->TextCol(3, 6, _("TOTAL ORDER EX TAXES"), -2);
        ...
        {
            if ($myrow["discount1"] != 0) {
                $rep->TextCol(4, 5, _("Discount") . $myrow["discount_percent"], -2);
                $rep->TextCol(8, 9, price_format($total_discount), -2);
                $rep->NewLine();
            }
            if ($myrow["discount2"] != 0) {
                $rep->TextCol(4, 7, _("Discount"), -2);
                $rep->AmountCol(8, 9, $myrow["discount2"], $dec);
            }
            $rep->NewLine();
            if ($myrow['freight_cost'] != 0.0) {
                $DisplayFreight = number_format2($myrow["freight_cost"], $dec);
		...
		$rep->MultiCell(100, 25, "".get_user_name_70123($user) ,0, 'C', 0, 2, 110,775, true);
