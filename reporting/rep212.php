...
//----------------------------------------------------------------------------------------------------
function get_remittance_tax($type, $trans_no)
{
  	$sql = "SELECT SUM(supply_disc+service_disc+fbr_disc+srb_disc) as TaxAmount FROM `0_supp_trans` AS trans
...
function print_remittances()
{
	...
	$cols = array(4, 85, 150, 225, 275, 380, 450, 515);
	...
	if ($email == 0)
		$rep = new FrontReport(_('REMITTANCE'), "RemittanceBulk", user_pagesize(), 7, $orientation);
    if ($orientation == 'L')
...
//	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j)
		{
			$myrow = get_remittance($j, $to);
			if (!$myrow)
				continue;
			if ($currency != ALL_TEXT && $myrow['curr_code'] != $currency) {
				continue;
			}
			$res = get_bank_trans($j, $to);
			...
			if ($email == 1)
			{
				...
				$rep->filename = "Remittance" . $to . ".pdf";
			}
			$rep->currency = $cur;
			$rep->Font();
			...
			$rep->SetHeaderType('Header213');
			$rep->NewPage();
			$myrow_tax = get_remittance_tax(ST_SUPPAYMENT,$myrow['trans_no']);
            ...
            $rep->NewLine();
            $rep->TextCol(3, 6, _("WHT Tax"), -2);
            $rep->AmountCol(6, 7, $myrow_tax['TaxAmount'], $dec, -2);
            // $rep->NewLine();
            ...
            $rep->NewLine();
            $rep->Font('bold');
            $rep->TextCol(3, 6, _("TOTAL PAYMENT"), - 2);
            // $total_remittance = $myrow['Total'] - $myrow_tax['TaxAmount'];
            $rep->AmountCol(6, 7, $myrow['Total'], $dec, -2);
            $rep->Font('');



            $rep->row = $rep->bottomMargin + (55 * $rep->lineHeight);
            ...
            $myrow_tax = get_remittance_tax(ST_SUPPAYMENT,$myrow['trans_no']);
            $result1 = get_allocatable_to_supp_transactions($myrow['supplier_id'], $myrow['trans_no'], $myrow['type']);
            $rep->TextCol(0, 4,	_("As advance / full / part / payment towards:"), -2);
            $rep->NewLine(2);
            $total_allocated1 = 0;
...
//            $rep->NewLine(-22);
            ...
            $myrow['Total'] *= -1;
            $myrow['ov_discount'] *= -1;
            // $rep->AmountCol(6, 7, $myrow['Total'] + $myrow['ov_discount'] - $total_allocated1, $dec, -2);
            if (floatcmp($myrow['ov_discount'], 0))
            {
                $rep->NewLine();
                $rep->TextCol(3, 6, _("Discount"), - 2);
