...
function get_sales_return ($stock_id,$loc_code,$from,$to)
{
...
function getTransactions($category, $location, $fromcust, $from, $to)
{
	...
	SUM(-".TB_PREF."stock_moves.qty ) AS qty,
			...
			SUM(-IF(".TB_PREF."stock_moves.standard_cost <> 0, ".TB_PREF."stock_moves.qty * ".TB_PREF."stock_moves.standard_cost, ".TB_PREF."stock_moves.qty *(".TB_PREF."stock_master.material_cost + ".TB_PREF."stock_master.labour_cost + 
			 ".TB_PREF."stock_master.overhead_cost))) AS cost,
			 
	 SUM(IF(".TB_PREF."stock_moves.stock_id IS NULL,0,".TB_PREF."stock_moves.qty))AS QtyOnHand,
		
SUM(-".TB_PREF."stock_moves.qty AND ".TB_PREF."stock_moves.type = ".ST_CUSTCREDIT."  ) AS sales_return_qty
...
function print_inventory_sales()
{
	...
	$cols = array(0, 41, 180, 250, 300, 370, 438,	480,535);

	$headers = array(_('Category'), _('Description'), _('Customer'), _('Sales Qty'), _('Sales Value'),_('Sal.Return'), _('  Bal. Qty After Sale'),_('Phy.Stock Qty'));
	if ($fromcust != '')
		$headers[2] = '';

	$aligns = array('left',	'left',	'left', 'right', 'right', 'right', 'right', 'right', 'left','left');
    ...
    $rep = new FrontReport(_('Inventory &Sales With Balance Stock Report'), "InventorySalesReport", user_pagesize(), 9, $orientation);
   	if ($orientation == 'L')
		...
		if ($catt != $trans['cat_description'])
		{
			if ($catt != '')
			{
				...
				$rep->AmountCol(4, 5, $total, $dec);
                                $rep->AmountCol(5, 6, abs($total3), $dec);
                                $rep->AmountCol(6, 7, $total2, $dec);
				$rep->AmountCol(3, 4, abs($total1), $dec);
				$rep->Line($rep->row - 2);
				$rep->NewLine();
				$rep->NewLine();
...
$sales_return = get_sales_return ($trans['stock_id'],$location,$from,$to);

$rep->AmountCol(3, 4, abs($trans['qty']), $dec);
		$rep->AmountCol(4, 5, $trans['amt'], $dec);

		$rep->AmountCol(5, 6, abs($trans['sales_return_qty']), $dec);


$qoh = get_qoh_on_date($trans['stock_id'],$trans["loc_code"]);

                $rep->AmountCol(6, 7, $qoh, $dec);
...
$total3 += $trans['sales_return_qty'];

		$grandtotal += $trans['amt'];
		$grandtotal1 += $trans['qty'];
		$grandtotal2 += $qoh;
$grandtotal3 += $trans['sales_return_qty'];
	}
	$rep->NewLine(2, 3);
	$rep->TextCol(0, 4, _('Total'));
...
$rep->AmountCol(5, 6, abs($grandtotal3), $dec);
