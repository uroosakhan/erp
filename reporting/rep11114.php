...
$rep->Font();
            ...
            $rep->row = $newrow;
			//$rep->NewLine(1);
			if ($rep->row < $rep->bottomMargin + (39 * $rep->lineHeight))
				$rep->NewPage();
		}
...
//					$rep->TextCol(3, 7, $tax_type_name, -2);
    ...
    $rep->MultiCell(800, 20, $tax_type_name,0, 'L', 0, 2, 285,300, true);
    	$rep->NewLine(2);
			...
			{
			 //   $rep->NewLine(-0.5);
				$SubTotal1 += $tax_item['Value'];
				$inclusive_tax = $tax_item['Value'] + $DisplayPrice;
              	 $rep->MultiCell(800, 20, $tax_type_name,0, 'L', 0, 2, 285,302, true);
                	$rep->TextCol(6, 7,	price_format($DisplayTax), -2);
