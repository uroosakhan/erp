...
{
    $sql = "SELECT SUM(`amount`)*-1 FROM `0_gl_trans` INNER JOIN 0_chart_master ON 0_chart_master.account_code=0_gl_trans.`account` INNER JOIN 0_chart_types ON 0_chart_types.id=0_chart_master.`account_type` INNER JOIN 0_chart_class ON 0_chart_class.cid=0_chart_types.`class_id` WHERE 0_chart_class.ctype=4";
	...
	$tags, &$pg, $graphics,$class)
{
	...
	//	display_error($type."---".$class);
$total_income_revenue=get_sales_revenue();
        ...
        if($class==50)
            $rep->AmountCol(4, 5, (($acc_balance * $convert)/$total_income_revenue)*100 , get_qty_dec($myrow2['stk_code']));
        //else
         //   $rep->AmountCol(4, 5, $acc_balance * $convert, $dec);
	...
	if (($code_per_balance + $per_balance_total + $code_acc_balance + $acc_balance_total) != 0 || $printtitle)
	{
        ...
        //display_error($total_income_revenue);
        if($class==50)
            $rep->AmountCol(4, 5, ((($code_acc_balance + $acc_balance_total) * $convert)/$total_income_revenue)*100 , get_qty_dec($myrow2['stk_code']));
	...
	while ($class = db_fetch($classresult))
	{
		...
		while ($accounttype=db_fetch($typeresult))
		{
			$classtotal = display_type($accounttype["id"], $accounttype["name"], $from, $to, $begin, $end, $compare, $convert, $dec, 
				$pdec, $rep, $dimension, $dimension2, $tags, $pg, $graphics/*,$class['cid']*/);
