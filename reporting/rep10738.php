...
function print_invoices()
{
	...
	$cols = array(0, 13, 70, 105, 125, 155, 170, 205, 225, 267,
        285, 298, 360, 390, 410, 440, 455, 490 ,510, 550);
	...
	while($row = db_fetch($range))
	{
		...
		$s_no=0;
			...
			while ($myrow2=db_fetch($result))
			{
	    		...
	    		$DisplayDisc = number_format2($myrow2["discount_percent"],$dec);
...
//                $item=get_item($myrow2['stk_code']);
                    ...
                    $rep->TextColLines(4, 5, $myrow2['text1'], -2);
                   $rep->TextCol(7, 8, $DisplayDisc	, -2);
...
//					$rep->TextCol(0, 11,	"__________________________________________________________________________________________________________________________________________________________________________________________________", -2);
		...
		$rep->MultiCell(340, 110, "".$sales_order['comments'] , 1, 'L', 0, 2, 40,620, true);
...
//-----------------------
	$rep->TextCol(0, 2, _("Details :"), -2);
	$rep->TextCol(10, 12, _("Details :"), -2);
	
	$rep->TextCol(2, 4, _("Total"), -2);
	$rep->TextCol(12, 14, _("Total"), -2);
	
	$rep->TextCol(4, 6, _("8 Ctns."), -2);
	$rep->TextCol(14, 16, _("8 Ctns."), -2);
	
	$rep->TextCol(6, 8, _("223232.2"), -2);
    $rep->TextCol(16, 18, _("223232.2"), -2);
    
	$rep->TextCol(7, 8, _("332.2"), -2);
	$rep->TextCol(17, 18, _("332.2"), -2);
	
	$rep->TextCol(8, 9, _("232333.22"), -2);
	$rep->TextCol(18, 19, _("232333.22"), -2);
	
	$rep->NewLine(1);
	$rep->TextCol(4, 7, _("Less Disc @ 0.00%"), -2);
	$rep->TextCol(14, 17, _("Less Disc @ 0.00%"), -2);
	
	$rep->TextCol(8, 9, _("0.00"), -2);
	$rep->TextCol(18, 19, _("0.00"), -2);
	
	$rep->NewLine(1);
	$rep->TextCol(4, 7, _("Sub-Total"), -2);
	$rep->TextCol(14, 17, _("Sub-Total"), -2);
	
	$rep->TextCol(8, 9, _("43,224.65"), -2);
	$rep->TextCol(18, 19, _("43,224.65"), -2);
	
	$rep->NewLine(1);
	$rep->TextCol(4, 7, _("15.00"), -2);
	$rep->TextCol(14, 17, _("15.00"), -2);
	...
	$rep->NewLine(1);
	$rep->TextCol(4, 7, _("Less Freight:"), -2);
	$rep->TextCol(14, 17, _("Less Freight:"), -2);
	
	$rep->TextCol(8, 9, _("0.00"), -2);
	$rep->TextCol(18, 19, _("0.00"), -2);
	
	$rep->NewLine(2);
	$rep->TextCol(4, 7, _("Net Rs. :"), -2);
	$rep->TextCol(14, 17, _("Net Rs. :"), -2);
	
	$rep->TextCol(8, 9, _("43,225.00"), -2);
	$rep->TextCol(18, 19, _("43,225.00"), -2);
...
// 			$rep->TextCol(10, 12, $DisplayTotal."  ", -2);
		...
		$total_amount =  $sign*($myrow["ov_freight"] + $myrow["ov_gst"] +
				$myrow["ov_amount"]+$myrow["ov_freight_tax"]-$myrow["discount1"]-$myrow["discount2"]) - $myrow["discount2"];
//		display_error($myrow["discount2"]);
		$rep->NewLine(-31.2);

//		$rep->TextCol(10, 9, abs($total_amount)."  ", -2);

//		$rep->MultiCell(525, 20, convert_number_to_words($total_amount),1, 'L', 0, 2, 40,572, true);

		if ($rep->formData['prepaid'])
			{
				$rep->NewLine();
				$rep->Font('bold');
				$rep->TextCol(2, 4, $rep->formData['prepaid']=='final' ? _("THIS INVOICE") : _("TOTAL INVOICE"), - 2);
