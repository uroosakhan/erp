			...
			if ($packing_slip == 0)
			{
				...
				if ($myrow['ov_freight'] != 0.0)
				{
                  ...
                  $discount_value =$myrow["disc1"]/100;
                                      $rep->MultiCell(410, 30, "Discount" ,0, 'L', 0, 2, 340,635, true);

                     $rep->MultiCell(410, 30, "".price_format(($myrow["discount1"])) ,0, 'L', 0, 2, 537,635, true);
		}
				if($myrow['discount2'] != 0) {
                    $discount_value =$myrow["disc2"]/100;
                    $rep->MultiCell(410, 30, "".price_format($myrow["discount2"]) ,0, 'L', 0, 2, 520,635, true);
                    $rep->MultiCell(410, 30, "Discount" ,0, 'L', 0, 2, 537,635, true);
				...
				// 			$rep->TextCol(4, 7,	$DisplayTax, -2);
							...
							$rep->TextCol(3, 7, _("") . " " . $taxj_type_name, -2);
							$rep->TextCol(4, 7,  $DisjplayTax, -2);
					}
    				else
    				{
				// 		$rep->TextCol(3, 6, $tax_type_name, -2);
				...
				$DisplayTotal = number_format2($myrow["ov_freight"] +$myrow["ov_freight_tax"] + $myrow["ov_gst"] +
					$myrow["ov_amount"] - $myrow["discount1"] - $myrow["discount2"] + $DisplayTax ,$dec);
				$rep->Font('bold');
