...
function get_tax_rate_10903($id)
{
	$sql = "SELECT ".TB_PREF."tax_types.rate FROM ".TB_PREF."tax_types
	 WHERE ".TB_PREF."tax_types.id = ".db_escape($id)."";
	$result = db_query($sql, "could not get taxes");
...
function get_tax_items($stock_id)
{
...
//				$amt_1 = get_tax_rate10903($myrow2['debtor_trans_no'], 2);
				$item_tax=	get_tax_items($myrow2['stk_code']);
				$item_tax_rate =	($item_tax['tax_type_id']);
				...
				}elseif($item_tax_rate==2){
					$rep->TextCol(5, 6,	"17%", -2);
					$rate =17/100;
					$sale_tax_amount = $Net*$rate;
					$rep->TextCol(6, 7,	number_format($sale_tax_amount,$dec), -2);
					
				}elseif($item_tax_rate==3){
					$rep->TextCol(5, 6,	"17%", -2);
					$rate =17/100;
					$sale_tax_amount = $Net*$rate;
				...
				}elseif($item_tax_rate==4){
					$rep->TextCol(5, 6,	"13%", -2);
					$rate =13/100;
					$sale_tax_amount = $Net*$rate;
				...
				$total_advance=$myrow['prep_amount'] ;
		...
		$total_percent=	round(($payment['ov_amount']/$totalvalue_inc_sales_tax)*100);
		...
		$rep->TextCol(1, 5, _("Amount in Words            ").$words."Rupees Only", -2);
