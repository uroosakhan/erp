...
function get_open_balance($supplier_id, $to)
{
    ...
    $sql = "SELECT SUM(IF(".TB_PREF."supp_trans.type = ".ST_SUPPINVOICE." OR ".TB_PREF."supp_trans.type = ".ST_BANKDEPOSIT.", 
		...
		AND ".TB_PREF."supp_trans.supplier_id = '$supplier_id' AND type != ".ST_SUPPCREDIT_IMPORT." GROUP BY supplier_id";
...
function getTransactions($supplier_id, $from, $to)
{
    ...
    $sql = "SELECT ".TB_PREF."supp_trans.*,
				((".TB_PREF."supp_trans.ov_amount + ".TB_PREF."supp_trans.ov_gst + ".TB_PREF."supp_trans.ov_discount + ".TB_PREF."supp_trans.gst_wh) -( supply_disc + service_disc + fbr_disc +  srb_disc))
...
//	$aligns = array('left',	'left',	'left',	'left',	'right', 'right', 'right', 'right', 'right', 'right');
    $cols = array(0, 50, 105, 175,  200, 240, 280, 335, 400, 460, 530, 600);
//	$cols = array(0, 80, 150, 160,  220, 260, 290, 335, 400, 460, 530, 600);
    ...
    $rep = new FrontReport(_('Supplier Balances - Detailed'), "SupplierBalancesDetailed", user_pagesize());
...
$QuantityTotal = 0;
		...
		//		$rep->DateCol(3, 4,	$trans['due_date'], true);
			...
			if ($trans['TotalAmount'] >= 0.0)
			{
				...
				$rep->Font('bold');					
				$rep->AmountCol(8, 9, $item[0], $dec);
				$accumulate -= $item[0];
				$rep->Font();


				$res2 = getTransactions2($myrow['supplier_id'], $from, $to, $trans['trans_no']);				
		...
		{				
				...
				$QuantityTotal += $trans2['quantity'];	
				$QuantityGrandTotal += $trans2['quantity'];
...
//			$rep->AmountCol(8, 9, $item[3], $dec);
				$rep->AmountCol(9, 10, abs($accumulate), $dec);
			...
			if ($memo != "")
			{
				$rep->Font('bold');
				$rep->NewLine();
				$rep->TextColLines(2, 5, "Memo : ".$memo, -2);
				$rep->Font();
			}
// 			$rep->Line($rep->row - 2);
		}
...
//		for ($i = 0; $i < 4; $i++)
		...
		//	$rep->AmountCol($i + 6, $i + 7, $total[$i], $dec);		
			$rep->AmountCol(8, 9, abs($total[0]), $dec);		
			$rep->AmountCol(7, 8, abs($total[1]), $dec);		
			$rep->AmountCol(9, 10, abs($total[2]), $dec);		
			$total[$i] = 0.0;
			$rep->AmountCol( 4, 5, $QuantityTotal	, $dec);
			$rep->AmountCol( 6, 7, $GrandTotalAmount	, $dec);
		}
		$rep->Font();					
    		$rep->Line($rep->row  - 4);
    		$rep->NewLine(2);
...
//		$rep->AmountCol($i + 7, $i + 8,$grandtotal[$i], $dec);
			...
			$rep->AmountCol(8, 9,abs($grandtotal[0]), $dec);		
			$rep->AmountCol(7, 8, abs($grandtotal[1]), $dec);		
			$rep->AmountCol(9, 10, abs($grandtotal[2]), $dec);		

		$rep->AmountCol( 4, 5, $QuantityGrandTotal, $dec);
		$rep->AmountCol( 6, 7, $Grand_TotalAmount, $dec);
	$rep->Font();
	$rep->Line($rep->row  - 4);	
	$rep->NewLine();
    $rep->End();
