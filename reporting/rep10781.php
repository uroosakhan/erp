...
function get_customer_information1($debtor_no)
{

    $sql = "SELECT master.debtor_no, branch.branch_code, master.name, branch.br_name, branch.br_address,branch.area,branch.salesman, DATE_FORMAT(CURDATE(),'%d-%m-%Y') as date
...
function print_invoices()
{
    ...
    $cols = array(2, 35,190, 220,260, 340, 395, 460, 520);
    // $headers in doctext.inc
    $aligns = array('center','left','center', 'center','center', 'center', 'right', 'right');
...
//                    $rep->TextCol(4, 5,	$ZeroValue, -2);
                ...
                $amount=abs(($myrow2['unit_price'] * $myrow2["quantity"] *$myrow2['discount_percent']/100)-($myrow2['unit_price'] * $myrow2["quantity"]));

                $sub_total+= $amount;
                if ($DisplayPrice < 0)
                {
                    $rep->TextCol(6, 7,	$amount, -2);
...
$net_price=$amount/$myrow2["quantity"];
...
// $PreviousBalance = number_format2($TotalDebit-$TotalCredit);
        ...
        $my_row1=get_customer_information1($debtor_no);
$area_name=get_area($my_row1['area']);
$sale_man=get_saleman($my_row1['salesman']);

$ship_via=get_shippers_name_rep($myrow['ship_via']);

        $rep->MultiCell(300, 18,"".$my_row1['br_name'], 0, 'L', 0, 2, 145,100, true);
       $rep->MultiCell(300, 18,"".$my_row1['br_address'], 0, 'L', 0, 2, 145,112, true);
       $rep->MultiCell(300, 18,"".$rep->company['phone'], 0, 'L', 0, 2, 145,127, true);
       $rep->MultiCell(300, 18,"".$area_name, 0, 'L', 0, 2, 145,140, true);
        ...
        $rep->MultiCell(300, 18,"".$rep->formData['tran_date'], 0, 'R', 0, 2, 245,102, true);
        $rep->MultiCell(300, 18,"".$rep->formData['reference'], 0, 'R', 0, 2, 245,116, true);
        $rep->MultiCell(300, 18,"".$sale_man, 0, 'R', 0, 2, 245,127, true);
        $rep->MultiCell(300, 18,"".$ship_via, 0, 'R', 0, 2, 245,138, true);
        $rep->MultiCell(300, 18,"".$rep->formData['h_text2'],   0, 'R', 0, 2, 245,150, true);
