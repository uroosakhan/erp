...
function print_workorders()
{
    ...
    $cols = array(4, 60, 245, 315, 365,  415,470);
    ...
    if ($email == 0)
        $rep = new FrontReport(_('WORK ORDER'), "WorkOrderBulk", user_pagesize(), 8, $orientation);
    if ($orientation == 'L')
    ...
    for ($i = $from; $i <= $to; $i++) {
        ...
        $rep->SetHeaderType('Header40999');
        $rep->NewPage();

        $result = get_wo_requirements_amount3($i);
...
//         continue;
        ...
        while ($myrow2 = db_fetch($result)) {
            ...
            $amount3_sum += $myrow2['amount3'];
            $rep->TextCol(0, 1, $myrow2['stock_id'], -2);
$wo = get_work_order($myrow2['workorder_id']);
$item = get_item($myrow2["stock_id"]);
//display_error($wo['wo_ref']."/".$myrow2['workorder_id']);
            $pry = $myrow2['amount3']  * $item['amount1'] ;
            $sec =  $pry * $item['con_factor'] ;

            $pry_sum += $pry;
            $sec_sum += $sec;
          //  $rep->TextCol(2, 3, $myrow2['location_name'], -2);
            $rep->TextCol(2, 3, $myrow2['WorkCentreDescription'], -2);
            $dec = get_qty_dec($myrow2["stock_id"]);
            $rep->AmountCol(3, 4, $myrow2['amount3'], $dec, -2);
            // $rep->AmountCol(5, 6, -$myrow2['amount3'] * $myrow['units_issued'], $dec, -2);

            // $rep->AmountCol(6, 7, $myrow2['amount3'], $dec, -2);
            $rep->AmountCol(4, 5, $pry, $dec, -2);
            $rep->AmountCol(5, 6, $sec, $dec, 2);
        ...
        $rep->TextCol(3, 4, $amount3_sum, -2);
        $rep->TextCol(4, 5, $pry_sum, -2);
        $rep->TextCol(5, 6, $sec_sum, -2);
        $rep->Line($rep->row  - 6);
