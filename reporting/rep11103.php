...
function print_sales_quotations()
{
	...
	$cols = array(4, 30, 50, 275, 350, 405, 475);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'left','center', 'right');
...
if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !isset($no_zero_lines_amount) || $no_zero_lines_amount == 0)
			{
                ...
                $total_net_amount=number_format($myrow2["quantity"]*$myrow2["unit_price"],$dec);

				$rep->TextCol(0, 1,	$myrow2['name'], -2);
                               $rep->TextCol(2, 3, $myrow2['description'], -2);
				$rep->AmountCol(3, 4,	$myrow2['unit_price'], $dec);
                                //$rep->TextCol(5, 6,	 $total_net_amount, $dec);
				$rep->AmountCol(4, 5,	$Net, $dec);



                //$rep->AmountCol(5, 6,	$tax_, $dec);
                $rep->AmountCol(5, 6,	$sales_tax_amount, $dec);
                $rep->AmountCol(6, 7,$Net, $dec);
			}
			$rep->row = $newrow;
			$rep->NewLine(1);
...
//        $DisplayFreight +=$tax_grand_amount;
		$rep->NewLine(-4.5);
		$rep->font('b');
		$rep->TextCol(3, 5, _("Total Net (Rs.)"), -2);
		$rep->font('');
		$rep->TextCol(6, 7, $DisplaySubTot, -2);
		//$rep->AmountCol(6, 7,	$SubTotal, $dec);
		$rep->NewLine();
		$rep->font('b');
		$rep->TextCol(3, 5, _("Carriage charges"), -2);
		$rep->font('');
...
$rep->font('');
		...
		$rep->NewLine(0.2);
        $DisplayTotal = $SubTotal + $myrow["freight_cost"];
		$rep->Font('bold');
                $rep->newline(0.9);
		$rep->TextCol(3, 5, _("Total Gross(Rs.)"), - 2);
		$rep->Font('');
