...
function print_deliveries()
{
	...
	$cols = array(6, 40, 115, 300, 325, 400, 450, 515);
	...
	for ($i = $from; $i <= $to; $i++)
	{
	    ...
	    $i = $row['gate_pass_no'];
			...
			$rep->SetHeaderType('Header11000011');
			$rep->NewPage();
		...
		$serial++;
	    		...
	    		if ($myrow2["discount_percent"]==0)
		  			...
		  			$DisplayDiscount = number_format2($myrow2["discount_percent"]*100,user_percent_dec()) . "%";
		  				$rep->TextCol(4, 5, $myrow["order_"], -2);
			    ...
			    if($myrow2['units_id']=='pack'){
                        ...
                        //      $total_packing +=$myrow2["quantity"];
                              ...
                              $rep->TextCol(5, 6, "".$SubTotal	, -2);


                    }else{


                        $rep->TextCol(5, 6,$qty	, -2);

                        $carton=$myrow2["quantity"]*$item["con_factor"];
                        $rep->TextCol(6, 7,number_format2($carton,user_percent_dec())	, -2);
                        $total_cartons +=$carton;

                        $total_packing +=$myrow2["quantity"];
                            //   $rep->TextCol(5, 6, "hareeem".$total_packing	, -2);
                    }

					
				

$rep->TextCol(3, 4,	$item['carton']);
					if ($packing_slip == 0)
					{
