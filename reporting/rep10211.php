...
function get_invoices($customer_id, $to, $all=true,$types, $dim, $no_of_days, $columns)
{
    ...
    if ($all)
		...
		trans.supply_disc + trans.service_disc + trans.fbr_disc + trans.srb_disc + trans.discount1 - trans.discount2)";*/
		 ...
		 $value = "IF (type=".ST_JOURNAL." && ov_amount < 0, -ov_amount + ov_gst + ov_freight + ov_freight_tax + ov_discount + trans.gst_wh +
		trans.supply_disc + trans.service_disc + trans.fbr_disc + trans.srb_disc + trans.discount1 - trans.discount2, 
           ov_amount + ov_gst + ov_freight + ov_freight_tax + ov_discount + trans.gst_wh +
		trans.supply_disc + trans.service_disc + trans.fbr_disc + trans.srb_disc + trans.discount1 - trans.discount2)";
 
    else
    {
           $value = "IF (type=".ST_JOURNAL." && ov_amount < 0, -ov_amount + ov_gst + ov_freight + ov_freight_tax + ov_discount +	trans.gst_wh  + trans.supply_disc + trans.service_disc + trans.fbr_disc + trans.srb_disc + trans.discount1 - trans.discount2 - alloc, 
           ov_amount + ov_gst + ov_freight + ov_freight_tax + ov_discount +	trans.gst_wh  + trans.supply_disc + trans.service_disc + 
           trans.fbr_disc + trans.srb_disc + trans.discount1 - trans.discount2 
          /* - (SELECT IFNULL(SUM(amt), 0) as total_amount FROM 0_cust_allocations WHERE date_alloc <= '$todate' AND person_id = '$customer_id')*/
    ...
    if($columns == 0)
    {
        $sql = "SELECT type, reference, tran_date, due_date, trans_no,
    ...
    if (!$all)
		...
		trans.supply_disc + trans.service_disc + trans.fbr_disc + trans.srb_disc - trans.discount1 - trans.discount2 - 
		IF(type = 10, (SELECT IFNULL(SUM(amt), 0) as total_amount FROM 0_cust_allocations WHERE date_alloc <= '$todate' AND person_id = '$customer_id'), alloc) ) > ".FLOAT_COMP_DELTA." ";
// 		- 
// 		(SELECT IFNULL(SUM(amt), 0) as total_amount FROM 0_cust_allocations WHERE date_alloc <= '$todate' AND person_id = '$customer_id')
    if($types != -1)
        $sql .= " AND type =".$types;
...
/*-
    ...
    if($columns == 0)
    {
        $sql = "SELECT debtor.name, debtor.curr_code, terms.terms, debtor.credit_limit,debtor.credit_allowed,
    			credit_status.dissallow_invoices, credit_status.reason_description, trans.trans_no,
    ...
    if (!$all)
        $sql .= " AND ABS(trans.ov_amount + trans.ov_gst + trans.ov_freight + trans.ov_freight_tax + trans.ov_discount - trans.discount1 - trans.discount2 - 
		IF(type = 10, (SELECT IFNULL(SUM(amt), 0) as total_amount FROM 0_cust_allocations WHERE date_alloc <= '$todate' AND person_id = '$customer_id'), alloc)
      /* (SELECT IFNULL(SUM(amt), 0) as total_amount FROM 0_cust_allocations WHERE date_alloc <= '$todate' AND person_id = '$customer_id') */ ) > ".FLOAT_COMP_DELTA;
...
function print_aged_customer_analysis()
{
    ...
    $columns = $_POST['PARAM_8'];
    $inv_due = $_POST['PARAM_9'];
    $summaryOnly = $_POST['PARAM_10'];
    $no_zeros = $_POST['PARAM_11'];
    $graphics = $_POST['PARAM_12'];
    $group = $_POST['PARAM_13'];
    ...
    if($columns == 0)
    {
        ...
        if ($orientation == 'P')
        {
            ...
            $headers = array(_('Customer'), 'Reference', '', _('Current'), $nowdue, $pastdue1, $pastdue2,
                _('Total Balance'));
        }
        else
            {
    ...
    else
        {
            ...
            if($summaryOnly==0) {
                $headers = array(_('Customer'), 'Reference', '', _('Current'), $nowdue, $pastdue1, $pastdue3, $pastdue4, $pastdue5, $pastdue2,
                    _('Total Balance'));
            }
            else
            {
                $headers = array(_('Customer'), 'Phone', 'Salesman', _('Current'), $nowdue, $pastdue1, $pastdue3, $pastdue4, $pastdue5, $pastdue2,
                    _('Total Balance'));
            }
    ...
    $params =   array( 	0 => $comments,
        ...
        7 => array('text' => _('Groups'), 'from' => $groups_name, 'to' => ''));
    if($summaryOnly==0) {
        if ($convert)
    ...
    if ($group != 0)
        $sql .= " AND ".TB_PREF."cust_branch.group_no = ".db_escape($group);
    ...
    while ($myrow=db_fetch($result))
    {
        ...
        if (!$custrec)
            continue;
        //   work cust_allocation# 1
       if($show_all == 0) {
          $to1 = date2sql($to);
        $sql = "SELECT SUM(amt) as Amount FROM ".TB_PREF."cust_allocations WHERE person_id = ".db_escape($myrow['debtor_no'])."
                AND trans_type_to = ".db_escape(ST_SALESINVOICE)."
                AND date_alloc <= '$to1'";
        $query = db_query($sql, " get allocation amount date wise error");
        $fetch = db_fetch($query);
        
        $custrec['Balance'] = $custrec['Balance'] - $fetch['Amount'];
        ...
        if($columns == 0)
        {
            ...
            $str = array($custrec["Balance"] - $custrec["Due"]/*  - $fetch['Amount']*/,
                $custrec["Due"] - $custrec["Overdue1"],
                $custrec["Overdue1"] - $custrec["Overdue2"],
                $custrec["Overdue2"],
                $custrec["Balance"]/* - $fetch['Amount']*/);
        }
        else
        {
            $custrec['Balance'] *= $rate;
        ...
        if ($orientation == 'L')
        {
            ...
            {
                $rep->TextCol(2, 3, get_salesman_name($myrow["salesman"]));
                $rep->TextCol(1, 2, $contact_no['phone']);
            }
        }
        else
        ...
        if (!$summaryOnly)
        {
            ...
            if($columns == 0)
            {
                while ($trans = db_fetch($res))
                {
                    ...
                    //   work cust_allocation# 2
                    if($show_all == 0) {
                        $to = date2sql($to);
                        $sql = "SELECT SUM(amt) as Amount FROM ".TB_PREF."cust_allocations WHERE trans_no_to = ".db_escape($trans['trans_no'])."
                                AND trans_type_to = ".db_escape($trans['type'])."
                                AND date_alloc <= '$to'";
                        $query = db_query($sql, " get allocation amount date wise error");
                        $fetch = db_fetch($query);
                        $trans["Balance"] = $trans["Balance"] - $fetch['Amount']; // work
                    }
                    foreach ($trans as $i => $value)
                        $trans[$i] *= $rate;
                    {
                        
                        $str = array($trans["Balance"] - $trans["Due"]/* - $fetch['Amount']*/,
                            $trans["Due"] - $trans["Overdue1"],
                            $trans["Overdue1"] - $trans["Overdue2"],
                            $trans["Overdue2"],
                            $trans["Balance"]/* - $fetch['Amount']*/);
