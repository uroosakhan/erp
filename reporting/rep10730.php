...
function print_invoices()
{
    ...
    $cols = array(4, 50,150,200, 250, 310, 350, 390,430, 470);
    ...
    while($row = db_fetch($range))
    {
        ...
        $rep->SetCommonData($myrow, $branch, $sales_order, $baccount, 10730, $contacts);
...
$csv = str_replace('MICRON', 'MIC', $item['text2']);
                ...
                if($item['units'] != $myrow2['units_id'] ){
                    ...
                    $rep->TextCol(5, 6,	$myrow2["quantity"]  , -2);
                }
                $rep->TextCol(6, 7,	$myrow2["amount3"]  , -2);

                $rep->TextCol(8, 9,	number_format2($myrow2["unit_price"]), -2);
                	$rep->TextCol(7, 8,	$myrow2['units_id'], -2);
        ...
        if ($myrow['ov_freight'] != 0.0)
        {
            ...
            $rep->TextCol(5, 6, _("Shipping"), -2);
            $rep->Amountcol(9, 10,	$DisplayFreight, -2);
            $rep->NewLine();
        }
        if($myrow['discount1']) {
            $rep->TextCol(5, 6, _("Discount 1"), -2);
            $rep->Amountcol(9, 10, $myrow['discount1'], $dec);
            $rep->NewLine();
        }
        if($myrow['discount2']) {
            $rep->TextCol(5, 6, _("Discount 2"), -2);
        ...
        while ($tax_item = db_fetch($tax_items))
        {
            ...
            $total_tax_val += $sign*$tax_item['amount'];
...
//                    $rep->TextCol(8, 9,	$DisplayTax, -2);
        ...
        $total_inv_value = $SubTotal + $total_tax_val + $myrow["ov_freight"] - ($myrow['discount1'] + $myrow['discount2']);
          $rep->NewLine();
              $rep->TextCol(5, 7,	"Total Amount ", -2);
               $rep->TextCol(9, 10,number_format2($total_inv_value,2) , -2);
        $pref=get_company_prefs();
        $rep->NewLine();
...
//        $rep->MultiCell(400, 25, "Signature:__________________",0, 'L', 0, 2, 35,745, true);
//        $rep->MultiCell(400, 25, "Customer Signature:__________________",0, 'L', 0, 2, 350,745, true);
        ...
        $rep->Font('');
        $rep->MultiCell(451, 20, $sales_order['term_cond'], 0, 'L', 0, 2, 151, 718, true);
        $rep->font('b');
