...
function get_workworder_id($from, $to)
{
    $date_after = date2sql($from);
    $date_before = date2sql($to);

    $sql = "SELECT * FROM ".TB_PREF."workorders ";
	$sql .=  " WHERE date_ >= '$date_after'"
	        ." AND date_ <= '$date_before'
	        ORDER BY id DESC";
...
// 	for ($i = $from; $i <= $to; $i++)
    $result1 = get_workworder_id($from, $to);
    while($i = db_fetch($result1))
	{
		$myrow = get_work_order($i['id']);
//	    if ($myrow === false)
//	    	continue;
		...
		$result = get_wo_requirements($i['id']);
...
// 		$rep->NewLine(2);
		...
		$memo = get_comments_string(ST_WORKORDER, $i['id']);
		if ($memo != "")
		{
			$rep->NewLine();
		...
		if ($email == 1)
		{
		    ...
		    $rep->multicell(50,22,$qty,1,'R',0,1,290,694);
    $rep->multicell(100,22,$tot_qty_cons,1,'R',0,1,340,694);
    $rep->multicell(60,22,price_format($amt),1,'R',0,1,440,694);
	}
