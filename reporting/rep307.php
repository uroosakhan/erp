...
function inventory_movements()
{
 ...
 if($qty_ctn==1)
             {
                 	$cols = array(0, 60, 220, 240, 300, 360, 410, 460,520);
	$headers = array(_('Category'), _('Description'),	_('UOM'),	_('Opening'), _('Purchase'), _('Sales'), _('Balance'), _('Ctn./Packing'));
...
//display_error($db_connections[$_SESSION["wa_current_user"]->company]["name"]);
	...
	while ($myrow=db_fetch($result))
	{
		...
		if ($no_zeros && $qoh_end == 0) continue;
		...
		$packing=get_packing_valu($myrow['stock_id']);
		$con_factor=get_factor_valu($myrow['stock_id']);
		global $db_connections;
		if ($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'SHAMIM') 
		$balance=$qoh_end/$con_factor;
		else
		$balance=$qoh_end/$packing;
	 	if($qty_ctn==1)
             {
		$rep->AmountCol(7, 8,$balance, get_qty_dec($myrow['stock_id']));
             }
