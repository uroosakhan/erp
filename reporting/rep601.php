...
// 	$orientation = ($orientation ? 'L' : 'P');
	...
	$cols = array(0, 80, 100, 185, 280, 350, 550, 640, 700);

	$aligns = array('left',	'left',	'left',	'left',	'left',	'left', 'left', 'left', 'right');

	$headers = array(_('Type'),	_('#'),	_('Reference'), _('Cheque No.'), _('Date'), _('Person/Item'),
		...
		if ($rows > 0)
		{
			...
			global $db_connections;
                if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'ALI')
                {
                    ...
                    $rep->TextCol(5, 6, get_counterparty_name($myrow["type"], $myrow["trans_no"], false));
				...
				{
					$rep->NewLine();
				$rep->TextColLines(5, 6, $memo, -2);
				}
				if($myrow['type']==4)
				$rep->TextColLines(5, 6, get_fund_memo_($myrow['trans_no']), -2);
