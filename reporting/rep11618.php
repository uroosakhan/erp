...
function print_invoices()
{
	...
	$cols = array(1, 30, 160, /*170, 210, 250,*/ 210, 280, 340,390,440, 550);
	$cols2 = array(1, 30, 160, /*170, 210, 250, */ 210, 280, 340,390,440, 550);
...
//					$rep->TextCol(3, 4,	$myrow2['units'], -2);
                    ...
                    $amt_2 = get_tax_rate($myrow2['debtor_trans_no'], 3);
                    $amt_3= get_tax_rate($myrow2['debtor_trans_no'], 4);
                    $amt_4= get_tax_rate($myrow2['debtor_trans_no'], 5);
                    $sales_tax1 = ($amt_1['rate']/100) * $DisplayPq;
                    $sales_tax2 = ($amt_2['rate']/100) * $DisplayPq;
                    $sales_tax3 = ($amt_3['rate']/100) * $DisplayPq;
                    $sales_tax4 = ($amt_4['rate']/100) * $DisplayPq;
                    if($amt_1['rate'])
                        $Tax1 = $amt_1['rate']."%";
                    else
                        $Tax1 = "     ";
...
//					$rep->TextCol(6, 7,	$Tax3, -2);
					...
					if($sales_tax1)
					$rep->TextCol(4, 5,	price_format($sales_tax1), -2);
                    else
                        $rep->TextCol(4, 5,	"       ", -2);
					if($sales_tax2)
					$rep->TextCol(5, 6,	price_format($sales_tax2), -2);
					else
                        $rep->TextCol(5, 6,"       ", -2);
                    if($sales_tax3)
					$rep->TextCol(6, 7,price_format($sales_tax3), -2);
                    else
                        $rep->TextCol(6, 7,"       ", -2);

                    if($sales_tax4)
                        $rep->TextCol(6, 7,price_format($sales_tax4), -2);
                    else
                        $rep->TextCol(6, 7,"       ", -2);
...
$rep->TextColLines(1, 2, $myrow2['StockDescription'], -2);
	...
	//	$rep->NewLine();
		...
		if($tot_sales_tax1)
			$rep->AmountCol(4, 5,($tot_sales_tax1), 2);
		else
            $rep->TextCol(4, 5,"", 2);
        if($tot_sales_tax2)
			$rep->AmountCol(5, 6,($tot_sales_tax2), 2);
        else
            $rep->TextCol(5, 6,"", 2);
        if($tot_sales_tax3)
			$rep->AmountCol(6, 7,($tot_sales_tax3), 2);
        else
            $rep->TextCol(6, 7,"", 2);
