...
function get_cust_amt1($trans_no)
{
    //SELECT `trans_no_to` FROM `0_supp_allocations` WHERE `trans_no_from`=6 AND `trans_type_from`=22 AND `trans_type_to`=20 
    $sql = "SELECT trans_no_to FROM ".TB_PREF."supp_allocations WHERE trans_no_from = ".db_escape($trans_no)."
    
    AND trans_type_from=22 AND trans_type_to =20";
...
function get_sup_ref($trans_no)
{
    //SELECT `trans_no_to` FROM `0_supp_allocations` WHERE `trans_no_from`=6 AND `trans_type_from`=22 AND `trans_type_to`=20
    $sql = "SELECT trans_no_to FROM ".TB_PREF."supp_allocations WHERE trans_no_from = ".db_escape($trans_no)."
    
    AND trans_type_from=22 AND trans_type_to =0";
    $result = db_query($sql, "error");
    $row = db_fetch_row($result);
    return $row[0];
}
...
{
    $sql = "SELECT bt.*, act.*,st.reference,st.trans_no AS Trans_no,
...
function get_jv_ref($trans_no)
{
    $sql= "SELECT reference FROM " . TB_PREF . "journal WHERE type = 0 AND trans_no =" . db_escape($trans_no);
    $result = db_query($sql, "could not process Requisition to Purchase Order");
    $row = db_fetch_row($result);
    return $row[0] ;
}
...
function print_Chart_of_Accounts()
{
    ...
    $cols = array(2, 50, 199, 380, 440, 480);
...
//    $rep->getFontSize(12);
    ...
    $rep->MultiCell(160, 20, "Ref:   ". $from_trans["ref"] ,0, 'L', 0, 2, 50,90, true);

    $rep->MultiCell(160, 20, "Invoice No : ".get_cust_amt1($from_trans['trans_no']) ,0, 'L', 0, 2, 435,155, true);

$ref = get_sup_ref($from_trans['trans_no']);
    $rep->MultiCell(160, 20, "Journal Entry:   ".get_jv_ref($ref) ,0, 'L', 0, 2, 50,110, true);
...
//        $rep->MultiCell(160, 20, "Number :  ". $res["ref"] ,0, 'L', 0, 2, 50,125, true);
...
//        alt_table_row_color($k);
    ...
    /* if($from_trans['type']==22){
        ...
        //   $rep->MultiCell(220, 20,  ""."Voucher".get_cust_amt1($myrow['trans_no']) ,0, 'C', 0, 2, 190,70, true);
    ...
    $rep->MultiCell(40, 15,"".number_format2($dr),1, 'R', 0, 2, 479.5,516, true);
    $rep->MultiCell(45, 15," ".number_format2($dr),1, 'R', 0, 2, 520,516, true);
 global  $db_connections;
    ...
    $rep->MultiCell(150, 25, "___________________" ,0, 'L', 0, 2, 170,755, true);
