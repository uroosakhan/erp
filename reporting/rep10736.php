...
function get_sales_man_name($user_id)
{
	$sql = "SELECT salesman  FROM ".TB_PREF."cust_branch WHERE debtor_no=".db_escape($user_id);
...
if($pref['batch'] == 1){
	...
	$aligns = array('left',	'left',	'left',	'left',	'right', 'center', 'right', 'right', 'right');
      ...
      //-------------code-Descr-Qty--uom--tax--prc--Disc-Tot--//
	$cols = array(4, 60,205, 255, 325, 345, 385, 450, 515);
...
//                $item=get_item($myrow2['stk_code']);
		...
		if($myrow['discount1']) {
			$rep->TextCol(4, 7, _("Discount 1"), -2);
			$rep->Amountcol(7, 8, $myrow['discount1'], $dec);
			$rep->NewLine();
		}
		if($myrow['discount1']) {
			$rep->TextCol(4,7 , _("Discount 2"), -2);
