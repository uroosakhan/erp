...
function get_items_partno($type_no)
{
    $sql = "SELECT description,text3 
    FROM ".TB_PREF."debtor_trans_details
    WHERE debtor_trans_no = ".db_escape($type_no)."
    AND debtor_trans_type IN(10,13)    ";
    $result = db_query($sql, 'Error');
...
function get_cust_id_704($type_no)
{
    $sql = "SELECT person_id 
...
function get_cust_name_704($cust_id)
{
    $sql = "SELECT name FROM ".TB_PREF."debtors_master WHERE debtor_no = ".db_escape($cust_id)."";
    $result = db_query($sql, 'Error');
    $fetch = db_fetch_row($result);
    return $fetch[0];
}
		...
		if ($rows > 0)
		{
			while ($myrow=db_fetch($trans))
			{
                ...
                elseif($myrow["type"] == 12){
                     ...
                     $cust_id = get_cust_id_704($myrow['type_no']);
                    $customer_name = get_cust_name_704($cust_id);
                    $rep->TextCol(3, 4, $systypes_array[$myrow["type"]]);
                    $rep->NewLine();
                    $rep->TextCol(3, 4, $customer_name);
                    $rep->NewLine(-1);
...
$items =get_items_partno($myrow['type_no']);
 $memo = get_comments_string($myrow['type'], $myrow['type_no']);
           ...
           if ($myrow['type']==10 || $myrow['type']==13){
                 ...
                 $rep->NewLine(2);
                $rep->TextCol(0, 10, "Description:".$items['description']."         Part# ".$items['text3'], -2);
