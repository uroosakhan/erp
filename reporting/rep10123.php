...
function get_transactions($debtorno, $from, $to,$dimension,$dimension2, $sys_type )
{
   $from = date2sql($from);
   $to = date2sql($to);
...
function get_transactions2($debtorno, $from, $to, $transno)
{
   ...
   AND ".TB_PREF."debtor_trans_details.debtor_trans_type IN( 10,11)
...
function print_customer_balances()
{
   ...
   if ($no_zeros) $nozeros = _('Yes');
    ...
    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DMNWS') {
        ...
        $cols = array(0, 80, 170, 260,  260, 275, 290, 335, 390, 450, 520, 600);
        $headers = array(_('No.'), _('Date/Narration'), _(''), _('Rate'), _(''), _(''), _(''), _('Dr'), _('Cr'), _('Balance'));
        $aligns = array('left', 'left', 'left', 'left', 'right', 'right', 'right', 'right', 'right', 'right');

    }
else
{
    $cols = array(0, 80, 150, 200,  230, 260, 290, 335, 390, 450, 520, 600);
    $headers = array(_('No.'), _('Date/Narration'), _(''), _('Rate'), _('Qty'), _(''), _(''),  _('Dr'), _('Cr'), _('Balance'));
    ...
    $rep = new FrontReport(_('Customer Balances - Detailed'), "CustomerBalances", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
   ...
   while ($myrow = db_fetch($result))
   {
      ...
      $res = get_transactions($myrow['DebtorNo'], $from, $to,$dimension,$dimension2, $sys_type);
      ...
      $rep->fontSize -= 2;
      $rep->TextCol(3, 8,    _("Open Balance"));
...
$rep->NewLine(1);
            ...
            $rep->TextCol(0, 4, $trans2['description'],$dec);
                    ...
                    if($db_connections[$_SESSION["wa_current_user"]->company]["name"] == 'DMNWS') {
                         ...
                         $rep->TextCol(2, 3, $trans2['text1'], $dec);
