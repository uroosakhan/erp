...
function print_po()
{
    ...
    $cols = array(4, 50,150,200, 280, 330, 375, 400,435);
    ...
    for ($i = $from; $i <= $to; $i++)
    {
        ...
        $rep->SetCommonData($myrow, null, $myrow, $baccount, ST_SALESQUOTE, $contacts);

        $rep->SetHeaderType('Header11116');
        $rep->NewPage();
...
// 				$rep->TextCol(8, 9,$myrow['curr_code'], -2);
        ...
        $rep->NewLine();
//   display_error($prices[0]."/".$prices[1]);
        ...
        $words = price_in_words($SubTotal, ST_SALESQUOTE);
