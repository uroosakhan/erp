...
function getTransactions($loc_frm, $loc_to, $from, $to)
{
    $from = date2sql($from);
    $to = date2sql($to);
    $sql = "SELECT ".TB_PREF."stock_moves.stock_id, ".TB_PREF."stock_moves.qty,".TB_PREF."stock_moves.reference," .TB_PREF."stock_moves.con_factor, " .TB_PREF."stock_moves.amount3,
...
function print_inventory_sales()
{
	...
	$cols = array(0, 55, 120, 310,350,390,  420, 460,	520);
    if($uom == 1)
	    $headers = array(_('Item Code'), _('Reference'), _('Description'), _('Units'), _('Pry Qty'), _('Sec Qty'), _('Price'), _('Amount'));
	else
        $headers = array(_('Item Code'), _('Reference'), _('Description'), _(''), _('Quantity'), _(''), _('Price'), _('Amount'));
    ...
    if($uom == 1)
    $rep->AmountCol(3, 4, abs($trans['amount3']), $dec);
    $rep->AmountCol(4, 5, abs($trans['qty']), $dec);
        if(!user_check_access('SA_ITEMSPRICES')) {
            $rep->AmountCol(6, 7, $trans['material_cost'], $dec);
            $rep->AmountCol(7, 8, $trans['qty'] * $trans['material_cost'], $dec);
        }
$sec_aty = $trans['qty'] * $trans['con_factor'];
        if($uom == 1)
            $rep->AmountCol(5, 6, $sec_aty, $dec);

        $amt_tot += $trans['qty'] * $trans['material_cost'];
        $qt_tot += abs($trans['qty']);
        $sec_tot += abs($trans['qty'] * $trans['con_factor']);
        $units_total += abs($trans['amount3']);
    ...
    if($uom == 1)
        $rep->AmountCol(3, 4, $units_total, $dec);
    $rep->AmountCol(4, 5, $qt_tot, $dec);
    if($uom == 1)
        $rep->AmountCol(5, 6, $sec_tot, $dec);
    if(!user_check_access('SA_ITEMSPRICES')) {
        $rep->AmountCol(7, 8, $amt_tot, $dec);
    }
$rep->font('');
    $rep->Line($rep->row  - 4);
    $rep->End();
