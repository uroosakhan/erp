...
//----------------------------------------------------------------------------------------------------
function get_user_name_20999($user_id)
{
...
function get_user_id_time20999($trans_no,$type)
{
...
function get_purchase_req_no($reference)
{
    $sql = "SELECT  pr FROM ".TB_PREF."purch_orders
...
function get_item_description_name($item_code)
{
...
{
	$sql = "SELECT ".TB_PREF."grn_items.*, units,".TB_PREF."grn_items.text1
...
function print_po()
{
    ...
    $cols = array(4, 20, 70, 310,380,430,500,510);
	...
	for ($i = $from; $i <= $to; $i++)
	{
        ...
        $myrow = get_grn_($i);
        $remarks = $myrow['Comments2'];
		$baccount = get_default_bank_account($myrow['curr_code']);
		$params['bankaccount'] = $baccount['id'];
        $user =get_user_id_time20999($myrow['id'],ST_SUPPRECEIVE);
		...
		$rep->SetHeaderType('Header208');
		$rep->currency = $cur;
		$rep->Font();
        $rep->Info($params, $cols, $headers, $aligns, null, $headers2);

		$contacts = get_supplier_contacts($myrow['supplier_id'], 'order');
		$rep->SetCommonData($myrow, null, $myrow, $baccount, ST_SUPPRECEIVE, $contacts);
		$rep->NewPage();
...
//            $rep->TextColLines(1, 2, $myrow2['description'], -2);
            ...
            if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !isset($no_zero_lines_amount) || $no_zero_lines_amount == 0)
            {
                ...
                $purpose = get_purpose($myrow2['po_detail_item']);
...
//				$rep->TextCol(3, 4,	$myrow2['units'], -2);
                ...
                $rep->TextCol(3, 4,	number_format2($qoh,get_qty_dec($myrow2['item_code'])), -2);
				$rep->TextCol(4, 5,	number_format2($myrow2['qty_recd'],get_qty_dec($myrow2['item_code'])), -2);
                $rep->TextCol(5, 6,	number_format2($qoh + $qty,get_qty_dec($myrow2['item_code'])), -2);
//                $rep->TextCol(4, 5,	$DisplayNet, $dec);
                $rep->TextCol(6, 7,		get_unit($myrow2['item_code']), $dec);
//                $rep->NewLine(1);
                $rep->TextCollines(2, 3,	 get_item_description_name($myrow2['item_code']), -2);
                if($myrow2['text1'] != '')
...
//            $rep->NewLine();
        ...
        $rep->Font('bold');
        $rep->TextCol(2, 3, _("Total Quantity"), - 2);
      //  $rep->AmountCol(4, 5, $TotalGrossAmount, $dec);
       ...
       // $rep->AmountCol(7, 8,$total_sales_tax_other, $dec);
        // $rep->AmountCol(5, 6, number_format2(($qty_total),get_qty_dec($myrow2['item_code'])), $dec);
         $rep->MultiCell(70, 15,number_format2(($qty_total),get_qty_dec($myrow2['item_code'])), 0, 'R', 0, 2, 397,633, true);
        $rep->NewLine(2);
...
//   $array =explode(' ', $user['stamp']);
    ...
    $rep->MultiCell(225, 60,get_user_name_20999($user['user'])." ".  sql2date(date("Y-m-d", $user['unix_stamp']))/*." ".  date("H:i:s", $user['unix_stamp'])*/, 0, 'L', 0, 2, 45,740, true);
    ...
    $rep->MultiCell(225, 60, "_______________________________" , 0, 'L', 0, 2, 225,750, true);
    $rep->MultiCell(225, 60, "Checked by"  , 0, 'L', 0, 2, 275,770, true);
    $rep->MultiCell(225, 60, "_______________________________" , 0, 'L', 0, 2, 410,750, true);
    $rep->MultiCell(225, 60, "Approved by"  , 0, 'L', 0, 2, 460,770, true);
    if ($email == 0)
		$rep->End();
}
