...
function print_Chart_of_Accounts()
{
	...
	$cols = array(2, 50, 180, 380, 440, 430);

	$headers1 = array(_('Code'), _('Name'), _('Narration'),_('Centre'), _('Rs'), _('Rs'));
...
//    $rep->MultiCell(60, 20, sql2date($from_trans['trans_date']),0, 'C', 0, 2, 500,100, true);
    $rep->MultiCell(350, 20, convert_number_to_words($from_trans['amount']*-1),1, 'C', 0, 2, 60,340, true);
...
//    $rep->getFontSize(12);
    ...
    $rep->MultiCell(160, 20, "Number :  ". $from_trans["trans_no"] ,0, 'L', 0, 2, 50,80, true);

    $rep->MultiCell(160, 20, "Debit Code:  ".$from_trans['account_code']." - ". $from_trans['bank_account_name'] ,0, 'L', 0, 2, 50,140, true); //  $from_trans['person_id']$from_trans['account_code']." - ". $from_trans['bank_account_name']
...
//    $k = 0;
     $res=get_gl_trans11($from_trans['type'],$from_trans['trans_no']);
        ...
        while ($myrow1 = db_fetch($res)) {
            ...
            $rep->TextCol(2, 3,$myrow1['memo_'], -2);
...
//        alt_table_row_color($k);
    ...
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 45,380, true);
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 170,380, true);
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 300,380, true);
    $rep->MultiCell(400, 25, "_________________" ,0, 'L', 0, 2, 400,435, true);
    $rep->MultiCell(100, 25, "Prepered By" ,0, 'C', 0, 2, 40,400, true);
    $rep->MultiCell(100, 25, "Approved  By" ,0, 'C', 0, 2, 170,400, true);
    $rep->MultiCell(100, 25, "Checked  By" ,0, 'C', 0, 2, 290,400, true);
    $rep->MultiCell(100, 25, "Recieved  By" ,0, 'C', 0, 2, 390,450, true);
