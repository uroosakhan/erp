...
function print_invoices()
{
	...
	$cols = array(4, 60, 250, 280, 365, 440, 450, 515);
	...
	while($row = db_fetch($range))
	{
		...
		$total_discount=0;
			while ($myrow2=db_fetch($result))
			{
	    		...
	    		$DisplayNet = $myrow2["unit_price"] * $myrow2["quantity"];
...
//                $item=get_item($myrow2['stk_code']);
                   /* if($pref['alt_uom'] == 1)
                    ...
                    {
                        $rep->TextCol(3, 4, $myrow2['units_price'], -2);
                    }*/
                    
                    $rep->AmountCol(3, 4, $myrow2['unit_price'], $dec);
				// 	$rep->TextCol(4, 5,	$DisplayPrice, -2);
				// 	$rep->TextCol(5, 6,	$DisplayDiscount, -2);
					$rep->AmountCol(6, 7,	$DisplayNet, $dec);
					$tot_net += $Net;
				}
				$rep->row = $newrow;
				//$rep->NewLine(1);
...
// 			$rep->TextCol(6, 7,	$DisplaySubTot, -2);
    ...
    // 			$rep->TextCol(6, 7, $previous_balance - $DisplayTotal , - 2);

		    $rep->Font();
			$rep->NewLine();
			$rep->Font('bold');
				$rep->TextCol(3, 6, _("Balance Due") .' '.'(' .$rep->formData['curr_code'].')', - 2);
				$rep->TextCol(6, 7, number_format2($previous_balance,$dec), - 2);

			if ($rep->formData['prepaid'])
			{
				// $rep->NewLine();
				// $rep->Font('bold');
				// $rep->TextCol(3, 6, $rep->formData['prepaid']=='final' ? _("THIS INVOICE") : _("TOTAL"), - 2);
...
// 		$rep->MultiCell(100, 25, "".get_user_name_70123($user) ,0, 'C', 0, 2, 125,770, true);
...
// 			$rep->NewPage();
// 		$rep->AddImage($image, $rep->cols[1] +300, $rep->row +580, null,$rep->company['logo_w'], $rep->company['logo_h']);
// 		$rep->AddImage($imageheader, $rep->cols[1] -55, $rep->row -170, 510,20, $SysPrefs->pic_height);
...
//     	$user =get_user_id($row['trans_no'],ST_SALESINVOICE);
        $rep->Font('bold');
		$rep->MultiCell(100, 25,"Terms:" ,0, 'L', 0, 2, 40,590, true);
		$rep->MultiCell(100, 25,"Notes:" ,0, 'L', 0, 2, 40,710, true);
		$rep->MultiCell(300, 25,"Balance Due".' '.'(' .$rep->formData['curr_code'].')' ."                          ". number_format2($previous_balance,$dec),0, 'L', 0, 2, 340,175, true);
        $rep->Font('');
		$rep->MultiCell(250, 110,$myrow['term_cond']  ,0, 'L', 0, 2, 40,600, true);
        $rep->MultiCell(250, 110,$myrow['f_text1']  ,0, 'L', 0, 2, 40,720, true);
