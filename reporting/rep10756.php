...
function print_invoices()
{
	...
	$cols = array(4, 60, 225, 300, 390, 450);
...
//					$rep->TextCol(5, 6,	$DisplayDiscount, -2);
			...
			{
				$rep->NewLine();
                $rep->multicell(400,40,$memo,0,'L',0,0,120,575);
...
//				$rep->TextCol(2, 3, _("Shipping"), -2);
                $rep->multicell(525,20,"Shipping:",0,'L',0,0,322,456);

                $rep->Amountcol(5, 6,	$DisplayFreight, -2);
				$rep->NewLine(+14);
			}
		if($myrow['discount1']) {
            $rep->NewLine(-13);
//			$rep->TextCol(2, 3, _("   Discount 1"), -2);
            $rep->multicell(525,20,"Discount 1:",0,'L',0,0,322,470);

            $rep->Amountcol(5, 6, $myrow['discount1'], $dec);
			$rep->NewLine(+13);
		}
		...
		if($myrow['discount2']) {
            $rep->NewLine(-15.5);
            $rep->multicell(525,20,"Discount 2:",0,'L',0,0,322,470);
			$rep->Amountcol(5, 6, $myrow['discount2'], $dec);
//            $rep->multicell(60,35, $myrow['discount2'],1,'R',0,0,505,451);
            $rep->NewLine();
		}
//		else{
//            $rep->multicell(60,35,number_format2($tot_net,get_qty_dec($myrow2['stock_id'])-$DisplayFreight),0,'R',0,0,505,484);
//
//        }
//        display_error($tot_net);
//        display_error($DisplayFreight);
//        display_error($myrow["ov_gst"]);
//        $rep->multicell(60,35,number_format2($tot_net,get_qty_dec($myrow2['stock_id'])-$DisplayFreight),0,'R',0,0,505,520);
    		...
    		while ($tax_item = db_fetch($tax_items))
    		{
    			...
    			if ($myrow['tax_included'])
    			{
    				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
    				{
						...
						$rep->TextCol(2, 4, _("Included") . " " . $tax_type_name . _("Amount") . ": " . $DisplayTax, -2);
				}
    			else
    			{
                    $rep->NewLine(-15);
        ...
        $rep->multicell(60,16,number_format2($tot_balance,$dec),1,'R',0,0,505,505);
         $rep->font('b');
        $rep->multicell(220,15,"Opening Balance:",1,'L',0,0,345,187);
        $rep->multicell(500,10,"Good once sold are not returnable. We warrant to the extent of purchase price that the seeds we sell according to quality standard as describe .We give no other gurantee expressed or implied and will not be responsible for the crop failure due to factor beyond our extent.",0,'L',0,0,100,770);
        $rep->multicell(300,10,"N.B. In case of any confusion, please contact immediately.",0,'L',0,0,40,670);
        $rep->font('');
          $rep->multicell(100,15,"".number_format2($previous_balance,$dec),1,'R',0,0,464,187);

        //   $rep->multicell(200,10,$sales_order['reference'],0,'L',0,0,424,145);

        $rep->setfontsize(12);
        $rep->multicell(160,20,get_shipping_through($myrow['ship_via']),0,'L',0,0,152,530);
          
        $rep->multicell(170,20," ".get_user_name_1($prepared_by),0,'L',0,0,117,730);
            $rep->setfontsize(8);
    		$rep->NewLine();
			$DisplayTotal = number_format2($sign*($myrow["ov_freight"] + $myrow["ov_gst"] +
				$myrow["ov_amount"]+$myrow["ov_freight_tax"] - ($myrow['discount1'] + $myrow['discount2'])),$dec);
//        $rep->multicell(60,35,number_format2($tot_net + $DisplayFreight + $myrow["ov_gst"]  - $myrow['discount1'] - $myrow['discount2']),0,'R',0,0,503,484);



        $rep->Font('bold');
        $rep->multicell(60,35,($DisplayTotal),0,'R',0,0,503,484);
