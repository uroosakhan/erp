...
//else{
      //-------------code-Descr-Qty--uom--tax--prc--Disc-Tot--//
	$cols = array(4, 40, 280, 340, 430);
	$cols2 = array(4, 40, 280, 340, 430);

	// $headers in doctext.inc
	$aligns = array('left',	'center', 'center', 'center', 'center');
	$aligns2 = array('left', 'center', 'center', 'center', 'center');
...
//}
	...
	while($row = db_fetch($range))
	{
			...
			$rep->currency = $cur;
			$rep->Font();
			$rep->Info($params, $cols, null, $aligns, $cols2, $aligns2);

			$contacts = get_branch_contacts($branch['branch_code'], 'invoice', $branch['debtor_no'], true);
			$baccount['payment_service'] = $pay_service;
			$rep->SetCommonData($myrow, $branch, $sales_order, $baccount, ST_SALESINVOICE, $contacts);
			$rep->SetHeaderType('Header10720');
			$rep->NewPage();
...
//				$rep->TextCol(2, 3,	number_format2($tot_pym, $dec));
		...
		$rep->NewLine(-3);
		$rep->TextCol(3, 4, _("Sub Total PKR "), -2);
		$rep->TextCol(4, 5,	$SubTotal, -2);
		$rep->NewLine(+1.2);
		$rep->TextCol(3, 4, _("SST @ 13% PKR: "), -2);
		$rep->NewLine(+1.2);
		$rep->TextCol(3, 4, _("Grand Total Amount PKR "), -2);
