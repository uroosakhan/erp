...
function get_discount_offer_details($order_no)
{
    $sql = "SELECT * FROM ".TB_PREF."discount_offer_details WHERE order_no = ".db_escape($order_no);
...
function print_invoices()
{
	...
	$cols = array(4, 60, 145, 170, 215, 255, 280, 315, 345, 395, 450, 520);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'left', 'left', 'right', 'left', 'right', 'right', 'right');
	...
	while($row = db_fetch($range))
	{
			...
			while ($myrow2=db_fetch($result))
			{
				...
				{
					$rep->TextCol($c++, $c,	$myrow2['text1'], -2);
					$rep->TextCol($c++, $c,	$myrow2['amount6'], -2);
					$rep->TextCol($c++, $c,	$myrow2['text6'], -2);
					if($myrow2['combo4'] == 1)
					    $combo4 = "V";
					elseif($myrow2['combo4'] == 2)
					    $combo4 = "Q";
					$rep->TextCol($c++, $c,	$combo4, -2);
            ...
            $DiscountOffer = get_discount_offer_details($row['trans_no']);
			if(db_num_rows($DiscountOffer) > 0)
            {
                $rep->NewLine();
                $rep->Font('bold');
                $rep->TextCol(0, 5,	"Less Amount Details", -2);
                $rep->NewLine();
                $rep->TextCol(0, 5,	"Description", -2);
                $rep->TextCol(5, 6,	"Values", -2);
                $rep->Font('');
                $LessAmount = 0.0;
                while ($myrow3=db_fetch($DiscountOffer))
                {
                    $rep->NewLine();
                    $rep->TextCol(0, 3,	get_sales_offers_1($myrow3['offers_id']), -2);
                    if($myrow3['flag'] == 0)
                        $rep->AmountCol(4, 4, $myrow3['rate'], $dec);
                    elseif($myrow3['flag'] == 1)
                        $rep->TextCol(4, 6, $myrow3['rate']."%", -2);
                    $rep->AmountCol(5, 6, $myrow3['amount1'], $dec);
                    $LessAmount += $myrow3['amount1'];
                }
                $rep->NewLine();
                $rep->Font('bold');
                $rep->TextCol(0, 2, _("Less-Amount"), -2);
                $rep->AmountCol(5, 6, $LessAmount, $dec);
                $rep->Font('');
            }
			...
			$rep->cols[3] += 20;
			$rep->aligns[3] = 'left';
            $rep->TextCol(9, 10, _("Sub-total"), -2);
            $rep->TextCol(10, 11,	$DisplaySubTot, -2);
            $rep->NewLine();
            $rep->TextCol(9, 10, _("Less-Amount"), -2);
            $rep->AmountCol(10, 11, $LessAmount, $dec);
            $rep->NewLine();

			if ($myrow['ov_freight'] != 0.0)
			{
   				$DisplayFreight = number_format2($sign*$myrow["ov_freight"],$dec);
				$rep->TextCol(9, 10, _("Shipping"), -2);
    		...
    		while ($tax_item = db_fetch($tax_items))
    		{
    			...
    			if ($myrow['tax_included'])
    			{
    				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
    				{
						...
						$rep->TextCol(9, 10, _("Included") . " " . $tax_type_name . _("Amount") . ": " . $DisplayTax, -2);
				}
    			else
    			{
					$rep->TextCol(8, 10, $tax_type_name, -2);
			...
			if (!$myrow['prepaid']) $rep->Font('bold');
				$rep->TextCol(9, 10, $rep->formData['prepaid'] ? _("TOTAL ORDER VAT INCL.") : _("TOTAL INVOICE"), - 2);
