...
//{
//    $sql = "SELECT * FROM ".TB_PREF."dimensions WHERE id=".db_escape($dimension_id);
//
...
if($pref['batch'] == 1){
	...
	$aligns = array('left',	'left',	'left',	'left',	'right', 'center', 'right', 'right', 'right');
      ...
      //-------------code-Descr-Qty--uom--tax--prc--Disc-Tot--//
	$cols = array(4, 60, 330, 380, 450, 515);
...
// 			}
        // $rep->MultiCell(770, 30, "sdf".$rep->formData["comments"], 0, 'L', 0,1,100,760, true);//second
			...
			while ($myrow2=db_fetch($result))
			{
				...
				$c=0;
				$rep->TextCol(0, 1,	sql2date($myrow2['date1']), -2);
				$oldrow = $rep->row;
...
// 		$rep->NewLine();
				...
				$rep->TextCol(0, 2, "".$sales_order['f_text10'], -2);

// 			$rep->MultiCell(400, 25, "".$sales_order['f_text10'],0, 'L', 0, 2, 45,400, true);
...
$rep->fontSize += 4;
		...
		$rep->Amountcol(6, 7, $total_amount, $dec);
		$rep->fontSize -= 4;
    			...
    			{	$rep->NewLine(-1);
                    ...
                    $rep->TextCol(3, 6, $tax_type_name, -2);
                        			$rep->NewLine( );
						                    ...
						                    $rep->TextCol(3, 5,	"Total Balance ", -2);

                        $rep->NewLine(6);

                        $rep->TextCol(1,2, $tax_type_name, -2);
                    $rep->TextCol(1, 3,	"                                                                                         ".$DisplayTax, -2);
						                    $rep->TextCol(3, 4,	"                                                                                            ". $total_amount - $DisplayTax, -2);
	...
	$pref=get_company_prefs();
        ...
        $a =get_dimension($myrow['dimension2_id']);
        
        // display_error($myrow['dimension2_id']);
        $c =get_comments_string(40,$myrow['dimension2_id']);
        $rep->Font('');
        $rep->MultiCell(240, 20,"".$sales_order['h_text4'] , 0, 'L', 0, 2, 50,320, true);
        $rep->MultiCell(240, 20,"".$sales_order['h_text3'] , 0, 'L', 0, 2, 250,320, true);
        $rep->MultiCell(240, 20,"".$sales_order['h_text2'] , 0, 'L', 0, 2, 450,320, true);
        
         $rep->Font('b');
         $rep->fontSize -= 7;

        $rep->MultiCell(240, 20,"".$a['name'] , 0, 'L', 0, 2, 50,40, true);
        $rep->fontSize -= 7;

        $rep->Font();
        $rep->MultiCell(240, 20,$c, 0, 'L', 0, 2, 50,50, true);
