...
function get_open_balance($debtorno, $to)
{
	if($to)
     ...
     $sql = "SELECT SUM(IF(t.type = ".ST_SALESINVOICE." OR (t.type = ".ST_JOURNAL." AND t.ov_amount>0),
     	-abs(t.ov_amount + t.ov_gst + supply_disc + service_disc + fbr_disc +  srb_disc+ t.ov_freight + 
     	t.ov_freight_tax + t.ov_discount - t.discount1 - t.discount2), 0)) AS charges,";
     $sql .= "SUM(IF(t.type != ".ST_SALESINVOICE." AND (t.type = ".ST_JOURNAL." AND t.ov_amount<0),
     	abs(t.ov_amount + t.ov_gst + supply_disc + service_disc + fbr_disc +  srb_disc + t.ov_freight +
     	 t.ov_freight_tax + t.ov_discount - t.discount1 - t.discount2) * -1, 0)) AS credits,";
    $sql .= "SUM(IF(t.type != ".ST_SALESINVOICE." AND NOT(t.type = ".ST_JOURNAL." AND t.ov_amount<0), t.alloc
     * -1, t.alloc)) AS Allocated,";

 	$sql .=	"SUM(IF(t.type = ".ST_SALESINVOICE.", 1, -1) *
 			(-abs(t.ov_amount + t.ov_gst + supply_disc + service_disc + fbr_disc +  srb_disc + t.ov_freight 
 			+ t.ov_freight_tax + t.ov_discount - t.discount1 - t.discount2) - abs(t.alloc))) AS OutStanding
...
function print_customer_balances()
{
	...
	$headers = array(_('Customer Name'), _(''), _(''), _(''), _(''), _(''),
		_('Total Balance'), 	_(''));
	...
	$sql = "SELECT dm.debtor_no, dm.name, dm.curr_code FROM ".TB_PREF."debtors_master dm,".TB_PREF."cust_branch cb
	 WHERE dm.debtor_no != 0 
	 AND dm.debtor_no = cb.debtor_no ";
	if($zone != 0 )
    {
        $sql.=" AND cb.area = ".db_escape($zone)." ";
