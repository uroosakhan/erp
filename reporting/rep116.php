...
function get_phoness($debtor_no)
{
	...
	$sql = "SELECT name,phone FROM `0_crm_persons` WHERE `id` IN (
...
function get_attn ($debtor_no)
{
	$sql = "SELECT attn FROM ".TB_PREF."debtors_master
...
//                $item=get_item($myrow2['stk_code']);
				...
				$DisplayTotal += $Net_amount + $sales_tax_amount+$myrow['ov_freight'];
		...
		$rep->AmountCol(5, 6,	$myrow['ov_freight'], $dec);
