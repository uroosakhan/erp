        ...
        $SubTotal2 = 0;
			...
			while ($myrow2=db_fetch($result))
			{
				...
				$Net = round2($sign * ((1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $myrow2["quantity"]),
				   user_price_dec());
				$SubTotal += $Net;
				$SubTotal2 += (1 - $myrow2["discount_percent"]) * $myrow2["unit_price"] * $myrow2["quantity"];
...
//                    display_error($myrow2['stock_id']);
        ...
        if($myrow['discount1'] != 0) {
            ...
            $rep->MultiCell(410, 30, "Discount" ,0, 'L', 0, 2, 340,650,  true);
        }

        if($myrow['discount2'] != 0) {

            $discount_value =$myrow["disc2"]/100;
            $rep->MultiCell(410, 30, "".price_format($myrow["discount2"]) ,0, 'R', 0, 2, 150,650, true);
            $rep->MultiCell(410, 30, "Discount" ,0, 'L', 0, 2, 390,650,  true);
...
//	     	$rep->NewLine();
        ...
        if ($myrow['tax_included'] == 0) {
            ...
            if ($pref['alt_uom'] == 1 && $myrow['sale_enable'] == 1) {
        ...
        $rep->Font('');
    		...
    		while ($tax_item = db_fetch($tax_items))
    		{
    			...
    			if ($tax_item['amount'] == 0)
    				continue;
//    			$DisplayTax = $sign*$tax_item['amount'];
                $tax_rate= $tax_item['rate']/100;
                $DisplayTax = number_format(($myrow["freight_cost"] + $SubTotal  - $myrow["discount1"] - $myrow["discount2"]) * $tax_rate,$dec);
                $tax =($myrow["freight_cost"] + $SubTotal  - $myrow["discount1"] - $myrow["discount2"]) * $tax_rate;
    			...
    			if ($myrow['tax_included'])
    			{
    				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
    				{
    					if ($first)
    					{
							...
							$rep->TextCol(7, 8,	$sign*$tax_item['net_amount'], -2);
							$rep->NewLine();
    					}
    			...
    			if($pref['batch'] ==1){
    			    	...
    			    	$rep->Amountcol(7, 8, $DisplayTax, $dec);
    			    	 $total_tax += $tax_item['amount'];
    ...
    // 		$rep->NewLine();
        $total_amount = abs($SubTotal-$myrow['discount1']-$myrow['discount2'] + $DisplayFreight + $total_tax );
        $total_amount2 = number_format2($myrow["freight_cost"] + $SubTotal2 - $myrow["discount1"] - $myrow["discount2"] + $tax , $dec);
        $rep->Font('bold');
        $rep->TextCol(3, 7,  _("TOTAL INVOICE"). ' (' . $rep->formData['curr_code'].')', -2);
		$rep->TextCol(7, 8, $total_amount2 , -2);
        $rep->Font('');
	$pref=get_company_prefs();
    		$rep->NewLine();
...
// 			$rep->TextCol(8, 9, $DisplayTotal + $total_tax, -2);
//             }
// 			if ($rep->formData['prepaid'])
// 			{
// 				$rep->NewLine();
