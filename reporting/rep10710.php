...
//----------------------------------------------------------------------------------------------------
function convert_number_to_words_10710($number) {
    ...
    if ($number < 0) {
        return $negative . convert_number_to_words_10710(abs($number));
    }
    ...
    switch (true) {
        ...
        case $number < 1000:
            ...
            if ($remainder) {
                $string .= $conjunction . convert_number_to_words_10710($remainder);
            }
            break;
        default:
            ...
            $string = convert_number_to_words_10710($numBaseUnits) . ' ' . $dictionary[$baseUnit];
            if ($remainder) {
                $string .= $remainder < 100 ? $conjunction : $separator;
                $string .= convert_number_to_words_10710($remainder);
            }
            break;
    }
...
if($pref['batch'] == 1){
	...
	$aligns = array('left',	'left',	'left',	'left',	'right', 'center', 'right', 'right', 'right');
      ...
      //-------------code-Descr-Qty--uom--tax--prc--Disc-Tot--//
	$cols = array(4, 60, 225, 350,  430, 450, 515);
...
//                $item=get_item($myrow2['stk_code']);
        ...
        $total_amount = abs($SubTotal-$myrow['discount1']-$myrow['discount2'] + + $tax_amt);

        $rep->TextCol(3, 6, _("Total Amount"), -2);
	...
	$pref=get_company_prefs();
			...
			if ($rep->formData['prepaid'])
			{
				...
				$rep->TextCol(4, 5, number_format2($myrow['prep_amount'], $dec), -2);
			}
        ...
        $user =get_user_id($row['trans_no'],ST_SALESINVOICE);
			$rep->MultiCell(400, 25, "Signature:__________________",0, 'L', 0, 2, 35,790, true);
			$rep->MultiCell(400, 25, "Customer Signature:__________________",0, 'L', 0, 2, 350,790, true);

// 		$rep->MultiCell(100, 25, "".get_user_name_70123($user) ,0, 'C', 0, 2, 65,785, true);
        $rep->MultiCell(500, 10,"Amount in Words: ".convert_number_to_words_10710($total_amount), 0, 'L', 0, 2, 40,725, true); // 3
...
//            $rep->TextColLines(0, 5, $memo, -2);
            $rep->MultiCell(400, 40,$memo, 0, 'L', 0, 2, 40,740, true); // 3
