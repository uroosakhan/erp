...
//     if($pref['alt_uom'] == 1  && $myrow233['sale_enable'] == 1){
	$cols = array(4, 40, 120, 190, 310, 350, 430);
...
//                    $rep->TextCol(6, 7,	$DisplayNet, -2);
        ...
        $rep->font('');
        $rep->MultiCell(50, 10,number_format($qty_total,get_qty_dec($myrow2['item_code'])) ,0, 'L', 0, 2, 350,500, true);
...
//		$rep->MultiCell(525, 30, "".$myrow['discount1'] ,0, 'L', 0, 2, 540,645, true);//S.no
	...
	$rep->MultiCell(525, 30, 'Please note: Above prices are based on quantity and prices may change as per quantity difference.' ,0, 'L', 0, 2, 40,530, true);
	$rep->MultiCell(525, 30, 'Hope the provided offer will meet your requirment and please do not hesitate to contact us if you need any further assistance or clarification.' ,0, 'L', 0, 2, 40,700, true);
	$rep->MultiCell(525, 30, 'Thanks & kind regards, ' ,0, 'L', 0, 2, 40,730, true);
		$rep->Font('bold');
		$rep->MultiCell(525, 30, get_salesman_name1111($myrow['salesman']),0, 'L', 0, 2, 40,750, true);
		$rep->Font();
		...
		$rep->setfont('','','16');
		$rep->font('b');
		$rep->MultiCell(525, 30, 'QUOTATION' , 0, 'L', 0, 2, 250,80, true);
		$rep->font();
		$rep->setfont('','','');
	}
	if ($email == 0)
