...
function get_invoice_($order_no)
{
   	$sql = "SELECT ".TB_PREF."supp_trans.*, ".TB_PREF."suppliers.supp_name, ".TB_PREF."suppliers.supplier_id,  "
   		.TB_PREF."suppliers.supp_account_no,".TB_PREF."suppliers.tax_included,".TB_PREF."suppliers.gst_no AS tax_id,
   		".TB_PREF."suppliers.curr_code, ".TB_PREF."suppliers.payment_terms, ".TB_PREF."locations.location_name,
   		".TB_PREF."suppliers.address, ".TB_PREF."suppliers.contact, ".TB_PREF."suppliers.tax_group_id
   		 ,".TB_PREF."supp_trans.supplier_id as dimensions
		FROM ".TB_PREF."supp_trans, ".TB_PREF."suppliers, 
...
function get_invoice_details_($order_no)
{
...
function print_po()
{
    ...
    $cols = array(4, 40, 100, 160, 270,355,410,480,520);
	...
	for ($i = $from; $i <= $to; $i++)
	{
        ...
        $myrow = get_invoice_($i);
        $a = get_supp_invoice_items(20,$i);
...
//            $rep->TextColLines(1, 2, $myrow2['description'], -2);
            ...
            if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !isset($no_zero_lines_amount) || $no_zero_lines_amount == 0)
            {
               ...
               $rep->TextCol(1, 2,	$myrow2['po_detail_item_id'], -2);
               // $serial_no++;
                $rep->TextCol(2, 3,	$myrow2['gl_code'], -2);
//				$rep->TextCol(3, 4,	$myrow2['units'], -2);
                $rep->TextCol(3, 4,	$a['description'], -2);
                $qoh = get_qoh_on_date_new($myrow2['item_code'], null,null );
                $rep->TextCol(4, 5,	$qoh, -2);
				$rep->TextCol(5, 6,	$myrow2['quantity'], -2);
//                $rep->TextCol(4, 5,	$DisplayNet, $dec);
                $rep->TextCol(7, 8, get_unit($myrow2['stock_id']), $dec);
