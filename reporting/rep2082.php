...
{
   	$sql = "SELECT ".TB_PREF."grn_batch.*, ".TB_PREF."suppliers.supp_name, ".TB_PREF."suppliers.supp_address, ".TB_PREF."suppliers.supplier_id,  "
   		.TB_PREF."suppliers.supp_account_no,".TB_PREF."suppliers.tax_included,".TB_PREF."suppliers.gst_no AS tax_id,
   		".TB_PREF."suppliers.curr_code, ".TB_PREF."suppliers.payment_terms, ".TB_PREF."locations.location_name,
   		".TB_PREF."suppliers.address, ".TB_PREF."suppliers.contact, ".TB_PREF."suppliers.tax_group_id
...
function print_po()
{
    ...
    $cols = array(4, 20, 60, 100, 300,375,400,500);

    // $headers in doctext.inc

    $aligns = array('center',	'center',	'center', 'left', 'center', 'center', 'center', 'center');
...
//            $rep->TextColLines(1, 2, $myrow2['description'], -2);
            ...
            if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !isset($no_zero_lines_amount) || $no_zero_lines_amount == 0)
            {
                ...
                $rep->TextCol(1, 2,	$myrow['purch_order_no'], -2);
                $serial_no++;

//                $rep->TextCol(2, 3,	$myrow2['item_code'], -2);

                $rep->TextCol(5, 6,	$myrow2['qty_recd'], -2);
                $qoh = get_qoh_on_date_new($myrow2['item_code'], null,null );
                $rep->TextCol(6, 7,	$qoh -2);
                $before_stock =$qoh - $qty;
                $rep->TextCol(4, 5,	$before_stock -2);
...
//			$rep->TextColLines(1, 5, $myrow['comments'], -2);
        ...
        $rep->row = $rep->bottomMargin + (60 * $rep->lineHeight);
        $doctype = ST_PURCHORDER;
        if($serial_no >= 2) {
            $rep->NewLine(45 - $serial_no);
        }
        else{
            $rep->NewLine(44);
        }
        $serial_no = 0;
        $result1 = get_grn_details_($i);
...
//            $rep->NewLine();
         ...
         $user =get_user_id($myrow['purch_order_no'],ST_PURCHORDER1);
    $rep->MultiCell(200, 25, "".get_user_name_1($user) ,0, 'L', 0, 2, 100,788, true);
        $rep->MultiCell(200, 25, "".get_user_name_1($user) ,0, 'L', 0, 2, 100,388, true);
        ...
        $rep->Font('bold');
            ...
            $rep->TextCol(5, 6, $qty_total1, - 2);
//        $rep->NewLine(+26.5);
        $rep->NewLine(+34);
        $rep->TextCol(3, 4, _("Total Quantity"), - 2);
        $rep->TextCol(5, 6, $qty_total, - 2);
//        $rep->NewLine(-33.9);
...
//    $rep->MultiCell(225, 60, "Authorized Signatures:" , 0, 'L', 0, 2, 40,750, true);
    $rep->Font('');
    $rep->MultiCell(225, 60, "_______________________________" , 0, 'L', 0, 2, 55,790, true);
    $rep->MultiCell(225, 60, "Prepared by"  , 0, 'L', 0, 2, 100,810, true);
    $rep->MultiCell(225, 60, "_______________________________" , 0, 'L', 0, 2, 420,790, true);
    $rep->MultiCell(225, 60, "Checked by"  , 0, 'L', 0, 2, 460,810, true);


    $rep->MultiCell(225, 60, "_______________________________" , 0, 'L', 0, 2, 55,390, true);
    $rep->MultiCell(225, 60, "Prepared by"  , 0, 'L', 0, 2, 100,400, true);
    $rep->MultiCell(225, 60, "_______________________________" , 0, 'L', 0, 2, 420,390, true);
    $rep->MultiCell(225, 60, "Checked by"  , 0, 'L', 0, 2, 460,400, true);
    if ($email == 0)
        $rep->End();
}
