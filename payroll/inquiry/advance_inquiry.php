...
if (!@$_GET['popup'])
{
	...
	page(_($help_context = "Advance Inquiry"), isset($_GET['employee_id']), false, "", $js);
}
...
advance_against_list(_("Advance Against:"), 'advance_against', null,true,true, false,  false);
...
function edit_link($row) 
{
  ...
  return pager_link( _("Edit"),
    "/payroll/manage/advance.php?id=".$row['id'].'&date='.$row['date'], ICON_EDIT);
}
function advance_against($row)
{
	if($row['advance_on_base_of']==1)
		$text = 'Gratuity';
	elseif($row['advance_on_base_of']==2)
		$text = 'Salary';
	elseif($row['advance_on_base_of']==3)
...
//------------------------------------------------------------------------------------------------
function get_sql_for_employee_inquiry($employee_id, $emp_dept, $emp_grade,$emp_desig,$approve, $datefrom,$dateto,$advance_against)
{
	$emp_grade =$_POST['grade'];
	$employee_id=$_POST['employee_id'];
	$emp_dept=$_POST['emp_dept'];
	$emp_desig=$_POST['emp_desig'];
	$advance_against=$_POST['advance_against'];

$sql = "SELECT ".TB_PREF."employee.emp_name,".TB_PREF."grade.description As grade,".TB_PREF."desg.description As desination,
".TB_PREF."dept.description,".TB_PREF."advance.date,".TB_PREF."advance.remarks, ".TB_PREF."advance.`amount` ,
".TB_PREF."advance.`advance_on_base_of`	,

IF(  `approve` =0,  'Unapprove',  'Approve' ) AS approve, ".TB_PREF."advance.`id` 
FROM ".TB_PREF."advance
INNER JOIN ".TB_PREF."employee ON ".TB_PREF."advance.`emp_id` =".TB_PREF."employee.`employee_id` 
...
WHERE (
(
".TB_PREF."advance.date >= '$datefrom'
AND ".TB_PREF."advance.date <=  '$dateto'
))";	
...
//	if ($approve != '')
//	{
//   		$sql .= " AND ".TB_PREF."advance.approve = ".db_escape($approve);
//	}
...
$sql = get_sql_for_employee_inquiry($_POST['employee_id'],$_POST['emp_dept'],$_POST['emp_grade'],
	$_POST['emp_desig'],$_POST['approve'],date2sql($_POST['datefrom']),date2sql($_POST['dateto']),$_POST['advance_against']);
 $cols = array(
			_("Employee Name"),
			_("Grade"),
			_("Desination"),
			...
			_("Remarks"), 
			_("Amount"),
			_("Advance Against")  => array('fun'=>'advance_against'),
			_("Approval")
