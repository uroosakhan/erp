...
if (!@$_GET['popup'])
{
	...
	page(_($help_context = "Leave Encashment Inquiry"), isset($_GET['employee_id']), false, "", $js);
}
...
function edit_link($row)
{
  ...
  return pager_link( _("Edit"),
    "/payroll/manage/leave_encashment.php?id=".$row['id'].'&date='.$row['date'], ICON_EDIT);
}

//------------------------------------------------------------------------------------------------
function get_sql_for_employee_inquiry($employee_id, $emp_dept, $emp_grade,$emp_desig,$approve, $datefrom,$dateto)
{
	$emp_grade =$_POST['grade'];
	$employee_id=$_POST['employee_id'];
	$emp_dept=$_POST['emp_dept'];
	$emp_desig=$_POST['emp_desig'];
$sql = " SELECT ".TB_PREF."employee.emp_name,".TB_PREF."grade.description As grade,".TB_PREF."desg.description As desination,".TB_PREF."dept.description,".TB_PREF."leave_encashment.payment_date,".TB_PREF."leave_encashment.remarks, ".TB_PREF."leave_encashment.`amount` ,IF(  `approve` =0,  'Unapprove',  'Approve' ) AS approve, ".TB_PREF."leave_encashment.`id` 
FROM ".TB_PREF."leave_encashment
INNER JOIN ".TB_PREF."employee ON ".TB_PREF."leave_encashment.`emp_name` =".TB_PREF."employee.`employee_id` 
...
WHERE (
(
".TB_PREF."leave_encashment.payment_date >= '$datefrom'
AND ".TB_PREF."leave_encashment.payment_date <=  '$dateto'
))";
...
*/
	...
	if ($approve != ALL_TEXT)
	{
   		$sql .= " AND ".TB_PREF."leave_encashment.approve = ".db_escape($approve);
	}
...
$sql = get_sql_for_employee_inquiry($_POST['employee_id'],$_POST['emp_dept'],$_POST['emp_grade'],$_POST['emp_desig'],$_POST['approve'],date2sql($_POST['datefrom']),date2sql($_POST['dateto']));
 $cols = array(
			_("Employee Name"),
			_("Grade"),
			_("Desination"),
