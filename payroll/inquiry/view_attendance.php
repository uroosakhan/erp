...
if (!@$_GET['popup'])
{
	...
	page(_($help_context = "View Attendance"), true, false, "", $js);
}
...
//	$_POST['id'] = $_GET['id'];
//}
function get_emp_attendance($month_id,$fiscal_year,$emp_id)
{
	$sql = "SELECT *  FROM ".TB_PREF."emp_attendance WHERE month_id=".db_escape($month_id)."
	AND fiscal_year =".db_escape($fiscal_year)."   AND empl_id=".db_escape($emp_id)."";
...
$result = get_emp_attendance($_GET['month_id'],$_GET['fiscal_year'],$_GET['empl_id']);

start_form();
start_table(TABLESTYLE, "width=50%");
$th = array(_("Date"),_("Check In"), _("Check Out"),"", "");
...
//    inactive_control_cell($myrow["id"], $myrow["inactive"], 'age', 'id');
