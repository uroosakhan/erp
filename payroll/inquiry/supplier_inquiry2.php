...
{
	return print_document_link($row['employee_id'], _("Print"), true, ST_10889, ICON_PRINT);
}
...
//------------------------------------------------------------------------------------------------
function get_sql_for_employee_inquiry($employee_id, $emp_dept, $grade, $emp_desig)
{
	$grade =$_POST['grade'];
	...
	$emp_desig=$_POST['emp_desig'];
        ...
        $sql = "SELECT  
	emp_code, emp_name,".TB_PREF."grade.description As grade,".TB_PREF."desg.description As desination,".TB_PREF."dept.description As department, emp_father, emp_cnic, DOB, j_date, l_date ,emp_reference, emp_home_phone, emp_mobile, emp_email,emp_bank, emp_address, notes,employee_id
...
INNER JOIN ".TB_PREF."desg ON ".TB_PREF."desg.`id` = ".TB_PREF."employee.`emp_desig`
	...
	if ($grade != ALL_TEXT)
	{
   		$sql .= " AND ".TB_PREF."employee.emp_grade = ".db_escape($grade);
	}
