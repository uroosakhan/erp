...
include($path_to_root . "/payroll/includes/db/location_db.inc");
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_loaction($selected_id, $_POST['description']);
			$note = _('Selected Description name has been updated');
    	} 
    	else 
    	{
    		add_loaction($_POST['description']);
...
$result = get_location12(check_value('show_inactive'));
...
while ($myrow = db_fetch($result)) 
{
	...
	inactive_control_cell($myrow["id"], $myrow["inactive"], 'location_name', 'id');
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_locations12($selected_id);
