...
function handle_submit(&$selected_id)
{
	  ...
	  for($i = 1; $i <= $_POST['count']; $i++)
		{
		add_overtime( $_POST['overtime'.$i],$_POST['employee_id'.$i], $_POST['employee_dept'], $_POST['date1']);
...
if (isset($_POST['Addattendance'])) 
{
	...
	$error=0;
	  if(can_process())
	  {
		$check = check_month_duplication($selected_id,$_POST['date1']); 
		if($check > 0)
		{
		...
		if($error==0)
		handle_submit($selected_id,$_POST['date1']);
...
//['present']:'','');
		text_cells( null,'overtime'.$a, $myrow['overtime'] ? $myrow['overtime'] :'','');
