...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_emp_deduction($selected_id, $_POST['description']);
			$note = _('Selected deduction has been updated');
			refresh('deduction.php');
    	} 
    	else 
    	{
    		add_emp_deduction($_POST['description']);
...
$result = get_emp_deductions(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_emp_deduction($selected_id);
