...
page(_($help_context = "Designation"));

include($path_to_root . "/payroll/includes/db/desg_db.inc");
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_emp_desg($selected_id, $_POST['description']);
			$note = _('Selected Designation has been updated');
    	} 
    	else 
    	{
    		add_emp_desg($_POST['description']);
...
$result = get_emp_desgs(check_value('show_inactive'));

start_form();
start_table(TABLESTYLE, "width=30%");
$th = array(_("ID"), _("Designation"), "Edit", "Delete");
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_emp_desg($selected_id);
...
text_row_ex(_("Designation:"), 'description', 30); 
