...
page(_($help_context = "Training"));

include($path_to_root . "/includes/ui.inc");
include($path_to_root . "/payroll/includes/db/training_db.inc");
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_training($selected_id, $_POST['description']);
			$note = _('Selected Description name has been updated');
    	} 
    	else 
    	{
    		add_training($_POST['description']);
...
$result = get_training(check_value('show_inactive'));
...
while ($myrow = db_fetch($result)) 
{
	...
	inactive_control_cell($myrow["id"], $myrow["inactive"], 'training', 'id');
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_trainings($selected_id);
