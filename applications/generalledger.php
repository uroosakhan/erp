...
class general_ledger_app extends application
{
	...
	{
		if (get_company_pref_display('ledger'))
            parent::__construct("GL", _(get_company_pref_display('ledger_text')));
		$this->add_module(_("Transactions"));
		if (get_company_pref_display('bank_payments_voucher'))
		...
		if (get_company_pref_display('journal_entry'))
			$this->add_lapp_function(0, _(get_company_pref_display('journal_entry_text')),
				"gl/gl_journal.php?NewJournal=Yes", 'SA_JOURNALENTRY', MENU_TRANSACTION);

		if (get_company_pref_display('budget_entry'))
        if (get_company_pref_display('budget_entry'))
		$this->add_lapp_function(0, _(get_company_pref_display('budget_entry_text')),
			"gl/gl_budget.php?", 'SA_BUDGETENTRY', MENU_TRANSACTION);


		if (get_company_pref_display('reconcile_account'))
			$this->add_lapp_function(0, _(get_company_pref_display('reconcile_account_text')),
				"gl/bank_account_reconcile.php?", 'SA_RECONCILE', MENU_TRANSACTION);

		if (get_company_pref_display('revenue_cost'))
			$this->add_lapp_function(0, _(get_company_pref_display('revenue_cost_text')),
		...
		if (get_company_pref_display('journal_inquiry'))
			$this->add_lapp_function(1, _(get_company_pref_display('journal_inquiry_text')),
...
//         $this->add_lapp_function(1, _("GL Accounts Inquiry"),
// 			"gl/inquiry/gl_account_types.php?", 'SA_GLTRANSVIEW', MENU_INQUIRY);
        ...
        if (get_company_pref_display('outstanding_cheques_inquiry'))
            $this->add_lapp_function(1, _(get_company_pref_display('outstanding_cheques_inquiry_text')),
                "gl/inquiry/outstanding_cheques.php?", 'SA_BANKTRANSVIEW', MENU_INQUIRY);
      ...
      //  if (get_company_pref_display('cash_flow_statment'))
			$this->add_rapp_function(1, _("Cash Flow Statment"),
		"gl/inquiry/cash_flow_statement.php?", 'SA_GLANALYTIC', MENU_INQUIRY);
		...
		if (get_company_pref_display('gl_account'))
			$this->add_rapp_function(2, _(get_company_pref_display('gl_account_text')),
				"gl/inquiry/gl_account_types.php?", 'SA_GLACCOUNT', MENU_ENTRY);
        ...
        if (get_company_pref_display('gl_account_transfer'))
		$this->add_rapp_function(2, _(get_company_pref_display('gl_account_transfer_text')),
			"gl/inquiry/gl_account_transfer_inquiry.php?", 'SA_ACCOUNTTRANSFER', MENU_MAINTENANCE);
