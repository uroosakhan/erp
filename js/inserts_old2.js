...
function mbselect(elm)
{
    ...
    var option_selected = selectedoption.innerHTML.replace(/&amp;/gi,'&');

    typer.setAttribute("value",option_selected);
    typer.value = option_selected;
    ...
    typer.onblur = function (event) {
        if (optgropc != 1) {
            pop("none");
            optgropc = 0;
            if (typer.value == null || typer.value == '' ||typer.value!=option_selected) {
                var selval=soption_selected;
                selval = selval.replace(/\<u\>/ig, '');
                selval = selval .replace(/\<\/u\>/ig, '');
                selval= selval.replace(/&nbsp;/gi,' ');
                typer.value = selval;
...
function addItemToCart(e){
    ...
    if (validate(e)) {
        ...
        if (asp && (asp.indexOf('process') !== -1))
            JsHttpRequest.request(e, null, 600000); // ten minutes for backup
        else
            JsHttpRequest.request(e);
    }
}
//asad
function _set_barcode_input(e){
...
var inserts = {
    'input': function(e) {
        ...
        if (e.name == '_barcode_edit') {
            _set_barcode_input(e);

            if(e.value.length){
                var AddItem = document.getElementsByName('AddItem')[0];
