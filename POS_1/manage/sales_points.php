...
{
	add_sales_point_pos($_POST['name'], $_POST['location'], $_POST['account'],
...
if ($Mode=='UPDATE_ITEM' && can_process())
{
	...
	update_sales_point_pos($selected_id, $_POST['name'], $_POST['location'],
...
if ($Mode == 'Delete')
{
	if (key_in_foreign_table($selected_id, 'users', 'pos'))
	{
		...
		delete_sales_point_pos($selected_id);
...
$result = get_all_sales_points_pos(check_value('show_inactive'));
...
if ($selected_id != -1)
{
 	...
 	if ($Mode == 'Edit') {
		$myrow = get_sales_point_pos($selected_id);
