...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_sales_group_pos($selected_id, $_POST['description']);
			$note = _('Selected sales group has been updated');
    	} 
    	else 
    	{
    		add_sales_group_pos($_POST['description']);
...
if ($Mode == 'Delete')
{
	...
	if ($cancel_delete == 0) 
	{
		delete_sales_group_pos($selected_id);
...
$result = get_sales_groups_pos(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_sales_group_pos($selected_id);
