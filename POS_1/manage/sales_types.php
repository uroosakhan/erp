...
{
	add_sales_type_pos($_POST['sales_type'], isset($_POST['tax_included']) ? 1:0,
	    input_num('factor'));
...
if ($Mode=='UPDATE_ITEM' && can_process())
{
	...
	update_sales_type_pos($selected_id, $_POST['sales_type'], isset($_POST['tax_included']) ? 1:0,
	     input_num('factor'));
...
if ($Mode == 'Delete')
{
	...
	else
	{
		...
		{
			delete_sales_type_pos($selected_id);
...
$result = get_all_sales_types_pos(check_value('show_inactive'));
...
if ($selected_id != -1)
{
 	...
 	if ($Mode == 'Edit') {
		$myrow = get_sales_type_pos($selected_id);
