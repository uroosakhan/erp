...
{ // first page call
	...
	if (isset($_GET['SInvoice'])) {
		...
		$inv = get_customer_trans_pos($_GET['SInvoice'], ST_SALESINVOICE);
		if($inv) {
...
if (list_updated('BranchID')) {
	...
	$br = get_branch_pos(get_post('BranchID'));
...
function can_process()
{
	...
	if (!check_num('discount')) {
		display_error(_("The entered WHT is not a valid number."));
		set_focus('discount');
		return false;
	}
...
if (get_post('AddPaymentItem') && can_process()) {
	...
	//Chaitanya : 13-OCT-2011 - To support Edit feature
	$payment_no = write_customer_payment_pos($_SESSION['alloc']->trans_no, $_POST['customer_id'], $_POST['BranchID'],
		$_POST['bank_account'], $_POST['DateBanked'], $_POST['ref'],
		input_num('amount'), input_num('discount'), $_POST['memo_'], $rate, input_num('charge'), input_num('gst_wh'));
...
function read_customer_data()
{
	...
	$myrow = get_customer_habit_pos($_POST['customer_id']);
...
if (isset($_GET['trans_no']) && $_GET['trans_no'] > 0 )
{
	...
	$new = 0;
	$myrow = get_customer_trans_pos($_POST['trans_no'], ST_CUSTPAYMENT);
	...
	$_POST["amount"] = price_format($myrow['Total'] - $myrow['ov_discount'] - $myrow['gst_wh'] );
	$_POST["gst_wh"] = price_format($myrow['gst_wh']);
...
start_form();
	...
	if (list_updated('customer_id') || ($new && list_updated('bank_account'))) {
		$_SESSION['alloc']->read();
		$_POST['memo_'] = $_POST['amount'] = $_POST['discount'] = $_POST['gst_wh'] = '';
		$Ajax->activate('alloc_tbl');
	}
...
$_POST['DateBanked']= get_current_company_date(3);
}
else{
   $_POST['DateBanked']; 
...
date_row(_("Date of Deposit:"), 'DateBanked', '', true, 0, 0, 0, null, true);
	...
	amount_row(_("WHT:"), 'discount');

	$display_discount_percent = percent_format($_POST['pymt_discount']*100) . "%";
		

	amount_row(_("GST WH:"), 'gst_wh');
	...
	if ($cust_currency != $bank_currency)
		display_note(_("Amount and WHT are in customer's currency."));
