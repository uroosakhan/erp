...
if (list_updated('branch_id')) {
	...
	$br = get_branch_pos(get_post('branch_id'));
...
/*	global $path_to_root, $pdf_debug,$def_print_orientation;
	...
	header("Location: ".$path_to_root."/reporting/prn_redirect.php?PARAM_0=$doc_no&PARAM_1=$doc_no&PARAM_2=&PARAM_3=0&PARAM_4=&PARAM_5=&PARAM_6=$def_orientation&REP_ID=1099");
*/
...
submenu_option(_("Modify Order"), "/POS/sales_order_entry.php?ModifyOrderNumber=" . $order_no."&&change=YES", ICON_DOC,
		_("You are about to generate DN and Invoice. Do you want to continue?") );
...
//	submenu_option(_("Select A Different &Order"),
	...
	submenu_option(_("Make &Sales Order Against This Quotation"),
		"/POS/sales_order_entry.php?NewQuoteToSalesOrder=$order_no");
	...
	submenu_option(_("Select A Different &Quotation"),
		"/POS/inquiry/sales_orders_view.php?type=".ST_SALESQUOTE);
	...
	submenu_option(_("Make &Invoice Against This Delivery"),
		"/POS/customer_invoice.php?DeliveryNumber=$delivery");

	if ((isset($_GET['Type']) && $_GET['Type'] == 1))
		submenu_option(_("Enter a New Template &Delivery"),
			"/POS/inquiry/sales_orders_view.php?DeliveryTemplates=Yes");
	else
		submenu_option(_("Enter a &New Delivery"), 
			"/POS/sales_order_entry.php?NewDelivery=0");
	...
	if ((isset($_GET['Type']) && $_GET['Type'] == 1))
		submenu_option(_("Enter a &New Template Invoice"), 
			"/POS/inquiry/sales_orders_view.php?InvoiceTemplates=Yes");
	else
		submenu_option(_("Enter a &New Direct Invoice"),
			"/POS/sales_order_entry.php?NewInvoice=0");
...
{
	$sql = "SELECT item_code FROM ".TB_PREF."item_codes ";
	$db = db_query($sql,'error');
	//$ft = db_fetch($db);
	...
	$sql ="SELECT `deal_amount` FROM `".TB_PREF."deal_amount` 
	INNER JOIN ".TB_PREF."item_codes ON ".TB_PREF."item_codes.item_code=".TB_PREF."deal_amount.`item_code`
	 WHERE ".TB_PREF."deal_amount.`item_code`='$item_code'";
	$db = db_query($sql,'error');
	$ft = db_fetch($db);
	return $ft[0];
...
function handle_new_item($stk_id)
{
	...
	add_to_order($_SESSION['Items'], $stk_id, $qty,
		$price, input_num('Disc') / 100, get_post('stock_id_text'),$deal_price,$stk_id);
...
function handle_new_item_old()
{
	...
	add_to_order($_SESSION['Items'], get_post('stock_id'), input_num('qty'),
		$price, input_num('Disc') / 100, get_post('stock_id_text'));
...
function  handle_cancel_order()
{
	...
	if ($_SESSION['Items']->trans_type == ST_CUSTDELIVERY) {
		...
		submenu_option(_("Enter a New Sales Delivery"),	"/POS/sales_order_entry.php?NewDelivery=1");
		...
		submenu_option(_("Enter a New Sales Invoice"),	"/POS/sales_order_entry.php?NewInvoice=1");
	} elseif ($_SESSION['Items']->trans_type == ST_SALESQUOTE)
	{
		delete_sales_order_pos(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);
		display_notification(_("This sales quotation has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Quotation"), "/POS/sales_order_entry.php?NewQuotation=Yes");
	} else { // sales order
		if ($_SESSION['Items']->trans_no != 0) {
			$order_no = key($_SESSION['Items']->trans_no);
			if (sales_order_has_deliveries_pos($order_no))
			{
                close_sales_order_pos($order_no);
				display_notification(_("Undelivered part of order has been cancelled as requested."), 1);
				submenu_option(_("Select Another Sales Order for Edition"), "/POS/inquiry/sales_orders_view.php?type=".ST_SALESORDER);
			} else {
				delete_sales_order_pos(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);

				display_notification(_("This sales order has been cancelled as requested."), 1);
				submenu_option(_("Enter a New Sales Order"), "/POS/sales_order_entry.php?NewOrder=Yes");
			}
		} else {
			processing_end();
...
{ 
	...
	elseif($type != ST_SALESORDER && $type != ST_SALESQUOTE && $trans_no != 0) { // this is template
		...
		if ($type == ST_SALESINVOICE) {
			$doc->due_date = get_invoice_duedate_pos($doc->payment, $doc->document_date);
			$doc->pos = get_sales_point_pos(user_pos());
		} else
