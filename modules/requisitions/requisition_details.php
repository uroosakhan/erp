...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1) 
	{
    	if ($selected_id != -1) 
    	{
    		update_requisition_detail($selected_id, $_POST['item_code'], $_POST['purpose'], input_num('order_quantity'), input_num('estimate_price'), $_POST['supplier_id']);
			display_notification(_('Selected requisition details has been updated.'));
    	} 
    	else 
    	{
    		add_requisition_detail($_POST['requisitionid'], $_POST['item_code'], $_POST['purpose'], input_num('order_quantity'), input_num('estimate_price'), $_POST['supplier_id']);
...
$th = array(_("Item Code"), _("Supplier"), _("Item Name"), _("Memo"), _("Qrder Quantity"), _("Estimate Price"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell(get_supplier_name($myrow["supplier_id"]));
	label_cell($myrow["description"]);
	label_cell($myrow["purpose"]);
	amount_cell_prc_only($myrow["order_quantity"]);
	amount_cell_prc_only($myrow["estimate_price"]);
...
text_row(_("Memo :"), 'purpose', null, 50, 5000);

$res = get_item_edit_info(get_post('item_code'));
$dec =  $res["decimals"] == '' ? 0 : $res["decimals"];
$units = $res["units"] == '' ? _('kits') : $res["units"];

qty_row_prc_only(_("Requisition Quantity:"), 'order_quantity', null, '', null, 2);
amount_row_prc_only(_("Estimate Price :"), 'estimate_price', null, null, null, 2);
