...
//	if ($etype == 6)
    {
        $fname = "OBCreditCustSample.csv";
        $sql = "SELECT master.debtor_no, branch.branch_code, master.name, branch.br_name, DATE_FORMAT(CURDATE(),'%d-%m-%Y') as date, 'OB' as code, '1' as qty, '' as amount, '' as optional_reference 
...
//		$sep = $_POST['sep'];
        ...
        while($data = fgetcsv($fp, 4096, $sep))
        {
            ...
            $Item = get_stock_master_info_credit_ob($stock_id);

            $trans_no = write_customer_import_credit_note($customer_id,
                $Item, $date, $qty, $price, $_POST['payment_terms'],
                $_POST['sales_type_id'], $_POST['dimension_id'],
                $_POST['dimension2_id'], $_POST['default_ship_via'],
                $_POST['location'], $reference, $comments, $due_date);

            if(!$trans_no)
                display_error('Could not be insert. '.$lines.'--'.$trans_no);
            else
                display_notification("Records Successfully. ".$lines.'--'.$trans_no);
        }
...
if ($action == 'import') {
    ...
    table_section_title("Import CSV Customer Credit");
...
//	item_tax_types_list_row("Item Tax Type:", 'tax_type_id', null);
    dimensions_list_cells("Dimension1 :", 'dimension_id', null, true, " ", false, 1);
    dimensions_list_row("Dimension1 :", 'dimension2_id', null, true, " ", false, 2);
