...
function getTransactions($from, $to,$employee,$approve)
{
...
$sql .= "AND ".TB_PREF."advance.date >=".db_escape($fromdate);
$sql .= "AND ".TB_PREF."advance.date<=".db_escape($todate);
...
if ($month_name == ALL_TEXT)
  ...
  if ($employee == ALL_TEXT)
	...
	$cols = array(0, 20, 70, 135, 165, 200, 230, 270, 320, 360, 410, 460, 510, 565);

	$headers = array(_('Employee'), _('Name'), _('Date'), _('Remarks'), _(''), _(''), _(''), _('Debit'), _(''), _('Credit'), _(''), _(''), _(''));
    ...
    $params =   array( 	0 => $comments,
				...
				4 => array('text' => _('Approval'), 'from' => $approval, 'to' => $to)   			
...
while ($myrow1 = db_fetch($result1))
{
	...
	$rep->TextCol(0, 5, $myrow1['emp_name']);			
    	$rep->Font();
	$rep->fontSize -= 2;
	//$rep->TextCol(7,8, $myrow1['basic_salary']);
    	$rep->NewLine();
$result = getTransactions($from, $to,$myrow1['employee_id'],$approve);
...
//		$rep->fontSize += 2;
		...
		$rep->TextCol(2, 3, $myrow['date']);
		if($myrow['amount'] > 0)
		{
	    $rep->TextCol(7, 8, $myrow['amount'], $dec);
		}
		else
		{
			 $rep->TextCol(9, 10, $myrow['amount'], $dec);
		}
		$rep->TextCol(3, 4, $myrow['remarks']);
