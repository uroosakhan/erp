...
$sql .= "AND ".TB_PREF."leave.from_date >=".db_escape($fromdate);
$sql .= "AND ".TB_PREF."leave.to_date<=".db_escape($todate);
...
if ($month_name == ALL_TEXT)
  ...
  if ($employee == ALL_TEXT)
	...
	$headers = array(_('Employee'), _('Name'), _('From Date'), _(''), _('To Date'), _(''), _('Leave Type'), _(''), _('No Of Leave'), _(''), _('Reasons'), _(''), _(''));
...
//		$rep->fontSize += 2;
		...
		$rep->TextCol(4, 5, $myrow['to_date']);
		//$rep->TextCol(2,3, get_emp_dept_name($myrow['emp_dept']));
	 $rep->TextCol(2, 3, $myrow['from_date'], $dec);
