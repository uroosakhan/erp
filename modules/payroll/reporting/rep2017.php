...
function getTransactions($from, $to,$employee,$approve)
{
	...
	$todate = date2sql($to);
	$sql = "SELECT * 
FROM  ".TB_PREF."overtime 
WHERE  `employee_id` =".db_escape($employee);
if ($dept != ALL_TEXT)
$sql .= "AND ".TB_PREF."overtime.emp_dept =".db_escape($dept);
$sql .= "AND ".TB_PREF."overtime.date >=".db_escape($fromdate);
$sql .= "AND ".TB_PREF."overtime.date<=".db_escape($todate);
...
if ($month_name == ALL_TEXT)
  ...
  if ($employee == ALL_TEXT)
	...
	$cols = array(0, 35, 70, 135, 160, 200, 230, 270, 320, 360, 410, 460, 510, 565);

	$headers = array(_('Employee'), _('Name'), _('Department Name'), _(''), _('Date'), _(''), _('Overtime'), _(''), _(''), _(''), _(''), _(''), _(''));
...
while ($myrow1 = db_fetch($result1))
{
	...
	//$rep->TextCol(2,3, get_emp_dept_name($myrow1['emp_dept']));
    	$rep->Font();
$result = getTransactions($from,$to,$myrow1['employee_id'],$dept);
...
//		$rep->fontSize += 2;
		...
		$rep->TextCol(4, 5, $myrow['date']);
		$rep->TextCol(2,3, get_emp_dept_name($myrow['emp_dept']));
	   // $rep->TextCol(2, 3, $myrow['emp_dept'], $dec);
		
			 
		
		$rep->TextCol(6, 7, $myrow['over_time']);
