...
$orientation = 'L';
    $rep = new FrontReport(_(''), "SupplierBalances", user_pagesize(), 14, $orientation);
...
//	$man_month=get_emp_man_month_from_emp($result['emp_id']);
	$rep->NewPage();
	$rep->MultiCell(100, 150, "The Manager" ,0, 'L', 0, 2, 50,50, true);
	$rep->MultiCell(100, 150, "No.3/9" ,0, 'L', 0, 2, 650,50, true);
	$rep->MultiCell(100, 100, "Bank Alfalah" ,0, 'L', 0, 2, 50,70, true);
	$rep->MultiCell(120, 100, "24-January-2017" ,0, 'L', 0, 2, 650,70, true);
	$rep->MultiCell(100, 100, "Lal Kothi" ,0, 'L', 0, 2, 50,90, true);

	$rep->setfontsize(16);
	$rep->font('b');
	$rep->MultiCell(422.5, 15, "Salary Payment For Month of December 2016" ,0, 'L', 0, 2, 220,150, true);
	$rep->font('');
	//$rep->setfontsize(-16);
	$rep->setfontsize(12);
	$rep->MultiCell(700, 40, "Kindly remit the following amounts in names and accounts of our staff member maintaining in your Askari Ltd. and debit the total amount Rupees                          so remitted to Techno Consult Intl. (Pvt) Ltd , A/C No.               in your bank." ,0, 'L', 0, 2, 80,180, true);
	$rep->MultiCell(700, 10, "_______________________________________________________________________________________________________" ,0, 'L', 0, 2, 80,210, true);
	$rep->MultiCell(700, 30, "" ,1, 'L', 0, 2, 80,245, true);
	$rep->font('b');
	$rep->MultiCell(100, 30, "Employee Name" ,0, 'L', 0, 2, 100,253, true);
	$rep->MultiCell(100, 30, "Net Salary" ,0, 'L', 0, 2, 300,253, true);
	$rep->MultiCell(100, 30, "Account No" ,0, 'L', 0, 2, 450,253, true);
	$rep->MultiCell(100, 30, "Remarks" ,0, 'L', 0, 2, 620,253, true);
	$rep->MultiCell(700, 10, "__________________________________________________________________" ,0, 'L', 0, 2, 80,280, true);
	$rep->MultiCell(150, 10, "Grand Total:" ,0, 'L', 0, 2, 80,300, true);
	$rep->MultiCell(700, 10, "__________________________________________________________________" ,0, 'L', 0, 2, 80,280, true);
	$rep->MultiCell(150, 10, "Rupees:" ,0, 'L', 0, 2, 80,330, true);
	$rep->font('');
	$rep->MultiCell(700, 40, "Kindly send us the list of beneficiaries and amount of them with advice number." ,0, 'L', 0, 2, 80,350, true);
	$rep->MultiCell(100, 40, "Thank You," ,0, 'L', 0, 2, 80,365, true);
	$rep->MultiCell(100, 40, "Very truely Yours " ,0, 'L', 0, 2, 600,380, true);
	$rep->MultiCell(100, 40, "for " ,0, 'L', 0, 2, 580,400, true);
	$rep->font('b');
	$rep->MultiCell(220, 40, "TECHNO-CONSULT INTL., (Pvt), Ltd.," ,0, 'L', 0, 2, 600,400, true);
	$rep->MultiCell(150, 40, "Mr Saleem Ahmed" ,0, 'L', 0, 2, 600,420, true);
