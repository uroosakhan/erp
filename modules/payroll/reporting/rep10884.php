...
function get_employee_name_n($id)
{
...
function get_employment_nomination($selected_id)
{
	$sql = "SELECT * FROM ".TB_PREF."employee_nomination WHERE employee_id=" .db_escape($selected_id);
...
//	
		...
		$rep->multicell(270, 20, " ", 1, 'C', 0, 0, 98, 92, true);
		$rep->multicell(270, 20, get_employee_name_n($employee), 0, 'L', 0, 0, 105, 95, true);
//		$rep->Font('','','');
		$rep->MultiCell(560, 50, "", 1, 'C', 0, 0, 25, 120, true);
		$rep->MultiCell(150, 50, "", 1, 'C', 0, 0, 25, 120, true);
		$rep->MultiCell(150, 50, "Nominee Name:", 0, 'C', 0, 0, 25, 135, true);
		$rep->MultiCell(80, 50, "", 1, 'C', 0, 0, 175, 120, true);
		$rep->MultiCell(80, 50, "Relation With Nominee", 0, 'C', 0, 0, 175, 135, true);
		$rep->MultiCell(80, 50, "", 1, 'C', 0, 0, 255, 120, true);
		$rep->MultiCell(80, 50, "Nominee Age", 0, 'C', 0, 0, 255, 135, true);
		$rep->MultiCell(93, 50, "", 1, 'C', 0, 0, 335, 120, true);
		$rep->MultiCell(93, 50, "Nominee Share", 0, 'C', 0, 0, 335, 135, true);
	...
	$employee_nomination = get_employment_nomination($employee);
	$rep->NewLine(-20);
	while ($myrow=db_fetch($employee_nomination))
	{

		$rep->TextCol(0, 1, $myrow['nominee_name']);
		$rep->TextCol(2, 3, "          ".$myrow['relation']);
		$rep->TextCol(2, 4,"                                         ".$myrow['age']);
		$rep->TextCol(4, 5, "          ".$myrow['share']);
