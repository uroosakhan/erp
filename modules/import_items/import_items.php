...
if (isset($_POST['export'])) {
    ...
    if ($etype == 3) {
        ...
        $sql = "SELECT 'BUY' as type, pd.stock_id, '' as dummy, pd.supplier_description, s.supp_name as supplier, pd.suppliers_uom, pd.conversion_factor, '' as dummy1, '" . $currency ."', pd.price, pd.discount from ".TB_PREF."purch_data pd left join ".TB_PREF."suppliers s on pd.supplier_id = s.supplier_id where 1";
    }
    if ($etype == 4) {
        $fname = "uom.csv";
...
if (isset($_POST['import'])) {
	if (isset($_FILES['imp']) && $_FILES['imp']['name'] != '') {
		...
		while ($data = fgetcsv($fp, 4096, $sep)) {
			if ($lines++ == 0) continue;
			list($type, $code, $id, $description, $category, $units, $qty, $mb_flag, $currency, $price, $discount) = $data;
			...
			if ($type == 'ITEM' || $type == 'KIT' || $type == 'PRICE') {
				...
				$row = db_fetch_row($result);
				if (!$row) add_item_price($code, $_POST['sales_type_id'], $currency, $price, $discount);
				else update_item_price($row[0], $_POST['sales_type_id'], $currency, $price, $discount);
				$pr++;
			    }
			}
			if ($type == 'BUY') {
				...
				if (check_stock_id($code)) {
					...
					if ($supplier_id == 0) display_notification("Supplier $category not found");
						...
						if (!$row) $sql = "INSERT INTO ".TB_PREF."purch_data
							...
							supplier_description, discount)
							VALUES ($supplier_id, $code, $price, $units, $qty, $descr, $discount)";
						else $sql = "UPDATE ".TB_PREF."purch_data SET price = $price,
							supplier_description = $descr, suppliers_uom = $units,
							conversion_factor = $qty, discount = $discount
