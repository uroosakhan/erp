...
// S.H
function get_stock_master_records($stock_id)
{
    ...
    return $fetch['TotalRecords'];
}
// S.H
function get_loc_stock_records($stock_id, $loc_stock='')
{
    $sql = "SELECT COUNT(*) as TotalRecords 
            FROM 0_loc_stock 
            WHERE stock_id=".db_escape($stock_id);
//    if($loc_stock != '')
        $sql .= " AND loc_code=".db_escape($loc_stock);
    $query = db_query($sql, "Error");
    $fetch = db_fetch($query);
    return $fetch['TotalRecords'];
}
...
if (isset($_POST['import'])) {
    if (isset($_FILES['imp']) && $_FILES['imp']['name'] != '') {
        ...
        while ($data = fgetcsv($fp, 4096, $sep)) 
        {
            ...
            list($item_code, $stock_id, $description, $category2, $units, $mb_flag, $price) = $data;


            $sql = "SELECT COUNT(*) AS Total FROM ".TB_PREF."stock_master WHERE stock_id = ".db_escape($item_code);
            $query = db_query($sql, "Error");
            $fetch  = db_fetch($query);
            
            if($fetch['Total'] == 0)
            {
                $sql = "SELECT category_id FROM ".TB_PREF."stock_category WHERE description = ".db_escape($category2);
                $query = db_query($sql, "Error");
                $category1 = db_fetch_row($query);
                $category = $category1[0];
			    if (!$category){
			        add_item_category($category2);
			        	$category = db_insert_id();
			    }
				
                

                $sql1 = "INSERT INTO ".TB_PREF."stock_master (stock_id, description, long_description,
                         carton, category_id,  tax_type_id, units, mb_flag, sales_account, inventory_account, cogs_account,
                            adjustment_account,  wip_account, dimension_id, dimension2_id)
                            VALUES ('$item_code', " . db_escape($description) . ", '', '',
                            '$category', {$_POST['tax_type_id']}, '$units', '$mb_flag',
                            '{$_POST['sales_account']}', '{$_POST['inventory_account']}', '{$_POST['cogs_account']}',
                            '{$_POST['adjustment_account']}',
                             0,'$dim', 0)";
                db_query($sql1, "The item could not be added");
                
                $sql2 = "INSERT INTO ".TB_PREF."loc_stock (loc_code, stock_id)
                        SELECT ".TB_PREF."locations.loc_code, ".db_escape($stock_id)
                        ." FROM ".TB_PREF."locations";
                db_query($sql2, "The item locstock could not be added");
    
                add_item_code($stock_id, $stock_id, $description, $category, 1, 0);
    
                add_item_price($item_code, $_POST['sales_type_id'], $currency, $price); 
            }
            

//            $StockCheck = get_stock_master_records($stock_id);
           /* if ($StockCheck != 0)
            {
                $LocCheckForInsert = get_loc_stock_records($stock_id, 'SH');
                if($LocCheckForInsert == 0)
                {
                    $sql = "INSERT INTO ".TB_PREF."loc_stock (stock_id, loc_code) VALUES (
		                    ".db_escape($stock_id).", 'SH')";
                    db_query($sql, "The item loc stock could not be added");
                }
                $LocCheck = get_loc_stock_records($stock_id, 'SH');
                if($LocCheck != 0)
                {
                    $sql = "UPDATE ".TB_PREF."loc_stock SET 
                        standard_cost = ".db_escape($standard_cost);
                    $sql .= " WHERE stock_id=".db_escape($stock_id);
                    $sql .= " AND loc_code=".db_escape('SH');
                    db_query($sql, "The item could not be updated");

                    display_notification("Line $lines: Updated $id");
                }
            }*/

            /*$type = strtoupper($type);
            $mb_flag = strtoupper($mb_flag);
            $price = user_numeric($price);
            ...
            if ($type == 'ITEM') {
                ...
                if (!$row) {
                    ...
                    $category_record = get_item_category($cat);

                    $sql = "INSERT INTO ".TB_PREF."stock_master (stock_id, description, long_description,
                     carton, category_id,  tax_type_id, units, mb_flag, sales_account, inventory_account, cogs_account,
                        adjustment_account, assembly_account, wip_account, dimension_id, dimension2_id)
                        VALUES ('$id', " . db_escape($description) . ", '', '',
                        '$cat', {$_POST['tax_type_id']}, '$units', '$mb_flag',
                        '{$_POST['sales_account']}', '{$_POST['inventory_account']}', '{$_POST['cogs_account']}',
                        '{$_POST['adjustment_account']}', '{$_POST['assembly_account']}',
                         '{$category_record["dflt_wip_account"]}',$dim, 0)";
                    ...
                    $sql = "UPDATE ".TB_PREF."stock_master SET long_description='',
                        ...
                        wip_account='{$category_record["dflt_wip_account"]}',
                        dimension_id=$dim,
                        dimension2_id=0,
                        units='$units',
                        mb_flag='$mb_flag',
            ...
            display_notification("$i item posts created, $j item posts updated.");
        }
...
if ($action == 'import') {
    ...
    locations_list_row("To Location:", 'location',  null, false, false, $_SESSION["wa_current_user"]->user);
