                ...
                if ($description == "CASH") {
                    ...
                    if($fetch1['TotalRecord'] == 0)
                    {
                        $total += add_gl_trans(ST_CUSTPAYMENT, $trans_no, $date,
                            '101001', $dimension_id, 0, '', -$fetch['Total'], 'PKR',
            ...
            if($StockAllow == 3 || $StockAllow == 5 || $StockAllow == 7 || $StockAllow == 8) {
                ...
                if($description == "CASH") {
                    ...
                    while($myrow = db_fetch($query2))
                    {
                        ...
                        $sql = "SELECT * FROM ".TB_PREF."discount WHERE discount_type = ".db_escape($myrow['text4']);
                        $query = db_query($sql, "Error");
                        $fetch = db_fetch($query);
                        if ($lineDiscount > 0)
                    ...
                    if($Allow == 1) {
                        ...
                        {
                            $sql = "SELECT * FROM ".TB_PREF."discount WHERE discount_type = ".db_escape($header_fetch['chalan']);
                            $query = db_query($sql, "Error");
                            $fetch = db_fetch($query);
                            $total += add_gl_trans_customer(ST_SALESINVOICE, $invoice_no, $date, $fetch['dis_account'], $dimension_id, $dim2,
...
//                    if($StockAllow == 6)
                if ($header_fetch['discount1'] > 0)
                    $total += add_gl_trans_customer(ST_SALESINVOICE, $invoice_no, $date, 402033, $dimension_id, $dim2,
            ...
            if($description == 'VOID VISA:')//For Payment VOID
            {
                $sql5 = " UPDATE ".TB_PREF."gl_trans SET amount = 0 WHERE type_no = $order_no AND type = 12";
                db_query($sql5, "order Details Cannot be Added");
                $sql5 = " UPDATE ".TB_PREF."gl_trans SET amount = 0 WHERE type_no = $order_no AND type = 10";
                db_query($sql5, "order Details Cannot be Added");
            }
            if($description == 'VOID MASTER:')//For Payment VOID
            {
                $sql5 = " UPDATE ".TB_PREF."gl_trans SET amount = 0 WHERE type_no = $order_no AND type = 12";
                db_query($sql5, "order Details Cannot be Added");
                $sql5 = " UPDATE ".TB_PREF."gl_trans SET amount = 0 WHERE type_no = $order_no AND type = 10";
