...
//		$sep = $_POST['sep'];
		...
		while($data = fgetcsv($fp, 4096, $sep))
		{
			...
			if ($lines++ == 0) continue;
			list($trans_no, $customer, $branch, $name, $br_name, $date, $stock_id, $qty, $price, $reference, $comments, $location, $dimension_id, $salesperson, $cash_account) = $data;
			$customer_id = get_debtorno_from_customer_table($customer);
            $checked = check_duplication($trans_no, $stock_id);
            $Item = get_stock_master_info($stock_id);
...
//              2 = return sales orders error
			...
			$order_no = add_sales_order_import($customer_id['customer_id'], $customer_id['branch_id'],
            $date, $stock_id, $qty, $Item['description'], $price, $customer_id, $_POST['payment_terms'], $_POST['sales_type_id'],
            $Item, $comments, $trans_no, $location, $dimension_id);
			...
			$delivery_no = write_sales_delivery_import($customer_id['customer_id'], $customer_id['branch_id'],
            $date, $stock_id, $qty, $Item['description'], $price, $order_no, $_POST['payment_terms'], $_POST['sales_type_id'],
            $Item, $customer_id, $comments,$trans_no, $location, $dimension_id, $t_price[0]);
			$invoice_no = write_sales_invoice_import($customer_id['customer_id'], $customer_id['branch_id'],
            ...
            $comments, $trans_no, $location, $dimension_id, $cash_account, $t_price[0]);
